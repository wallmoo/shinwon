...
<body class="page-login page-join"><!-- page class : page-login -->
				...
				<form id="frm" name="frm">
					...
					<dl>
						...
						<dd class="dd_SubsInfo">
							<p class="tx_msg">
								<span class="co_blu">귀하는 온라인회원으로 다음과 같이 가입되어 있습니다. 가입정보를 확인해주세요.</span>
							</p>
							<table>
								<caption>가입정보확인에 온라인 가입일, 아이디로 구성된 표</caption>
								<colgroup>
								<col style="width:45%" />
								<col />
								</colgroup>
								<tbody>
									<tr>
										<th><strong>온라인 가입일</strong></th>
										<td><ui:formatDate pattern="yyyy.MM.dd" value="${commandMap.MEM_MST_REG_DT }"/></td>
									</tr>
									<tr>
										<th><strong>아이디</strong></th>
										<td><c:out value="${commandMap.maskingId }"/></td>
									</tr>
								</tbody>
							</table>
...
$(function(){
	...
	goChangeAgree = function(){
		$("#frm").attr({"action":"<c:out value="${mobileSslDomain}" />/mobile/member/onlineMemStep2.do", "target":"_self", "method":"post"}).submit();
	};
});
//]]>
</script>
