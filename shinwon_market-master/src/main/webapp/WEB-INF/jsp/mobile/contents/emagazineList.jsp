        ...
        <c:when test="${commandMap.MGZ_MST_CTG eq '2279'}">
			...
			<ul class="list_d _scroll_list_ck" id="emagazineList">
				<c:forEach var="row" items="${list }" varStatus="i">
		            ...
		            <a class="_pg_link" href="javascript:callJavascriptNextPage('<c:out value="${mobileDomain}" />/mobile/contents/emagazineView.do?MGZ_MST_IDX=<c:out value="${row.MGZ_MST_IDX}" />', '_slef');">
		            <c:choose>
		            ...
		            <c:if test="${row.MGZ_MST_KID eq 'M'}">
				    	...
				    	<c:if test="${not empty row.IMG_PATH_MB  and row.MGZ_MST_KID eq 'S' }">
