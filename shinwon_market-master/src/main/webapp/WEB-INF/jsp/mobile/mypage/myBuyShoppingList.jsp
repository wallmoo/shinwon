...
<body class="page-mypage"><!-- page class : page-login -->
	...
	<section id="wrap">
        ...
        <div class="bbs-status"><!-- bbs-status -->
            내가 구매한 상품 <span class="warning"><fmt:formatNumber value="${totalCount }" groupingUsed="true" /></span>개 
        </div><!-- // bbs-status -->

        <div class="ct section-button bbs-button"><!-- section-button -->
            <a href="javascript:;" id="allCheck" class="button small">전체선택</a>
            <a href="javascript:mutInsertCart();" class="button small">장바구니 담기</a>
        </div><!-- // section-button -->
		<form name="cartForm" id="cartForm">
        <div class="padder product-list-wrap my-product"><!-- product-list-wrap -->
        	<c:choose>
					...
					<c:forEach var="listProduct" items="${listProduct }" varStatus="status">
						<li>
		                    <div class="module">
		                        ...
		                        <a href="javascript:callJavascriptNextPage('<c:out value="${mobileDomain }"/>/mobile/product/product.do?prdMstCd=<c:out value="${listProduct.PRD_MST_CD }" />','_self');">
		                            <span class="thumb"><img data-original="<c:out value="${cdnDomain}" /><c:out value="${listProduct.PRD_IMAGE}" />" src="../nas/cdn/mobile/img/lay/grey.gif" alt="<c:out value="${listProduct.PRD_IMAGE_ALT}" />" /></span>
		                        ...
		                        <c:if test="${listProduct.PRD_MST_ERP_PRC > 0 }">
			                        <p class="price-before">
			                            <span class="blind">소비자가</span><strong><fmt:formatNumber value="${listProduct.PRD_MST_ERP_PRC }" groupingUsed="true" /></strong>원
			                        </p>
		                        </c:if>
		                        <p class="price-after">
		                            <span class="blind">판매가</span><strong class="large"><fmt:formatNumber value="${listProduct.PRD_MST_PRICE }" groupingUsed="true" /></strong>원
		                        </p>
		                        <p class="date">
		                            <span class="small">주문일 <ui:formatDate value="${listProduct.ORD_MST_ORD_DT }" pattern="yyyy.MM.dd"/></span>
		                        </p>
		                    </div>
		                    <div class="action">
		                    	<c:choose>
                                	<c:when test="${listProduct.PRD_MST_IVB_YN eq 'Y'}">
                                    	<a href="javascript:setPaymentData('<c:out value="${listProduct.PRD_MST_CD }"/>','<c:out value="${listProduct.PRD_MST_OPT_EST_CNT}" />');" class="button warning"><span>상담예약</span></a>
                                    </c:when>
                                    <c:otherwise>
                                    	<a href="javascript:setPaymentData('<c:out value="${listProduct.PRD_MST_CD }"/>','<c:out value="${listProduct.PRD_MST_OPT_EST_CNT}" />');" class="button warning"><span>바로구매</span></a>
                                    </c:otherwise>
                                </c:choose>
		                        <a href="javascript:insertCart('<c:out value="${listProduct.PRD_MST_CD }"/>','<c:out value="${listProduct.PRD_MST_OPT_EST_CNT}" />');" class="button primary"><span>장바구니담기</span></a>
		                    </div>
		                </li>
					</c:forEach>
					</ul>
...
$(function(){
	goPage = function (cPage) 
    {
        ...
        $pageFrm.attr({"action":"<c:out value="${mobileDomain}" />/mobile/mypage/myBuyShoppingList.do", "target":"_self", "method":"post"}).submit();
    };
	...
	goProductView = function(prdMstCd){
		if(prdMstCd != ""){
			...
			callJavascriptNextPage('<c:out value="${mobileDomain}"/>/mobile/product/product.do?prdMstCd='+prdMstCd,'_self');
		}else{
