		...
		<!-- .cate_gnb S -->
	        ...
	        <div class="tab-a" data-role="navbar"><!-- tab-a -->
	            <ul>
	                ...
	                <li class="active"><a href="javascript:;">키핑기획전</a><span class="blind">현재위치</span></li>
	        ...
	        <div class="bbs-status"><!-- bbs-status -->
            	내가 키핑한 기획전 <span class="warning"><fmt:formatNumber value="${totalCount }" groupingUsed="true" /></span>개 
	        </div><!-- // bbs-status -->
	
	        <div class="ct section-button bbs-button"><!-- section-button -->
	            <div class="fl">
	                <a href="javascript:;" id="allCheck" class="button small _pg_link">전체선택</a>
	                <a href="javascript:selectedDeleteBtn();" class="button small _pg_link">선택기획전 삭제</a>
	            </div>
	            <div class="fr">
	                <a href="javascript:endDeleteBtn();" class="button small _pg_link">종료된 기획전 삭제</a>
	            </div>
	        </div><!-- // section-button -->
...
$(function(){
	...
	goPage = function (cPage) 
    {
        ...
        $pageFrm.attr({"action":"<c:out value="${mobileDomain}" />/mobile/mypage/myKeepPlanList.do", "target":"_self", "method":"post"}).submit();
    };
	...
	deleteKeepPlan = function(idx_arr) {
		$.ajax({
					...
					url : "<c:out value="${mobileDomain}" />/mobile/mypage/deleteKeepPlanAjax.do",
	...
	endDeleteBtn = function(){
		if(confirm("종료된 기획전을 삭제하시겠습니까?")){
			$.ajax({
				async : false,
				type : "POST",
				url : "<c:out value="${mobileDomain}" />/mobile/mypage/deleteEndKeepPlanAjax.do",
				success : function(data) {
					if(data.result == "Y"){
						alert("삭제되었습니다.");
						location.reload(true);							
