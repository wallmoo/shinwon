...
<body class="page-mypage"><!-- page class : page-login -->
	...
	<section id="wrap">
		...
		<article id="container" class="container" data-role="content">
	        ...
	        <table class="table-d table-col coupon-brand" border="0" cellpadding="0" cellspacing="0"><!-- table-d -->
	        <caption>브랜드, 적용 상품, 쿠폰유효기간으로 구성된 쿠폰 적용 브랜드목록입니다.</caption>
	        <colgroup>
	            <col style="width: 27%;">
	            <col style="width: 27%;">
	        	...
	        	<c:choose>
	        		<c:when test="${not empty list }">
	        			<c:forEach var="list" items="${list }" varStatus="status">
		        			<tr>
				                <td><a href="javascript:;"><c:out value="${list.BND_MST_BRAND_NM }" /></a></td>
				                <td>모든상품</td>
				                <td>
				                	<c:choose>
...
$(function(){
	goProduct = function(prdMstCd){
		window.open("<c:out value="${mobileDomain }"/>/pc/product/product.do?prdMstCd="+prdMstCd);
		self.close();
	};
	...
	goPage = function (cPage) 
    {
        ...
        $pageFrm.attr({"action":"<c:out value="${mobileDomain}" />/mobile/mypage/myCouponBrandPopup.do", "target":"_self", "method":"post"}).submit();
    };
}); //function
//]]>
</script>
