...
<section id="wrap">
	...
	<article id="container" class="container" data-role="content">
        ...
        <div class="order-number"><!-- order-number -->
            <span class="large"><span class="warning">13245678009123</span><br />주문을 취소하시겠습니까?</span>
        </div><!-- // order-number -->
        ...
        <caption>상품정보, 총결제금액, 진행현황으로 구성된 주문취소 상품목록입니다.</caption>
        <colgroup>
        ...
        <tbody>
            <tr>
                <td class="text-left">
                    <c:out value="${orderCancelInfo.PRD_MST_NM}" />
                    <c:if test="${orderCancelInfo.ORD_PRD_CNT gt 1}">
						<br />
							외 <fmt:formatNumber value="${orderCancelInfo.ORD_PRD_CNT - 1}" type="number" />건
					</c:if>
                </td>
                <td><fmt:formatNumber value="${orderCancelInfo.ORD_MST_PAY_PRC}" type="number" />원</td>
                <td><c:out value="${orderCancelInfo.ORD_PRD_ORD_STATE_NM }" /></td>
            </tr>
        </tbody>
        </table><!-- // table-a -->
        ...
        <div class="padder mt10">
            <p class="info">
                주문상품 중 일부만 취소하실 경우 주문취소 후 재주문 하시기 바랍니다.
            </p>
        </div>

        <h4 class="title">취소사유 (<span class="essential"><em class="blind">필수입력 항목은</em></span> 필수선택 )</h4>

        <table class="table-b table-row" border="0" cellpadding="0" cellspacing="0"><!-- table-b -->
        <caption>사유, 상세사유로 구성된 취소사유 입력 테이블입니다.</caption>
        <colgroup>
            <col style="width: 22%;">
            <col style="width: auto;">
        </colgroup>
		...
		<c:if test="${orderPayInfo.ORD_MST_PAY_DT ne '00000000000000' and (orderPayInfo.ORD_MST_PAY_TYPE eq Code.PAY_DEPOSIT or orderPayInfo.ORD_MST_PAY_TYPE eq Code.PAY_ESCROW)}">
	        ...
	        <tbody>
	            ...
	            <tr>
	                ...
	                <td>
	                	<input type="text" id="ORD_MST_RFD_ACT" name="ORD_MST_RFD_ACT" class="text full" maxlength="20" value="<c:out value="${orderPayInfo.ORD_MST_RFD_ACT }"/>"/>
	                </td>
	            </tr>
	            <tr>
        ...
        <div class="section-button"><!-- section-button -->
            <a href="javascript:;" id="goConfirm" class="button warning"><span>주문취소</span></a>
