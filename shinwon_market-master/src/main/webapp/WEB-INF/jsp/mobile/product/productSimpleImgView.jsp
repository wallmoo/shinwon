	...
	<section id="wrap">
		...
		<article id="container" class="container" data-role="content">
			...
			<dl class="img_zoom_list">
				<dt class="blind"><c:out value="${productInfo.PRD_MST_NM_MB }" /> 확대 이미지</dt>
				<dd class="dd_img">
					<%-- <a href="javascript:void(0);" data-img="<c:out value="${cdnDomain}" />/<c:out value="${productInfo.PRD_IMAGE}" />"><img src="<c:out value="${cdnDomain}" />/<c:out value="${productInfo.PRD_IMAGE}" />" alt="<c:out value="${productInfo.PRD_IMAGE_ALT}" />" /></a> --%>
					<c:if test="${not empty listProductImages }">
						<c:forEach var="listProductImages" items="${listProductImages }" varStatus="i">
							<c:if test="${fn:indexOf(listProductImages.large,Code._NO_IMAGE_MB_DETAIL_) == -1 }">
								<a href="javascript:void(0);" data-img="<c:out value="${listProductImages.large}" />"><img src="<c:out value="${listProductImages.large}" />" alt="<c:out value="${listProductImages.alt}" />" /></a>
							</c:if>
						</c:forEach>
					</c:if>
				</dd>
