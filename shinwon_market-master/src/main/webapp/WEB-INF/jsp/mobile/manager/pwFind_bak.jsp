			...
			<section class="inquiry_tab_d">
				<dl class="tab_st1">
					<dt class="tab_mn ui-grid-a">
						...
						<a href="javascript:void(0)" class="_pg_link ui-block-b on" ><strong>비밀번호 찾기</strong></a>
					</dt>
					...
					<dd class="dd_inq_certifi">
						...
						<form id="basicForm" name="basicForm">
							...
							<input type="hidden" name="RST_YN" id="RST_YN"/>
							
							<div class="inp_form memberBasic" style="display:block;">
								<label for="basicMemId">
									<strong>아이디</strong>
									<input type="text" name="MEM_MST_MEM_ID" id="basicMemId" maxlength="15" />
								</label>
								<label for="basicMemNm">
									<strong>이름</strong>
									<input type="text" name="MEM_MST_MEM_NM" id="basicMemNm" maxlength="10"/>
								</label>
							</div>
						</form>
	
						<div class="certifi_d memberBasic" style="display:block;">
							<dl class="form_phone">
								<dt>
						...
						<form id="simpleForm" name="simpleForm">
							...
							<div class="inp_form" id="memberSimple" style="display:none;">
								<label for="simpleMemId">
									<strong>아이디</strong>
									<input type="text" name="MEM_MST_MEM_ID" id="simpleMemId" maxlength="15"/>
								</label>
								<label for="simpleMemNm">
									<strong>이름</strong>
									<input type="text" name="MEM_MST_MEM_NM" id="simpleMemNm" maxlength="10"/>
								</label>
								
								<div class="inp_email">
									<strong>이메일</strong>
									<div>
										<input type="text" name="MEM_MST_MAIL" id="simpleMemMail" maxlength="50"/>
									</div>
								</div>
								
								<p class="tx_ex2">
									<span>회원정보에 입력하신 정보와 동일하게 입력해주시기 바랍니다. </span>
									<span>등록하신 아이디, 이름, 이메일을 확인하여 회원가입시 기재하신 이메일로 임시비밀번호를 발송해드립니다.</span>
								</p>
		
								<div class="btn_box">
									<a href="javascript:goPwFind();" class="_pg_link btn_red"><strong>확인</strong></a>
								</div>
							</div>
						</form>
						<!-- 간편회원 비밀번호 찾기 E -->
						...
						<form id="businessForm" name="businessForm">
							...
							<div class="inp_form" id="memberBusiness" style="display:none;">
								<label for="businessMemId">
									<strong>아이디</strong>
									<input type="text" name="MEM_MST_MEM_ID" id="businessMemId" maxlength="15"/>
								</label>
								...
								<div class="inp_corNumer" style="margin-top:10px;">
...
$(function(){
	...
	goBasicForm = function()
	{
		$.ajax({
	        async : false,
	        type : "POST",
	        data : {"MEM_MST_MEM_ID":$("#basicMemId").val(),"MEM_MST_MEM_NM":$("#basicMemNm").val(),"MEM_IPN_CI":$("#MEM_IPN_CI").val()},
	        url : "<c:out value="${mobileSslDomain}" />/mobile/manager/checkMemInfoAjax.do",
	        success : function (data) {
	            if(data.successYn == "Y")
	            {
	                ...
	                {
	                	$basicForm.attr({"action":"<c:out value="${mobileSslDomain}" />/mobile/manager/pwFindResult.do", "target":"_self", "method":"post"}).submit();
	                }
...
// 	                return;
	...
	goPwFind = function()
	{
		...
		if(searchType == "memberSimple"){
			...
			if($simpleForm.valid()){
				$simpleForm.attr({"action":"<c:out value="${mobileSslDomain}" />/mobile/manager/pwFindResult.do", "target":"_self", "method":"post"}).submit();	    			
			}	    			
		}
		// 기업회원의 경우
		if(searchType == "memberBusiness"){
			...
			if($businessForm.valid()){
				$("#businessForm").attr({"action":"<c:out value="${mobileSslDomain}" />/mobile/manager/pwFindResult.do", "target":"_self", "method":"post"}).submit();	    			
			}
		}
	};
});
