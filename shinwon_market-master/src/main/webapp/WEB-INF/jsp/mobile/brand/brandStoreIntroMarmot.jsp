		...
		<div class="container"><!-- container -->
			<div class="wrap gutter"><!-- warp -->
				...
				<div class="grid head-actions"><!-- head-actions -->
				    <div class="col-1-1 text-left">
				        <strong class="primary">
				            ...
				            <span class="warning">${marmotTotalCount }</span>개의 매장이 있습니다.
				        </strong>
				    </div>
				</div><!-- // head-actions -->
				...
				<div class="section-board section-shop-list"><!-- section-shop-list -->
					<c:choose>
						<c:when test="${not empty marmotStoreList }">
							<c:forEach var="marmotStoreRow" items="${marmotStoreList }" varStatus="i">
								<div class="grid row"><!-- list -->
								    <div class="col-3-4 info">
								        <p class="reset name">
								            <span class="icon map"><em>위치</em></span>
								            <span class="primary"><c:out value="${marmotStoreRow.BCNC_NM }"/> | <c:out value="${marmotStoreRow.TEL }"/></span>
								        </p>
								        <p class="reset address">
								            <span class="info"><c:out value="${marmotStoreRow.ADDR }"/></span>
								        </p>
								    </div>
								    <div class="col-1-4">
								        <div class="section-button actions">
								        	<a href="javascript:;" class="button primary full" data-store_addr = "${marmotStoreRow.ADDR }" data-store_tel="${marmotStoreRow.TEL }" data-store_nm="${marmotStoreRow.BCNC_NM }">
								            	<span>지도보기</span>
								            </a>
								        </div>
								    </div>
...
<script type="text/javascript">
	...
	$(function()
	{
	    ...
	    goPage = function (cPage) 
	    {
	        ...
	        $frm.attr({"action":"<c:out value="${mobileDomain}"/>/mobile/brand/brandStoreIntroMarmot.do", "target":"_self", "method":"get"}).submit();
	    };
