...
<section id="wrap">
				...
				<dd class="dd_img">
					...
					<div class="chk_box">
						<fieldset data-role="controlgroup">
							<legend class="blind">이벤트 선택 폼</legend>
							<c:forEach var="row" items="${surveyList}" varStatus="sta">
							<input type="radio" name="EVT_DTL_IDX" id="inp_ev_<fmt:formatNumber value="${row.EVT_DTL_NO}" pattern="00"/>" value="<c:out value="${row.EVT_DTL_IDX }" />" <c:if test="${sta.first }">checked</c:if>>
							<label for="inp_ev_<fmt:formatNumber value="${row.EVT_DTL_NO}" pattern="00"/>"><c:out value="${row.EVT_DTL_NM }" /></label>
							</c:forEach>
						</fieldset>	
					</div>
						</c:when>
						<c:when test="${info.EVT_MST_SVY_TYPE eq 'I' }">
					<div class="chk_box img_type">
						<fieldset data-role="controlgroup">
							<legend class="blind">이벤트 선택 폼</legend>						
							<c:forEach var="row" items="${surveyList}" varStatus="sta">
							<c:if test="${not empty row.FILE_NM }">						
							<input type="radio" name="EVT_DTL_IDX" id="inp_ev_<fmt:formatNumber value="${row.EVT_DTL_NO}" pattern="00"/>" value="<c:out value="${row.EVT_DTL_IDX }" />" <c:if test="${sta.first }">checked</c:if>>
							<label for="inp_ev_<fmt:formatNumber value="${row.EVT_DTL_NO}" pattern="00"/>"><img src="<c:out value="${cdnDomain }${row.FILE_NM }" />" alt="<c:out value="${row.CMM_FLE_ALT_TXT }" />" /></label>
							</c:if>
							</c:forEach>
						</fieldset>
					</div>											
	...
	<script type="text/javascript">
		...
		$(document).on("change", "#sel_ingEv", function(){
			...
			location.href = "<c:out value="${serverDoamin}" />/mobile/event/eventView.do?EVT_MST_IDX=" + evt_mst_idx;
		});
