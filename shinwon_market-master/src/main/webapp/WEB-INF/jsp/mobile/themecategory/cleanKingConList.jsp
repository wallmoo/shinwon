...
<section id="wrap">
	...
	<article id="container" class="container" data-role="content">
		...
		<section class="thema_list clean_list">
			<h2 class="sec_ti">
				<span>Clean GuideBook</span>
				<strong>홈클린 가이드북</strong>
			</h2>
			
			<!-- .list_type S -->
			<dl class="list_type">
				<dt class="blind">홈클린 리스트 정렬 조건 form</dt>
				<dd class="sel_ch">
		...
		<script type="text/javascript">
				...
				*/
				...
				if(_reNum <= <c:out value="${paging.totalPageCount}" />){
					...
					$.ajax({
			        	...
			        	url		: "<c:out value="${mobileDomain}" />/mobile/themecategory/cleanKingConListAjax.do",
			...
			goPage		= function(page){
				$("#cPage").val(page);
				$("#frm").attr("action", "<c:out value="${mobileDomain}" />/mobile/themecategory/cleanKingConList.do").submit();
			...
			goDetail	= function(idx){
				$("#MGZ_MST_IDX").val(idx);
				$("#frm").attr("action", "<c:out value="${mobileDomain}" />/mobile/themecategory/cleanKingConView.do").submit();
			};
