...
<body class="page-customer"><!-- page class : page-login -->
	...
	<article id="container" class="container" data-role="content">
        ...
        <div class="faq-nav" data-role="collapsible-set"><!-- faq-nav -->
            <div class="section" data-role="collapsible" data-collapsed="true">
                <h3>
                	
                	<c:if test="${not empty smallCateName}">
                		<c:out value="${cateName}" /> &gt; <c:out value="${smallCateName}" />(<fmt:formatNumber value="${totalCount }" groupingUsed="true" />)<i><!-- icon --></i>
                	</c:if>
                	<c:if test="${empty smallCateName}">
                		<c:out value="${cateName}" />(<fmt:formatNumber value="${totalCount }" groupingUsed="true" />) &gt; 전체보기 <i><!-- icon --></i>
                	</c:if>
                </h3>
                <div class="nav-list">
                    <ul>
                    
                    <c:forEach var="rowCode" items="${lowCodeList }" varStatus="i">
                        <li><a href="javascript:goDtl('<c:out value="${rowCode.CMN_COM_IDX}" />');"><c:out value="${rowCode.CMN_COM_NM}" /></a></li>
                    </c:forEach>
                    </ul>
                </div>
            </div>
        ...
        <div class="padder mt15">
            <div class="bbs-status"><!-- bbs-status -->
                <strong><c:out value="${cateName}" /></strong> ( <span class="warning"><fmt:formatNumber value="${totalCount }" groupingUsed="true" /></span> 건 )
            </div><!-- // bbs-status -->
        </div>
...
function _return_scrollList(_idx, _reNum){
	/*
		...
		*/
	var page = _reNum + 2;
	var BRD_CMN_COM_IDX = $("#BRD_CMN_COM_IDX").val();
	var BRD_CMN_COM_UP_IDX = $("#BRD_CMN_COM_UP_IDX").val();
	...
	{	
		$.ajax({
			...
			dataType	: "json",
			async       : false,
			data		: {	"BRD_CMN_COM_IDX" : BRD_CMN_COM_IDX
			...
			success : function(data) {
				...
				if(data.totalPageCount == 0){
					...
					$.each(data.resultMap.list, function(i,item){
						innerHtml += '<div class="section" data-role="collapsible" data-collapsed="true" data-brd_faq_idx="'+item.BRD_FAQ_IDX+'" >';
						innerHtml += '<h3>';
