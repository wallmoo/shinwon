...
<section id="wrap">
					...
					<dl>
						...
						<dd>
							<label for="inp_search">
								<input type="text" id="inp_search" name="searchKeyword" placeholder="home cleaning의 모든 것!  키워드로 검색하세요" value="<c:out value="${commandMap.searchKeyword }" />" />
						...
						<dt class="list_type_box">
							...
							<label for="sel_type_01">
								<select id="searchMgzMstThmCtg1" name="searchMgzMstThmCtg1" class="sel_chg" disabled>
									<option value="2251" <c:if test="${commandMap.searchMgzMstThmCtg1 eq '2253'}">selected </c:if> >클린킹</option>
								</select>
							</label>
							...
							<div class="btn_list_type">
								<a href="<c:out value="${frontDomain }" />/pc/themecategory/cleanKingMain.do" class="btn_home"><span>홈케어 홈</span></a>
	...
	<script type="text/javascript">
	    ...
	    goDetail	= function(idx){
	    	$("#MGZ_MST_IDX").val(idx);
			$("#frm").attr("action", "<c:out value="${frontDomain}" />/pc/themecategory/cleanKingConView.do").submit();
	    };
	    
	    goPage		= function(page){
	    	$("#cPage").val(page);
	    	$("#frm").attr("action", "<c:out value="${frontDomain}" />/pc/themecategory/cleanKingConList.do").submit();
	    };
