			...
			<div class="contents"><!-- .contents S -->
				<form id="dataForm" name="dataForm" method="post">
					...
					<dl class="join_top"><!-- .join_top S -->
						...
						<dd class="nv stp_4">
							<ol>
								...
								<li class="on"><strong><span>02</span>전환 약관동의</strong></li>
					...
					<ul class="join_con agree_con"><!-- .join_con S -->
						<li class="agree_list">
							<h3>통합회원 전환을 위해서는 통합회원에 관련된 추가 약관동의가 필요합니다. 
								<br/><span>멤버십 서비스 약관, 오프라인 매장 개인정보 취급방침, OK캐쉬백 서비스 이용약관, 개인정보 선택 제공에 대한 안내를 읽고 동의해주세요.</span></h3>
							<dl>
								<dt class="">
									<label for="join_ag_0">
...
※ PC통신, 무선 등을 이용하는 전자상거래에 대해서도 그 성질을 반하지 않는 한 이 약관을 준용합니다.
								...
								<dt class="">
									<label for="join_ag_1">
										...
										<span><span class="co_red">(필수)</span>마켓플랜트 쇼핑몰 개인정보 취급방침에 동의합니다.</span>
									</label>
								</dt>
								<dd>
									<pre>
...
<strong>1. 개인정보 수집에 대한 동의</strong>
 회사는 회원이 회사의 개인정보 보호정책 또는 이용약관의 내용에 대하여 동의 또는 동의안함을 선택할 수 있는 절차를 마련하여, 동의를 선택하면 수집에 동의한 것으로 봅니다. 
								...
								<dt>
									<label for="join_ag_4">
										<input type="checkbox" name="join_ag_4" id="join_ag_4" value="Y"/>
										<span><span class="co_red">(필수)</span>OK캐시백 서비스 이용약관에 동의합니다.</span>
									</label>
								</dt>
								<dd>
									<pre> <strong>OK캐쉬백 카드서비스 이용약관</strong><br/>
...
<script>
	$(function(){
		...
		goCheck = function()
	    {
	        ...
	        if(!$("#join_ag_4").is(':checked')){
	            alert("OK캐시백 서비스 이용약관에 동의해주세요.");
	            $("#join_ag_4").focus();
	            return;
	        }
	        
	        $dataForm.attr({"action":"<c:out value="${serverSslDomain}" />/pc/member/changeMemStep3.do", "target":"_self", "method":"post"}).submit();
	    };
