	        ...
	        <div class="minW contents"><!-- contents S -->
	            ...
	            <div id="body"><!-- #body -->
	                ...
	                <ul class="ct tab-a item-4"><!-- tab-a -->
	                    ...
	                    <li class="active"><a href="javascript:;"><span>키핑이벤트</span></a><span class="blind">현재위치</span></li>
	                </ul><!-- // tab-a -->
	
	                <div class="ct keeping-status"><!-- keeping-status -->
                    	내가 키핑한 이벤트 <strong class="warning"><fmt:formatNumber value="${totalCount }" groupingUsed="true" /></strong>개
	                    <div class="fr">
	                        <a href="javascript:;" id="allCheck" class="btn_st0 btn_white small26"><span>전체선택</span></a>
	                        <a href="javascript:selectedDeleteBtn();" class="btn_st0 btn_white small26"><span>선택이벤트 삭제</span></a>
	                        <a href="javascript:endDeleteBtn();" class="btn_st0 btn_white small26"><span>종료된 이벤트 삭제</span></a>
	                    </div>
	                </div><!-- // keeping-status -->
									...
									<c:forEach var="row" items="${list }" varStatus="i">
										<li>
				                            <div class="module">
				                                ...
				                                <a href="<c:out value="${frontDomain }"/>/pc/event/eventView.do?EVT_MST_IDX=<c:out value="${row.EVT_MST_IDX }"/>">
				                                    <span class="thumb">
...
$(function(){
	...
	goPage = function (cPage) 
    {
        ...
        $pageFrm.attr({"action":"<c:out value="${frontDomain}" />/pc/mypage/myKeepEventList.do", "target":"_self", "method":"post"}).submit();
    };
	...
	deleteKeepEvent = function(idx_arr) {
		$.ajax({
					...
					url : "<c:out value="${frontDomain}" />/pc/mypage/deleteKeepEventAjax.do",
	...
	endDeleteBtn = function(){
		if(confirm("종료된 이벤트를 삭제하시겠습니까?")){
			$.ajax({
				async : false,
				type : "POST",
				url : "<c:out value="${frontDomain}" />/pc/mypage/deleteEndKeepEventAjax.do",
				success : function(data) {
					if(data.result == "Y"){
						alert("삭제되었습니다.");
						location.reload(true);							
