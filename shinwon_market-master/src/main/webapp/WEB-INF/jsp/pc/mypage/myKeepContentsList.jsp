	        ...
	        <div class="minW contents"><!-- contents S -->
	            ...
	            <div id="body"><!-- #body -->
	                ...
	                <ul class="ct tab-a item-4"><!-- tab-a -->
	                    ...
	                    <li class="active"><a href="javascript:;"><span>키핑콘텐츠</span></a><span class="blind">현재위치</span></li>
	                ...
	                <div class="ct keeping-status"><!-- keeping-status -->
                    	내가 키핑한 콘텐츠 <strong class="warning"><fmt:formatNumber value="${totalCount }" groupingUsed="true" /></strong>개
	                    <div class="fr">
	                        <a href="javascript:;" id="allCheck" class="btn_st0 btn_white small26"><span>전체선택</span></a>
	                        <a href="javascript:selectedDeleteBtn();" class="btn_st0 btn_white small26"><span>선택콘텐츠 삭제</span></a>
	                    </div>
	                </div><!-- // keeping-status -->
	
	                <div class="keeping-list type-b"><!-- keeping-list -->
	                	<c:choose>
			                        	...
			                        	<li class="${row.MGZ_MST_KID eq 'T' ? 'view-text' : ''}">
				                            <div class="module">
				                                ...
				                                <c:if test="${row.MGZ_MST_CTG eq Code.EMAGAZINE_CATEGORY_LIFE }">
													<span class="category label-a">라이프가이드</span>				                                
				                                </c:if>
				                                <c:if test="${row.MGZ_MST_CTG eq Code.EMAGAZINE_CATEGORY_BUY }">
													<span class="category label-b">구매가이드</span>                                
				                                </c:if>
				                                <c:if test="${row.MGZ_MST_CTG eq Code.EMAGAZINE_CATEGORY_PRODUCT_REVIEW }">
													<span class="category label-c">상품리뷰</span>			                                
				                                </c:if>
				                                <c:if test="${row.MGZ_MST_CTG eq Code.EMAGAZINE_CATEGORY_PRODUCT_NEWS }">
													<span class="category label-d">상품뉴스</span>			                                
				                                </c:if>
				                                <c:if test="${row.MGZ_MST_CTG eq Code.EMAGAZINE_CATEGORY_STUFF }">
													<span class="category label-e">Stuff</span>			                                
				                                </c:if>
				                                <c:if test="${row.MGZ_MST_CTG eq Code.EMAGAZINE_CATEGORY_THEME }">
													<span class="category label-f">테마</span>	                                
				                                </c:if>
				                                <a href="<c:out value="${frontDomain }"/>/pc/contents/emagazineView.do?MGZ_MST_IDX=<c:out value="${row.MGZ_MST_IDX }"/>">
...
$(function(){
	...
	goPage = function (cPage) 
    {
        ...
        $pageFrm.attr({"action":"<c:out value="${frontDomain}" />/pc/mypage/myKeepContentsList.do", "target":"_self", "method":"post"}).submit();
    };
	...
	deleteKeepContents = function(idx_arr) {
		$.ajax({
					...
					url : "<c:out value="${frontDomain}" />/pc/mypage/deleteKeepContentsAjax.do",
