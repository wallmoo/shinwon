...
$(function(){
	...
	goMyKeeping = function(){
		$('#_blank_form').attr({'action':'<c:out value="${serverSslDomain}" />/pc/mypage/myKeepProductList.do','method':'get'}).submit();	
	};
});
...
function simplePayCart(ORD_MST_CD){
	var url = "<c:out value="${serverSslDomain}"/>/pc/cart/cartpayment.do?ORD_MST_CD=" + ORD_MST_CD;
	url = encodeURIComponent(url);
	window.open("<c:out value="${serverSslDomain}"/>/pc/manager/loginPopup.do?url=" + url,"loginPopup","width=850,height=644,scrollbars=no,target=_blank");
}
