		...
		<div class="contents"><!-- .contents S -->
			...
			<ul>
				...
				<li class="ti">
					...
					<dl class="score">
						<dt class="blind">상품에 대한 상품평, 점수</dt>
						<dd class="svg">
							<dl>
								<dt>
									...
									<span class="tx_comment">상품평 <strong><fmt:formatNumber value="${productReputation.REP_CNT}" groupingUsed="true" /></strong>건</span>
								</dt>
								<dd>									
									<div class="svg_box">
										<svg version="1.1" id="grp_svg" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 101 74.1" style="enable-background:new 0 0 101 74.1;" xml:space="preserve">
											<path class="st0" d="M50.5,5C25.4,5,5,25.4,5,50.5c0,8.4,2.3,16.5,6.6,23.6h77.8C93.7,67,96,58.9,96,50.5C96,25.4,75.6,5,50.5,5z" style="fill:none;" />
											<path class="st1" d="M5,50.5C5,25.4,25.4,5,50.5,5S96,25.4,96,50.5c0,8.4-2.3,16.5-6.6,23.6h5.7c3.8-7.3,5.9-15.3,5.9-23.6
												C101,22.7,78.3,0,50.5,0S0,22.7,0,50.5c0,8.3,2,16.4,5.9,23.6h5.7C7.3,67,5,58.9,5,50.5z" style="fill:#E6E6E6;" />
										</svg>
									</div>
								</dd>
							</dl>
										...
										<c:choose>
											<c:when test="${userInfo.MEM_MST_MEM_GBN eq '683' }">
												<li class="li_sel_prc">
													...
													<a href="javascript:getPrdEsmPop();" class="btn_st3"><span class="ir">가격흥정</span></a>
												</li>
											...
											<c:otherwise>
												...
												<c:if test="${productInfo.PRD_CPN_DST_PRICE > 0 || productInfo.PRD_BRD_CPN_CNT > 0}">
													...
													<c:choose>
														<c:when test="${productInfo.PRD_CPN_DST_PRICE > 0 && productInfo.PRD_BRD_CPN_CNT == 0 }">
															<li class="li_cou_prc">
																<span class="ti">
																	쿠폰적용가
									...
									<div class="inp_box">
										<c:if test="${productInfo.PRD_MST_LNG_DIV_MNT1 ne 0 }">
											<label for="MEM_CRT_LNG_DIV1" class="js-radio inp_radio">
												...
												<strong class="co_red"><span id="addPrdMstLngDivMnt1PrcTxt">
													<c:choose>
														<c:when test="${not empty productTimeSaleInfo || not empty productOneDayInfo}">
															<fmt:formatNumber value="${productInfo.PRD_MST_PRC }" groupingUsed="true" />
														</c:when>
										...
										<label for="MEM_CRT_LNG_DIV2" class="l js-radio inp_radio">
											...
											<strong class="co_red"><span id="addPrdMstLngDivMnt2PrcTxt">
												<c:choose>
													<c:when test="${not empty productTimeSaleInfo || not empty productOneDayInfo}">
														<fmt:formatNumber value="${productInfo.PRD_MST_PRC }" groupingUsed="true" />
													</c:when>
...
>>>>>>> .r2493
								<li class="li_btnBox2 sold_st">
									<c:choose>
										<c:when test="${commandMap.esmCart eq 'T' }">
											<a href="javascript:inertEsmCart('<c:out value="${Code.CART_DLV_GBN_NORMAL_CD }" />');" class="btn_st2 btn_blk777 f"><strong>견적담기</strong></a>
										</c:when>
										<c:when test="${commandMap.esmCart eq 'theme' }">
											<a href="javascript:optChoice('<c:out value="${Code.CART_DLV_GBN_NORMAL_CD }" />');" class="btn_st2 btn_blk777 f"><strong>옵션선택</strong></a>
										</c:when>
										<c:when test="${productInfo.PRD_MST_SEL_STATE eq 'I'}">
											<a href="javascript:setPaymentData();" class="btn_st2 btn_rd f"><strong>바로구매</strong></a>
											<a href="javascript:inertCart('<c:out value="${Code.CART_DLV_GBN_NORMAL_CD }" />');" class="btn_st2 btn_blk777"><strong>장바구니</strong></a>
											<a href="javascript:goProductDetail('<c:out value="${productInfo.PRD_MST_CD}" />');" class="btn_st2 btn_white"><strong>상품상세</strong></a>
											<a href="javascript:insertKeeping('<c:out value="${productInfo.PRD_MST_CD}" />');" class="btn_st2 btn_white"><i class="ico_keep"></i><strong>키핑</strong></a>
										...
										<c:otherwise>
											<c:choose>
												...
												<c:otherwise>
													<a href="javascript:void(0)" class="btn_st4 btn_blk"><i class="ico_chat"></i><span>일시품절</span></a>
													<a href="javascript:insertKeeping('<c:out value="${productInfo.PRD_MST_CD}" />');" class="btn_st2 btn_white"><i class="ico_keep"></i><strong>키핑</strong></a>
													<a href="javascript:void(0)" class="btn_st2 btn_white l"><i class="ico_chat"></i><span>채팅상담</span></a>
												</c:otherwise>
											</c:choose>
...
<p class="optionDtl">
	...
	<span class="tx_1">\${OPT_TITLE}</span>
	{{if PRD_OPT_CNT_YN == 'Y'}}
...
$(function(){
	...
	card_benefit_ico.on({
		mouseenter : function(){
			...
			//레이어
			_thisLay.show().css({position:'absolute', top: _css_top, left: _css_left});
			_thisLay.on({
				mouseenter : function(){
					_enter_db[1] = 1;
				},
	...
	var _ico_star = $('.ico_star_an'),
		_tx_num = _ico_star.eq(0).next('.tx_num').text(),
		_svg = $('.svg_box'),
		_svg_num = Math.floor(_tx_num/2),
		_svg_d = '';
		_pathHtml = '<svg version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 101 74.1" style="enable-background:new 0 0 101 74.1;" xml:space="preserve">'
		_pathHtml += '<path class="st0" d="M50.5,5C25.4,5,5,25.4,5,50.5c0,8.4,2.3,16.5,6.6,23.6h77.8C93.7,67,96,58.9,96,50.5C96,25.4,75.6,5,50.5,5z" style="fill:none;" />',
		_pathHtml += '<path class="st1" d="M5,50.5C5,25.4,25.4,5,50.5,5S96,25.4,96,50.5c0,8.4-2.3,16.5-6.6,23.6h5.7c3.8-7.3,5.9-15.3,5.9-23.6C101,22.7,78.3,0,50.5,0S0,22.7,0,50.5c0,8.3,2,16.4,5.9,23.6h5.7C7.3,67,5,58.9,5,50.5z" style="fill:#E6E6E6;" />';
	
	switch(_svg_num){
		case 0 : _svg_d = 'M9.7,74.1c-4.1-7-6.5-15.2-6.5-23.9';
				 break;
		case 1 : _svg_d = 'M9.7,74.1c-4.1-7-6.5-15.2-6.5-23.9c0-13.3,5.5-25.4,14.4-34';
				 break;
		case 2 : _svg_d = 'M9.7,74.1c-4.1-7-6.5-15.2-6.5-23.9c0-13.3,5.5-25.4,14.4-34c8.5-8.3,20.2-13.4,33-13.4';
				 break;
		case 3 : _svg_d = 'M9.7,74.1c-4.1-7-6.5-15.2-6.5-23.9c0-13.3,5.5-25.4,14.4-34c8.5-8.3,20.2-13.4,33-13.4c14.7,0,27.7,6.6,36.4,17.1';
				 break;
		case 4 : _svg_d = 'M9.7,74.1c-4.1-7-6.5-15.2-6.5-23.9C3.2,24,24.4,2.8,50.6,2.8S98,24,98,50.2c0,8.7-2.4,16.9-6.5,23.9';
				 break;
		default : _svg_d = 'M9.7,74.1c-4.1-7-6.5-15.2-6.5-23.9';
				 break;
	}
	
	//10개 0, 1, 1.5, 2, 2.5, 3, 3.5, 4, 4.5, 5
	var _ico_star_ch = _ico_star.children(),
		data_width = _ico_star_ch.data('width');
	_ico_star_ch.width('0%').animate({width: (data_width*10)+'%'}, 1000);
	
	_pathHtml += '<path class="st2" d="'+_svg_d+'" style="fill: none; stroke: rgb(252, 31, 40); stroke-width: 6px; stroke-miterlimit: 10; stroke-dasharray: 200px, 202px; stroke-dashoffset: 0px;"></path>';
	_pathHtml += '</svg>';
	
	_svg.html(_pathHtml).each(function(i){
		$(this).find('svg').attr('id','svg'+i)
	});
	
	if(ie != 7 && ie != 8){
		new Vivus('svg0', {duration: 100});
	};
		...
		if($(this).val() != ""){
			...
			if(!isbreak){
				...
				if(obj.PRD_OPT_EST_YN == "Y"){
					...
					obj.OPT_TITLE = "[선택]" + obj.PRD_OPT_NM + ":" + obj.OPT_DTL_NM;
	...
	inertCart = function(memCrtDlvGbn){
		...
		var memId = "<c:out value="${commandMap.MEM_ID}" />";
		...
		if(cartResult){
	        ...
	        ({
	            ...
	            url : "<c:out value="${serverDomain}" />/pc/cart/insertCartAjax.do",
	...
	setPaymentData = function()
	{
		...
		if(cartResult){
	        ...
	        ({
	            ...
	            type : "POST",
	            data : $("#cartForm").serialize(),
	            url : "<c:out value="${serverDomain}" />/pc/cart/insertDirectPurchaseAjax.do",
	            success : function (data) 
	            { 
					switch (data.result) {
					case 1:
						...
						$('#cartForm').attr({"action":"<c:out value="${serverSslDomain}" />/pc/cart/cartpayment.do","target":opener.parent.name}).submit();
						self.close();
						</c:when>
						</c:choose>
						break;
	...
	insertKeeping = function(prdMstCd){
		$.ajax({
			...
			url : "<c:out value="${serverDomain}" />/pc/cart/insertInterestProductAjax.do",
			success : function(data) {
	...
	getPrdStorePop = function(){
		...
		if(memId == ""){
			...
			popup('',"711","584","no","_ProductStorePopup");
	 		$("#cartForm").attr({"action":"<c:out value="${serverDomain}" />/pc/product/searchProductStorePopup.do", "target":"popup_ProductStorePopup", "method":"post"}).submit();
		}
    };
	
	goProductDetail = function(prdMstCd){
		opener.location.href = "<c:out value="${serverDomain}" />/pc/product/product.do?prdMstCd="+prdMstCd;
		self.close();
	};
	...
	inertEsmCart = function(memCrtDlvGbn){
		...
		if(cartResult){
	        ...
	        ({
	            ...
	            type : "POST",
	            data : $("#cartForm").serialize(),
	            url : "<c:out value="${serverDomain}" />/pc/product/insertEsmCartAjax.do",
	            success : function (data){
	            	alert("성공했습니다.");
	            	opener.goMyEsmList();
	            	self.close();
	...
	optChoice			= function(memCrtDlvGbn){
		...
		if(cartResult){
			...
			$.each(_list, function(i){
				...
				opt_text			+= '<span data-prd_dtl_price="'+ _PRD_DTL_PRCs +'" data-opt_idx="'+ _OPT_DTL_IDXs +'" data-prd_mst_cd="<c:out value="${productInfo.PRD_MST_CD}" />" data-opt_cnt="'+ _REQUEST_CNT + '" data-opt_est_yn="' + _PRD_OPT_EST_YNs + '" >' + _OPT_TEXT + '<a href="javascript:void(0);" id="optDel"><i class="ico_del"><span class="ir">옵션삭제</span></i></a></span>';
			...
			opener.weddings.pr_ck($(opener.document).find('#pr_list_'+tabIdx+' label'));
    	...
    	if(memId == ""){
			...
			if(memGbn == ememType){
				if(confirm("마켓플랜트 통합회원 전용혜택입니다.\n통합회원으로 전환하시겠습니까?")){
					opener.$('#couForm').attr({'action':'<c:out value="${serverSslDomain}" />/pc/manager/changeOnlineMemCert.do','method':'post'}).submit();	
				}
			}else{
				$("#couponType").val(dwnType);
				popup('',"520","400","no","_Product"+dwnType+"CpnPopup");
		 		$("#cartForm").attr({"action":"<c:out value="${serverDomain}" />/pc/product/searchProductCpnPopup.do", "target":"popup_Product"+dwnType+"CpnPopup", "method":"post"}).submit();
			}
		}
    };
