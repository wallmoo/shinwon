...
<meta property="og:title" content="${productInfo.PRD_MST_NM }" />
...
<meta property="og:url" content="<c:out value="${frontDomain}"/>/pc/product/product.do?prdMstCd=${productInfo.PRD_MST_CD}" />
	...
	<article id="container" class="container">
		<div class="contents"><!-- .contents S -->
			...
			<article class="minW pr_basic">
					...
					<c:if test="${not empty listCategoryPath }">
						<c:forEach var="listCategoryPath" items="${listCategoryPath }" varStatus="status">
							<c:choose>
								...
								<c:otherwise>
									<c:choose>
										<c:when test="${listCategoryPath.PRD_CTG_IDX == commandMap.cateIdx }">
				...
				<input type="hidden" name="cateIdx" id="cateIdx" value="<c:out value="${commandMap.CATEGORY_IDX }" />" />
				<input type="hidden" name="BRD_INQ_IDX" id="BRD_INQ_IDX" />
				<input type="hidden" name="chlCode" id="chlCode" value="<c:out value="${commandMap.chlCode }" />" />
				<input type="hidden" name="planIdx" id="planIdx" value="<c:out value="${commandMap.planIdx }" />" /><!-- 기획전 Idx -->
				<fieldset class="pr_detail"><!-- .pr_detail S -->
					<div class="pr_image">
						<dl class="img_box">
							<dt class="blind">상품이미지</dt>							
							<dd class="img"><img src="<c:out value="${cdnDomain}" />/<c:out value="${productInfo.PRD_IMAGE}" />" class="thumb198" alt="<c:out value="${productInfo.PRD_IMAGE_ALT}" />" /></dd>
							<dd class="list">
								<div class="img_sm">
									<c:if test="${not empty listProductImages }">
										<c:forEach var="listProductImages" items="${listProductImages }" varStatus="i">
											<a href="javascript:void(0);" data-img="<c:out value="${listProductImages.large}" />"><img src="<c:out value="${listProductImages.small}" />" alt="<c:out value="${listProductImages.alt}" />" /></a>
										</c:forEach>
									</c:if>
								</div>
							</dd>
							<!-- <dd class="ico_d"><i class="ico_sale10"><span class="ir">10% 브랜드쿠폰할인 적용상품</span></i></dd> -->
						</dl>
						<hr class="bg_line_5px" />
						<ol class="pr_exhibition">
							<li class="ti">
								<strong>관련 기획전</strong>
								<a href="<c:out value="${frontDomain }"/>/pc/event/planList.do" class="btn_more"><i class="ir">more</i></a>
							</li>
							<c:choose>
								<c:when test="${not empty listPlan }">
									...
									<c:forEach var="listPlan" items="${listPlan }" varStatus="status">
										<c:choose>
											<c:when test="${planNum eq '1' }">
												<li class="f">
													<a href="<c:out value="${frontDomain }"/>/pc/event/planProductList.do?PLN_MST_IDX=<c:out value="${listPlan.PLN_MST_IDX }" />"><c:out value="${listPlan.PLN_MST_TITLE }" />
								...
								<c:otherwise>
									<li class="f"><a href="javascript:void(0);">관련 기획전이 없습니다.</a></li>
								</c:otherwise>
							</c:choose>
						</ol>
						<ul class="pr_sns">
							<li class="blind"><strong>sns 공유하기</strong></li>
							<li class="f"><a href="javascript:goProductKakaoStory();" class="ico_kakao"><span class="ir">kakaotalk 공유</span></a></li>
							<li><a href="javascript:goProductTwitter();" class="ico_t"><span class="ir">twitter 공유</span></a></li>
							<li><a href="javascript:goProductFacebook();" class="ico_f"><span class="ir">facebook 공유</span></a></li>
							<!-- <li><a href="javascript:goProductPinterest();" class="ico_p"><span class="ir">pinterest 공유</span></a></li> -->
							<li class="l"><a href="javascript:goProductCopyClipBoard();" class="btn_st3 ico_url"><span class="ir">URL복사</span></a></li>
						</ul>
					</div>
					<div class="pr_info"><!-- .pr_info S -->
						<c:if test="${not empty productTimeSaleInfo || not empty productOneDayInfo}">
							<c:if test="${not empty productTimeSaleInfo }">
								<div class="timeSale">
									<strong class="ti">
										<span class="ir">온라인 단독! 타임세일</span>
									</strong>
									<span class="tx_ti">남은시간</span>
									<p id="tx_time"><span>00</span><span>00</span><span>00</span></p>
								</div>
							</c:if>
							<c:if test="${not empty productOneDayInfo }">
								<div class="oneDay">
									<strong class="ti">
										<span class="ir">오늘만 특가</span>
									</strong>
									<span class="tx_ti">남은시간</span>
									<p id="tx_time2"><span>00</span><span>00</span><span>00</span></p>
								</div>
							</c:if>
						</c:if>
						<ul class="info_list">
									...
									<c:choose>
										<c:when test="${userInfo.MEM_MST_MEM_GBN eq '683' }">
											<li class="li_sel_prc">
												...
												<c:if test="${productInfo.PRD_MST_IVB_YN ne 'Y' }">
										...
										<c:otherwise>
											<c:choose>
												<c:when test="${commandMap.PRD_CHL_TYPE ne '' }">
													<li class="li_sel_prc">
														<span class="ti">판매가</span>
																...
																<c:if test="${productInfo.PRD_NOW_DST_PRICE > 0}">
																	<li class="li_immed_prc">
																		...
																		<p class="bg_yel">
																			<strong class="price"><fmt:formatNumber value="${productInfo.PRD_NOW_DST_PRICE }" groupingUsed="true" /><span class="tx_won">원</span></strong>
																			<c:choose>
																				<c:when test="${productInfo.PRD_MST_PC_CPN_UNT eq 'R' }">
																					<span class="ico_selOff"><c:out value ="${productInfo.PRD_NOW_DST_POINT }" />% OFF</span>
																				</c:when>
																				<c:otherwise>
																					<span class="ico_selOff"><fmt:formatNumber value="${productInfo.PRD_MST_PC_CPN_PRC }" groupingUsed="true" />원 OFF</span>
																				</c:otherwise>
																			</c:choose>
																		</p>
																	</li>
								...
								<c:otherwise>
									<c:choose>
										<c:when test="${not empty productTimeSaleInfo || not empty productOneDayInfo }">
											<c:if test="${not empty productTimeSaleInfo }">
												<li class="li_sel_prc">
													<span class="ti">게릴라가</span>
													<strong class="price"><fmt:formatNumber value="${productInfo.PRD_MST_PRC }" groupingUsed="true" /><span class="tx_won">원</span></strong>
												</li>	
											</c:if>
											<c:if test="${not empty productOneDayInfo }">
												<li class="li_sel_prc">
													<span class="ti">only 1 day가</span>
													<strong class="price"><fmt:formatNumber value="${productInfo.PRD_MST_PRC }" groupingUsed="true" /><span class="tx_won">원</span></strong>
												</li>	
											</c:if>
										</c:when>
							...
							<li class="li_card_benefit">
								<c:if test="${not empty productInfo.PRD_MST_CRD_ICON }">
								...
								<div class="que_box">
									...
									<dl class="que_lay">
										...
										<dd class="que_con">
											<p class="tx_list">
												<strong><c:out value="${cardEvenFreetList[0].THIS_MONTH }" />월 할인 혜택안내</strong>
											</p>
                            		...
                            		<c:if test="${c_promotion_point_save_cnt gt 0 }">
										<dl class="que_lay">
											...
											<dd class="que_con">
												<p class="tx_list">
													<strong><c:out value="${cardEvenFreetList[0].THIS_MONTH }" />월 포인트 적립혜택 안내</strong>
												</p>
												<table>
							...
							<c:if test="${not empty productInfo.PRD_MST_BRAND_NM }">
								<li class="li_brand">
									<span class="ti">브랜드</span>
									<span class="tx"><c:out value="${productInfo.PRD_MST_BRAND_NM }" /> <a href="<c:out value="${frontDomain}" />/pc/search/search.do?query=<c:out value="${productInfo.PRD_MST_BRAND_NM }" />" class="btn_st3"><span>브랜드 상품보기</span></a></span>
								</li>
							</c:if>
							...
							<c:if test="${productInfo.PRD_MST_LNG_DIV_YN eq 'Y' && productInfo.PRD_MST_IVB_YN ne 'Y'}">
							...
							<li class="li_btnBox2">
								<c:choose>
									<c:when test="${productInfo.PRD_MST_SEL_STATE eq 'I'}">
										...
										<!-- <a href="javascript:chat();" class="btn_st2 btn_white l"><i class="ico_chat"></i><span>채팅상담</span></a> -->
									</c:when>
									<c:otherwise>
										<c:choose>
											<c:when test="${productInfo.PRD_MST_SEL_STATE eq 'T'}">
												<a href="javascript:vod(0);" class="btn_st4 btn_rd f"><i class="ico_chat"></i><span>SOLD OUT</span></a>
												<a href="javascript:insertKeeping('<c:out value="${productInfo.PRD_MST_CD}" />');" class="btn_st2 btn_rd_l"><i class="ico_keep"></i><strong>키핑</strong></a>
												<!-- <a href="javascript:void(0)" class="btn_st2 btn_white l"><i class="ico_chat"></i><span>채팅상담</span></a> -->
												<dl class="que_lay">
													<dt class="que_ti">
														<strong class="blind">SOLD OUT 안내</strong>
														<i class="ico_ar"></i>
													</dt>
													<dd class="que_con al">
														상품이 품절되었습니다.
											...
											<c:otherwise>
												<a href="javascript:vod(0);" class="btn_st4 btn_blk f"><i class="ico_chat"></i><span>일시품절</span></a>
												<a href="javascript:insertKeeping('<c:out value="${productInfo.PRD_MST_CD}" />');" class="btn_st2 btn_rd_l"><i class="ico_keep"></i><strong>키핑</strong></a>
												<!-- <a href="javascript:void(0)" class="btn_st2 btn_white l"><i class="ico_chat"></i><span>채팅상담</span></a> -->
												<dl class="que_lay">
													<dt class="que_ti">
														<strong class="blind">일시품절 안내</strong>
														<i class="ico_ar"></i>
													</dt>
													<dd class="que_con al">
														상품이 일시적으로 품절되어<br/>상품을 준비중입니다.
													</dd>									
												</dl>
											</c:otherwise>
										</c:choose>
								...
								<c:if test="${productInfo.PRD_MST_IVB_YN ne 'Y' }">
									<c:if test="${productInfo.PRD_MST_O2O_YN eq 'Y'}">
										<a href="javascript:getPrdStorePop();"><img src="<c:out value="${cdnDomain}" />/pc/img/btn/btn_getitNow.jpg" alt="Get it NOW 온라인에서 구매하고 매장에서 찾아요! 겟잇나우로 구매" /></a>
									</c:if>
									<c:if test="${productInfo.PRD_MST_LNG_DIV_YN eq 'Y'}">
										<a href="javascript:inertCart('<c:out value="${Code.CART_DLV_GBN_LONG_INS_CD }" />');"><img src="<c:out value="${cdnDomain}" />/pc/img/btn/btn_plan.jpg" alt="장기할부NOW 경제적 부담은 줄이고 생활의 편의를 누려라! 장기할부로 구매" /></a>
									</c:if>
								</c:if>
							</li>
						</ul>
				...
				</fieldset><!--// .pr_detail E -->
				</form>
				<dl class="pr_recom" id="menuViewTab">
					<dt>
						<a href="javascript:void(0)" data-tab_no="tab1" class="f on"><strong>추천상품</strong></a>
						<a href="javascript:void(0)" data-tab_no="tab2" class="l"><strong>카테고리 BEST</strong></a>
					</dt>
					<dd id="prdCtgBestList">
			...
			<!--// .pr_basic E -->	
			<!-- .pr_view S -->
			<fieldset class="pr_view">
				<!-- 상품 컨텐츠 S -->				
				<div id="pr_con" class="pr_con">
					<div class="minW">
						<h2>스마트 라이프를 위한 e매거진</h2>
						<dl class="pr_emagazine <c:if test="${empty productBuyGuideList }" >no_list</c:if>">
							<dt class="ti">
								<h3><strong>구매가이드</strong></h3>
							</dt>
							<c:choose>
								<c:when test="${not empty productBuyGuideList }">
									<c:set var="guNum" value="1" />
									<c:set var="MGZ_MST_TITLE" value="" />
									<c:forEach var="productBuyGuideList" items="${productBuyGuideList }" varStatus="status">
										<c:choose>
											<c:when test="${guNum eq 1 || guNum mod 4 == 0}">
												<dd class="f"><a href="<c:out value="${frontDomain }"/>/pc/contents/emagazineView.do?MGZ_MST_IDX=<c:out value="${productBuyGuideList.MGZ_MST_IDX}" />"><img src="<c:out value="${cdnDomain}${productBuyGuideList.CMM_FLE_ATT_PATH}${productBuyGuideList.CMM_FLE_SYS_NM }"/>" alt="<c:out value="${productBuyGuideList.CMN_FLE_ALT_TXT }" />" /><strong><c:out value="${productBuyGuideList.MGZ_MST_TITLE}" /></strong></a></dd>
											</c:when>
											<c:otherwise>
												<c:choose>
													<c:when test="${guNum mod 3 == 0}">
														<dd class="l"><a href="<c:out value="${frontDomain }"/>/pc/contents/emagazineView.do?MGZ_MST_IDX=<c:out value="${productBuyGuideList.MGZ_MST_IDX}" />"><img src="<c:out value="${cdnDomain}${productBuyGuideList.CMM_FLE_ATT_PATH}${productBuyGuideList.CMM_FLE_SYS_NM }"/>" alt="<c:out value="${productBuyGuideList.CMN_FLE_ALT_TXT }" />" /><strong><c:out value="${productBuyGuideList.MGZ_MST_TITLE}" /></strong></a></dd>
													</c:when>
													<c:otherwise>
														<dd><a href="<c:out value="${frontDomain }"/>/pc/contents/emagazineView.do?MGZ_MST_IDX=<c:out value="${productBuyGuideList.MGZ_MST_IDX}" />"><img src="<c:out value="${cdnDomain}${productBuyGuideList.CMM_FLE_ATT_PATH}${productBuyGuideList.CMM_FLE_SYS_NM }"/>" alt="<c:out value="${productBuyGuideList.CMN_FLE_ALT_TXT }" />" /><strong><c:out value="${productBuyGuideList.MGZ_MST_TITLE}" /></strong></a></dd>
													</c:otherwise>
												</c:choose>
											</c:otherwise>
										</c:choose>
								...
								<c:otherwise>
									<dd>
										<p>스마트 라이프를 위한 <br/>마켓플랜트의 감성채널 e매거진</p>
										<a href="<c:out value="${frontDomain }"/>/pc/contents/emagazineMain.do" class="btn_gre"><span>보러가기</span></a>
									</dd>
								</c:otherwise>
							</c:choose>
						</dl>
						<dl class="guideNnews">
							<dt class="ti"><h3><strong>상품뉴스</strong></h3></dt>
							<dd class="news">
								<ul class="list">
									<c:choose>
										<c:when test="${not empty productNewList }">
											<c:set var="num" value="1" />
											<c:forEach var="productNewList" items="${productNewList }" varStatus="i">
												<li 
													<c:if test="${num eq 1 }" >class="f"</c:if>
													<c:if test="${num eq 3 }" >class="l"</c:if>
												>
													<a href="<c:out value="${frontDomain }"/>/pc/contents/emagazineView.do?MGZ_MST_IDX=<c:out value="${productNewList.MGZ_MST_IDX}" />">
														<strong><c:out value="${fn:length(productNewList.MGZ_MST_TITLE) > 16 ? fn:substring(productNewList.MGZ_MST_TITLE,0,16) : productNewList.MGZ_MST_TITLE}" /></strong>
														<span>
															<c:choose>
																<c:when test="${fn:length(productNewList.MGZ_MST_CPY) > 46 }">
																	<c:out value="${fn:substring(productNewList.MGZ_MST_CPY,0,46) }" />...
																</c:when>
																<c:otherwise>
							...
							<c:choose>
								<c:when test="${productInfo.CMM_FLE_SAVE_PATH ne null && productInfo.CMM_FLE_IDX ne null}">
									<dd class="guide">
										<a href="javascript:;" class="file-list-title" data-cmm_fle_text="<c:out value="${productInfo.CMM_FLE_ORG_NM }"/>" data-cmm_fle_idx="<c:out value="${productInfo.CMM_FLE_IDX }"/>" data-cmm_fle_save_path="<c:out value="${productInfo.CMM_FLE_SAVE_PATH }"/>">
										<span class="tx"><strong>설치/사용 가이드</strong><span>DOWNLOAD<i class="ico_dw"></i></span></span></a>
									</dd>
								</c:when>
								<c:otherwise>
									<dd class="guide none">
										<img src="<c:out value="${cdnDomain}" />/pc/img/custom/banner_eMagazine.png" alt="내 인생의 쇼핑백서! 마켓플랜트 e매거진" />
									</dd>
								</c:otherwise>
							</c:choose>
						</dl>
						...
						<dl class="revws">
							<dt><h3><strong>상품 리뷰</strong></h3></dt>
							<c:set var="kgNum" value="1" />
							<c:forEach var="productEmagazineRevList" items="${productEmagazineRevList }" varStatus="status">
								<c:choose>
									<c:when test="${kgNum eq 1}">
										<dd class="f"><a href="<c:out value="${frontDomain }"/>/pc/contents/emagazineView.do?MGZ_MST_IDX=<c:out value="${productEmagazineRevList.MGZ_MST_IDX}" />"><img src="<c:out value="${cdnDomain}${productEmagazineRevList.MGZ_REV_IMG}"/>" alt="<c:out value="${productEmagazineRevList.CMN_FLE_ALT_TXT }" />" /><strong><c:out value="${productEmagazineRevList.MGZ_MST_TITLE}" /></strong><i class="ico_king"></i></a></dd>
									</c:when>
									<c:otherwise>
										<c:choose>
											<c:when test="${kgNum  == 4}">
											...
											<c:otherwise>
												<dd><a href="<c:out value="${frontDomain }"/>/pc/contents/emagazineView.do?MGZ_MST_IDX=<c:out value="${productEmagazineRevList.MGZ_MST_IDX}" />"><img src="<c:out value="${cdnDomain}${productEmagazineRevList.MGZ_REV_IMG}"/>" alt="<c:out value="${productEmagazineRevList.CMN_FLE_ALT_TXT }" />" /><strong><c:out value="${productEmagazineRevList.MGZ_MST_TITLE}" /></strong><i class="ico_king"></i></a></a></dd>
											</c:otherwise>
										</c:choose>
									</c:otherwise>
								</c:choose>
					...
					<a href="javascript:void(0)" class="btn_openClose"><strong class="tx">닫기</strong></a>
				</div>
				<!--// 상품 컨텐츠 E -->

				<div class="minW"><!-- .minW S -->
					<!-- 상품 상세 설명 S -->
					<ul class="pr_tab">
						<li class="f on"><a href="#pr_dts"><span>상품 상세 설명<i></i></span></a></li>
						<li><a href="#pr_revw"><span>상품 후기<i></i></span></a></li>
						<li><a href="#pr_qna"><span>상품 Q&A<i></i></span></a></li>
						<li class="l"><a href="#pr_clm"><span>배송/교환/반품<i></i></span></a></li>
					</ul>
					
					<div id="pr_dts" class="pr_dts">
						<!-- 전체공지 S -->
							...
							<c:forEach var="productViewBannerList" items="${productViewBannerList }" varStatus="i">
								<c:choose>
									<c:when test="${productViewBannerList.BNR_MST_TGT eq 'D' }">
										<img src="<c:out value="${cdnDomain}" /><c:out value="${productViewBannerList.CMM_FLE_ATT_PATH}" /><c:out value="${productViewBannerList.CMM_FLE_SYS_NM}" />"  alt="<c:out value="${productViewBannerList.CMN_FLE_ALT_TXT}" />" />	
									</c:when>
									<c:otherwise>
						...
						<c:if test="${not empty productViewBannerBrandList }">
							<h3 class="ti"><span>브랜드공지</span></h3>
							<c:forEach var="productViewBannerBrandList" items="${productViewBannerBrandList }" varStatus="i">
								<c:choose>
									<c:when test="${productViewBannerBrandList.BNR_MST_TGT eq 'D' }">
										<img src="<c:out value="${cdnDomain}" /><c:out value="${productViewBannerBrandList.CMM_FLE_ATT_PATH}" /><c:out value="${productViewBannerBrandList.CMM_FLE_SYS_NM}" />"  alt="<c:out value="${productViewBannerBrandList.CMN_FLE_ALT_TXT}" />" />	
									</c:when>
									<c:otherwise>
							...
							<c:forEach var="productViewBannerMdList" items="${productViewBannerMdList }" varStatus="i">
								<c:choose>
									<c:when test="${productViewBannerMdList.BNR_MST_TGT eq 'D' }">
										<img src="<c:out value="${cdnDomain}" /><c:out value="${productViewBannerMdList.CMM_FLE_ATT_PATH}" /><c:out value="${productViewBannerMdList.CMM_FLE_SYS_NM}" />"  alt="<c:out value="${productViewBannerMdList.CMN_FLE_ALT_TXT}" />" />	
									</c:when>
									<c:otherwise>
										<a href="<c:out value="${productViewBannerMdList.BNR_MST_URL}" />" target="<c:out value="${productViewBannerMdList.BNR_MST_TGT_TARGET}" />">
										<img src="<c:out value="${cdnDomain}" /><c:out value="${productViewBannerMdList.CMM_FLE_ATT_PATH}" /><c:out value="${productViewBannerMdList.CMM_FLE_SYS_NM}" />"  alt="<c:out value="${productViewBannerMdList.CMN_FLE_ALT_TXT}" />" />
										</a>
									</c:otherwise>
								</c:choose>
						...
						<c:if test="${productInfo.PRD_MST_VDR_DIY_CTS != null }">
							<h3 class="ti"><span>파트너공지</span></h3>
							<div class="tx_partner">
						...
						<div class="dts_con">
							<!-- 상품 상세 정보 S -->
							<h4 class="ti">상품 상세 정보</h4>
							<ui:replaceLineBreak content="${productInfo.PRD_MST_PC_DTL_DSC }"/>
							<!-- 상품 상세 정보 E -->

							<!-- 상품 필수 정보 S -->
							<h4 class="ti">상품 필수 정보
								<span>전자상거래 등에서의 상품정보제공 고시에 따라 작성되었습니다.</span>
							</h4>
							<div class="pr_info_d">
								<table>
									<caption>상품정보에대한 목록테이블 입니다.</caption>
									<colgroup>
					...
					<!-- 상품 리뷰 S -->
					<ul class="pr_tab">
						<li class="f"><a href="#pr_dts"><span>상품 상세 설명<i></i></span></a></li>
						<li class="on"><a href="#pr_revw"><span>상품 후기<i></i></span></a></li>
						<li><a href="#pr_qna"><span>상품Q&A<i></i></span></a></li>
						<li class="l"><a href="#pr_clm"><span>배송/교환/반품<i></i></span></a></li>
					</ul>
					
					<dl id="pr_revw" class="pr_revw">
						<dt>							
							<strong class="ti">“배송완료 상품 중 주문일 기준으로 90일 이내에  작성이 가능합니다.”</strong>
							<p class="sub_tx">
								<strong class="tx_ti">상품 후기 크라운 적립 안내</strong>
								<span class="tx">상품 후기 작성 : <strong>200C</strong> 적립</span>      
								<span class="tx">포토후기 작성 : <strong>500C</strong> 적립</span>
							</p>
							<dl class="revw_score">
								<dt>
									<div class="allGrade">										
										...
										<span class="tx_comment">상품후기 <strong class="co_red"><fmt:formatNumber value="${productReputation.REP_CNT}" groupingUsed="true" /></strong>개 기준</span>
									</div>
								</dt>
								<dd>									
									<div class="num_box">
										<strong class="tx">기능<strong><c:out value="${productRevStar.REP_FNC_POINT }" /></strong><span class="blind">점</span></strong>
										<i class="ico_star3"><i class="star_<c:out value="${productRevStar.REP_FNC_POINT_GRD }" />"><span class="ir"><c:out value="${productRevStar.REP_FNC_POINT }" />점</span></i></i>
									</div>
									<div class="num_box">
										<strong class="tx">가격<strong><c:out value="${productRevStar.REP_PRC_POINT }" /></strong><span class="blind">점</span></strong>
										<i class="ico_star3"><i class="star_<c:out value="${productRevStar.REP_PRC_POINT_GRD }" />"><span class="ir"><c:out value="${productRevStar.REP_PRC_POINT }" />점</span></i></i>
									</div>
									<div class="num_box">
										<strong class="tx">만족도<strong><c:out value="${productRevStar.REP_POINT }" /></strong><span class="blind">점</span></strong>
										<i class="ico_star3"><i class="star_<c:out value="${productRevStar.REP_POINT_GRD }" />"><span class="ir"><c:out value="${productRevStar.REP_POINT }" />점</span></i></i>
									</div>
								</dd>
							</dl>
						</dt>
						...
						<dd>
							<div class="type_box">
								<a href="javascript:productRevList();" class="f"><span>전체 상품후기(<span class="co_red" id="spanRepCnt"><fmt:formatNumber value="${productReputation.REP_CNT}" groupingUsed="true" /></span>건)</span></a>
								<a href="javascript:productRevList('PHOTO',1,'');"><span>포토 상품후기(<span class="co_red" id="spanPhotoCnt"><fmt:formatNumber value="${productReputation.PHOTO_CNT}" groupingUsed="true" /></span>건)</span></a>
								<a href="javascript:productRevList('TEXT',1,'');" class="l"><span>일반 상품후기(<span class="co_red" id="spanTextCnt"><fmt:formatNumber value="${productReputation.TEXT_CNT}" groupingUsed="true" /></span>건)</span></a>

								<div class="btn_box">
									<c:if test="${sessionUser.ROLE_USER eq 'user'}">
										<c:if test="${repCount > 0 }">
											<a class="f btn_st0 btn_blk777" id="myRevButton" href="javascript:productRevList('',1,'Y');"><span>MY 상품 후기 보기</span></a>
											<a class="f btn_st0 btn_blk777" id="allRevButton" style="display:none;" href="javascript:productRevList();"><span>전체 상품 후기 보기</span></a>
										</c:if>
									</c:if>
									<a class="l btn_st0 btn_sky" href="javascript:goProductRevWrite();"><span>상품 후기 작성</span></a>
								</div>
							</div>
							
							<table class="table_st0">
								<caption>상품 후기 : 번호, 만족도, 상품평, 구매자, 작성일로 구성된 표</caption>
								<colgroup>
								...
								<thead>
									<tr>
										...
										<th><strong>만족도</strong></th>
										<th><strong>상품평</strong></th>
										<th><strong>구매자</strong></th>
					...
					<ul class="pr_tab">
						<li class="f"><a href="#pr_dts"><span>상품 상세 설명<i></i></span></a></li>
						<li><a href="#pr_revw"><span>상품 후기<i></i></span></a></li>
						<li class="on"><a href="#pr_qna"><span>상품Q&A<i></i></span></a></li>
						<li class="l"><a href="#pr_clm"><span>배송/교환/반품<i></i></span></a></li>
					</ul>
					
					<dl id="pr_qna" class="pr_qna">
						<dt>
							상품에 관련된 문의가 아닌 배송 및 주문관련 문의는 고객센터의 FAQ나 1:1 문의를 이용하여 주십시오. <a class="btn_11" href="javascript:goMyInquiryList();"><span>1:1문의하기</span></a><br/>
							먼저 질문하신 고객님들의 내용을 확인하시면, 보다 유용한 정보를 빠르게 확인하실 수 있습니다.
						</dt>
						<dd>
							<div class="btn_box">
								<c:if test="${sessionUser.ROLE_USER eq 'user'}">
									<c:if test="${qnaCount > 0 }">
										<a class="f btn_st0 btn_blk777" id="myQnaButton" href="javascript:productQnaList(1,'Y');"><span>MY 상품Q&A 보기</span></a>
										<a class="f btn_st0 btn_blk777" id="allQnaButton" style="display:none;" href="javascript:productQnaList(1,'');"><span>전체 상품 후기 보기</span></a>
									</c:if>
								</c:if>
								<a class="l btn_st0 btn_rd" href="javascript:getPrdQnaForm();"><span>상품Q&A 작성</span></a>
							</div>
							<table class="table_st0">
								<caption>Q&A : 번호, 답변상태, 문의유형, 제목, 구매자, 작성일로 구성된 표</caption>
								<colgroup>
								<col style="width:94px" />
					...
					<!-- 교환/반품안내 S -->
					<ul class="pr_tab">
						<li class="f"><a href="#pr_dts"><span>상품상세설명<i></i></span></a></li>
						<li><a href="#pr_revw"><span>상품 후기<i></i></span></a></li>
						<li><a href="#pr_qna"><span>상품Q&A<i></i></span></a></li>
						<li class="l on"><a href="#pr_clm"><span>배송/교환/반품<i></i></span></a></li>
					</ul>
					
					<dl id="pr_clm" class="pr_clm">
						<dt><strong>교환/반품안내</strong></dt>
						<dd>
								...
								<tbody>
									...
									<tr>
										...
										<td>
											...
											<p>3만원 이상 무료(입점업체상품 제외)</p>
											<p>도서, 산간, 오지 일부지역 및 입점업체는 배송비가 추가될 수 있습니다.</p>
									...
									<tr>
										...
										<td>
											<p>① 결제(입금)일로부터 2~5일 (주말제외) 도서/산간지역은 문의바랍니다.</p>
											<p>② 주말전이나 월말에 주문을 하신 경우 출고가 늦어져 1~2일 배송이 지연될 수 있음</p>
											<p>③ 지정일배송을 원하실 경우 02-6925-1160로 상담바랍니다.</p>
											<p>④ 제품생산일정 또는 주문폭주로 배송기간이 길어질 수 있으며, 이러한 경우엔 별도의 전화연락을 드립니다.</p>
										</td>
									</tr>
									<tr>
										<td><strong>배송기사설치제품</strong></td>
										<td>
											<p>양문형/김치냉장고, 드럼세탁기, 대형tv등 연휴 및 공휴일에도 설치가능 <br>
											(단, 아래배송지역은 제품주문 시 배송일정을 상담 바랍니다.)</p>
											<p>① 광역시를 제외한 군단위 지역</p>
											<p>② 강원도지역 (원주/춘천시는 정상배송)</p>
										</td>
									</tr>
									...
									<tr>
										...
										<td>
											<p>① 해당제조사의 A/S기사로부터 불량판정을 받았을 경우</p>
											<p>② 배송된 제품이 주문한 내용과 상이하거나 쇼핑몰內 제품정보와 상이한경우<br/>
											<strong>※ 단순변심으로 인한 제품 반품시에는 관련 배송비 고객부담</strong><br>
											택배 : 5,000원 / 배송기사 설치제품 : 제품에 따라 다름</p>
											<p>③ 반품/교환은 상품 수령 후 7일 이내에 신청하실 수 있습니다.</p>
										</td>
									</tr>
									<tr>
										<td><strong>교환/반품이<br>불가능한 경우</strong></td>
										<td>
											<p class="co_red"><strong>다음 각 내용에 해당하는 경우에는 교환/반품이 불가능합니다.</strong></p>
											<p>① 제품을 설치 또는 사용하거나 상품의 가치가 훼손 (정품스티커 절개포함)된 경우</p>
											<p>② 제품특성상 제품포장(박스)개봉 / 훼손 또는 상품의 밀봉을 개봉한 경우</p>
											<p>③ 박스가 개봉되어 설치된 가전제품 및 컴퓨터제품</p>
											<p>④ 소비자의 부주의로 인한 제품 훼손시</p>
											<p>⑤ 제품수령 후 일주일 이상 경과한 경우</p>
											<p>⑥ 제품의 가치가 현저히 감소한 경우</p>
											<p>⑦ 상품의 구성품 중 일부가 누락되거나 파손된 경우</p>
										</td>
									</tr>
										
									<tr>
										<th><strong>주문취소안내</strong></th>
										<td><strong>주문취소시<br>승인취소/환불</strong></td>
										<td>
											<p>① 카드결제일 경우 4~5일 후 승인취소, 단 국민카드는 7일 후 승인취소</p>
											<p>② 무통장 입금일 경우 5~10일 후 환불</p>
										</td>
									</tr>

									<tr>
										<th rowspan="2"><strong>기타</strong></th>
										<td><strong>세금계산서발행</strong></td>
										<td>
											<p>① 세금계산서는 현금구매시 현금영수증을 신청하시지 않으셨을 경우에만 가능</p>
											<p>② 카드결제시 카드명세서로 세금계산서를 대체하실 수 있습니다.</p>
											<p>③ 입점업체 상품의 경우 해당 입점업체에서 별도로 세금계산서가 발급됩니다.</p>
											<p>④ 세금계산서를 신청하실때는 팩스 02-713-6734로 사업자등록증 사본과 함께<br>
											주문번호, 고객명, 연락처, 이메일 주소를 함께 기재하셔서 보내신 후 쇼핑몰 고객센터 (02-6925-1160)로 전화해 주시기 바랍니다.</p>
										</td>
									</tr>
									<tr>
										<td><strong>입점업체상품</strong></td>
										<td>
											<p>상품 상세설명페이지에 '공급업체가' 표기된 상품은 입점업체 상품이며, 마켓플랜트와는<br>
											별개로 운영되므로, 제품 및 배송에 관한 모든 책임은 입점업체에 있습니다.<br>
											입점업체상품의 세금계산서 발행은 입점업체에 직접 요청하셔야 합니다.</p>
										</td>
									</tr>
								</tbody>
							</table>
...
<tr class="ti" tabindex="0">
	<td>\${ListNum}</td>
	<td><i class="ico_star3"><i class="star_\${PRD_REV_SCORE_GRD}"><span class="ir">\${PRD_REV_SCORE_STAR}점</span></i></i></td>
	<td class="al">
		<a href="javascript:void(0);">
		{{if PRD_REV_CHN_GBN == "M"}}
			<i class="ico_mobile"><span class="ir">모바일 등록</span></i>
		{{/if}}		
		\${PRD_REV_TITLE}
		{{if REP_IMG != null && REP_IMG != ""}}
			<i class="ico_fileImg"><span class="ir">사진리뷰</span></i>
		{{/if}}
		
		</a>
	</td>
	<td class="td_id">\${PRD_REV_REG_ID}</td>
	<td class="td_date">\${PRD_REV_REG_DT}</td>
</tr>
<tr id="productRevList_\${PRD_REV_IDX}" class="con" tabindex="0">
	<td colspan=5 >
		<dl class="re_tx">
			<dt>
				<strong>
					기능 <i class="ico_star3"><i class="star_\${PRD_REV_FNC_STAR_GRD}"><span class="ir">\${PRD_REV_FNC_STAR}점</span></i></i>
				</strong>
				<strong>
					가격 <i class="ico_star3"><i class="star_\${PRD_REV_PRC_STAR_GRD}"><span class="ir">\${PRD_REV_PRC_STAR}점</span></i></i>
				</strong>
			</dt>
			<dd>
				<ui:replaceLineBreak content="\${PRD_REV_CONTENTS }"/><br/>
				{{if REP_IMG != null && REP_IMG != ""}}
...
<tr class="ti" tabindex="0">
	...
	<td>
		{{if BRD_INQ_REPLY_CD == 'Y'}}
			<span class="co_red">\${BRD_INQ_REPLY_CD_NM}</span>
		{{else}}
			\${BRD_INQ_REPLY_CD_NM}
		{{/if}}
	</td>
	<td class="al">
		<a href="javascript:void(0);">\${BRD_INQ_TITLE}</a>
		{{if BRD_INQ_CHN_GBN == "M" }}
			<i class="ico_mobile"><span class="ir">모바일 등록</span></i>
		{{/if}}
	</td>
	<td class="td_id">\${BRD_INQ_REG_ID}</td>
	<td class="td_date">\${BRD_INQ_REG_DT}</td>
</tr>
<tr class="con" tabindex="0">
	<td colspan="5">
		<dl class="q_tx">
			<dd>
				...
				{{if BRD_INQ_REPLY_CD != "Y"}}
					{{if BRD_INQ_REG_CHK_ID == MEM_LOG_ID}}
						<div class="btn_box">														
							<a href="javascript:productQnaDel(\${BRD_INQ_IDX});"><span>삭제</span></a>
							<a href="javascript:getPrdQnaForm(\${BRD_INQ_IDX});"><span>수정</span></a>
						</div>
					{{/if}}
				{{/if}}
			</dd>
...
$(function(){
	...
	list_toggle = function(obj){
		$("#"+obj+"").find('.table_st0 tbody .ti').on('click', function(){
			var _con = $(this).next('.con');
			if(_con.length > 0) _con.slideToggle();
		});
	};
	//옵션추가
		...
		if($(this).val() != ""){
			...
			if(!isbreak){
				...
				if(obj.PRD_OPT_EST_YN == "Y"){
					...
					obj.OPT_TITLE = "<span class=\"co_red\">[선택]</span>" + obj.PRD_OPT_NM + ":" + obj.OPT_DTL_NM;
	            ...
	            { 
	            	switch (data.result) {
      					case 1:
      						...
      						var url = "<c:out value="${serverDomain}"/>/pc/cart/cartpayment.do?ORD_MST_CD=" + data.ORD_MST_CD;
      						url = encodeURIComponent(url);
      						...
      						$('#cartForm').attr('action','<c:out value="${serverDomain}" />/pc/cart/cartpayment.do').submit();
      						</c:when>
      						</c:choose>
      						break;
      					default :
	...
	productRevList = function(searchRevType,cPage,searchMemId){
		...
		if(prdMstCd != ""){
	        ...
	        ({
	            ...
	            url : "<c:out value="${frontDomain}" />/pc/product/productRevListAjax.do",
	            success : function (data) 
	            { 
	            	...
	            	if(data.RevList != null)
	    			{
	    				...
	    				$.each(data.RevList, function(index,item)
	    				{
							...
							obj.PRD_REV_FNC_STAR = item.PRD_REV_FNC_STAR;
							obj.PRD_REV_PRC_STAR = item.PRD_REV_PRC_STAR;
							obj.PRD_REV_FNC_STAR_GRD = item.PRD_REV_FNC_STAR_GRD;
							obj.PRD_REV_PRC_STAR_GRD = item.PRD_REV_PRC_STAR_GRD;
	    					arrayObj.push(obj);
	    				});
	    				...
	    				$( "#applyProductReputationList" ).html("<tr><td colspan=5>등록된 상품평이 없습니다.</td></tr>");
	    				$( "#productReputationListPageDiv" ).html(data.page);
	    			}
	...
	productQnaList = function(cPage,searchQnaMemId){
		...
		if(prdMstCd != ""){
	        ...
	        ({
	            ...
	            url : "<c:out value="${frontDomain}" />/pc/product/productQnaListAjax.do",
	            success : function (data) 
	            { 
	            	if(data.QnaList != null)
	    			{
	    				...
	    				$( "#applyProductQnaList" ).html("<tr><td colspan=5 class=\"no_list\">문의한 상품Q&A가 없습니다.</td></tr>");
	    				$( "#productQnaListPageDiv" ).html(data.page);
	    			}
	            },
	            error : function (err)
    ...
    getPrdQnaForm = function(brdInqIdx){
		...
		if(memId == ""){
			...
			popup('',"715","650","no","_ProductQnaFormPopup");
	 		$("#cartForm").attr({"action":"<c:out value="${frontDomain}" />/pc/product/productQnaFormPopup.do", "target":"popup_ProductQnaFormPopup", "method":"post"}).submit();
		}
    };
	...
	{
		var paramUrl = "<c:out value="${frontDomain}"/>/pc/product/product.do?prdMstCd=<c:out value="${productInfo.PRD_MST_CD}" />";
	...
	goProductTwitter = function()
	{
		...
		var text = encodeURIComponent("<c:out value="${productInfo.PRD_MST_NM}" />");
	...
	goProductCopyClipBoard = function () {
		var paramUrl = "<c:out value="${frontDomain}"/>/pc/product/product.do?prdMstCd=${productInfo.PRD_MST_CD}"; 
	...
	getPrdStorePop = function(){
		...
		if(memId == ""){
			...
			popup('',"730","690","no","_ProductStorePopup");
  	...
  	//상품상세설명, 리뷰, Q&A, 배송/교환/반품
	$('.pr_tab a').on('click', function(e){
		e.preventDefault();
		var _this = $(this),
			_href = _this.attr('href'),
			_prev_tab = $(_href).prev('.pr_tab');

		$('html,body').animate({
			scrollTop: _prev_tab.offset().top
		}, 200);
	});
  
  	//상품이미지슬라이드
	var pr_list = $('.pr_detail .pr_image .img_box .list .img_sm'),
		pr_a = pr_list.children('a'),
		a_lg = pr_a.length,
		slider_opt = {
			controls:true,
			pager:false,
			...
			onSliderLoad: function(){
				pr_a.first().addClass('on');
			}
		};

	if(a_lg > 4) pr_list.bxSlider(slider_opt);
	else pr_a.first().addClass('on');
	pr_a.on('click', function(){
		var _this = $(this),
			_img = _this.data('img'),
			_img_b = $('.pr_image .img_box dd.img img');
		_img_b.attr('src', _img);
	...
	//일시품절, soldout
	var _sold_box = $('.pr_info .info_list .li_btnBox2');
	if(_sold_box.length){
		_sold_box.children('.btn_st4').on('mouseenter', function(){
			var _this = $(this),
				_this_w = _this.outerWidth(true),
				_this_h = _this.height(),
				_parent = _this.parent();
				css_left = (_this_w/2) + (_parent.offset().left - _this.offset().left) -40,
				css_top = _this_h+5;

			_parent.find('.que_lay').show().css({left:css_left, top:css_top});		
		}).on('mouseleave', function(){
			var _this = $(this),
				_parent = _this.parent();
			
			_parent.find('.que_lay').hide().removeAttr('style');			
		});
	}
	...
	com.timeSale = {
			showtime: function(_date, _time, _idx){
				...
				if(d_day <= -1){
					//종료
					_html_d = '<span>00</span><span>00</span><span>00</span>';
					$(_idx).html(_html_d);
	...
	if($("#promotionIdx").val() != "" && $("#promotionIdx").val() != undefined ){
		...
		if($("#promotionType").val() == "time"){
			...
			com.timeSale.showtime(edYY+"-"+edMS+"-"+edDD,edHH+":"+edMM+":00", '#tx_time2');
		}
	};
	...
	$('.btn_openClose').on('click', function(){
		var _this = $(this),
			_parent = _this.parent();
		if(!_parent.data('height')) _parent.attr('data-height', _parent.outerHeight(true));

		if(_this.hasClass('close')){
			_this.removeClass('close').children().text('닫기');
			_parent.animate({height: _parent.data('height')}, 200, function(){
				$(this).removeAttr('style');
			}).find('dl').removeAttr('style');
		}else{
			_this.addClass('close').children().text('열기');
			_parent.css({height:'auto'}).animate({height:'175px'}, 200).find('dl').hide();
		}		
	});
	...
	getPrdEsmPop = function(){
		...
		if(memId == ""){
			...
			popup('',"751","922","no","_ProductEsmPopup");
	 		$("#cartForm").attr({"action":"<c:out value="${frontDomain}" />/pc/product/searchProductEsmPopup.do", "target":"popup_ProductEsmPopup", "method":"post"}).submit();
		}
    };
    //쿠폰 다운로드 팝업
	...
	$("#menuViewTab").on("click", "a", function () {
 		...
 		if(this_tab_no == "tab1"){
 			$("#prdCtgBestList").empty();
 		}else if(this_tab_no == "tab2"){
 			prdCtgBestList();
 		}
 	});
    ...
    /* prdCtgBestList = function(){
    	prdMstCd = $("#PRD_MST_CD").val(); 
    	catIdx = $("#cateIdx").val();
    	$("#prdCtgBestList").load("<c:out value="${frontDomain}" />/pc/product/productCategoryBestListAjax.do", {"PRD_MST_CD":prdMstCd , "catIdx":catIdx});
    }; */
	...
	goMyKeeping = function(){
		$('#_blank_form').attr({'action':'<c:out value="${frontDomain}" />/pc/mypage/myKeepProductList.do','method':'get'}).submit();	
	};
	
	//1:1문의 이동
	goMyInquiryList = function(){
		var memId = "<c:out value="${commandMap.MEM_CRT_SES_ID}" />";
		var url = "<c:out value="${frontDomain}"/>/pc/mypage/myInquiryList.do";
		if(memId == ""){
			url = encodeURIComponent(url);
        	window.open("<c:out value="${serverSslDomain}"/>/pc/manager/loginPopup.do?url=" + url,"loginPopup","width=850,height=644,scrollbars=no,target=_blank");
        	return;
	...
	goSearch = function(searchText){
		...
		$searchTop.attr({'action':'<c:out value="${frontDomain}" />/pc/search/search.do','method':'post'}).submit();	
	};
	setTotalPrice();
...
function chat(){
	<c:choose>
	<c:when test="${sessionUser.MEM_MST_MEM_ID eq '' or empty sessionUser.MEM_MST_MEM_ID}">
	
	var prdurl = location.href+"&CHAT=Y";
	prdurl = encodeURIComponent(prdurl);
	window.open("<c:out value="${serverSslDomain}"/>/pc/manager/loginPopup.do?url="+prdurl+"&locationProtocol=<c:out value="${locationProtocol}"/>","loginPopup","width=850,height=644,scrollbars=no,target=_blank");
	</c:when>
	<c:otherwise>
	var url = "http://chat.etland.co.kr:7076/?id=<c:out value="${sessionUser.MEM_MST_MEM_ID }"/>";
	window.open(url,"chatPopup","width=700,height=800,scrollbars=no,target=_blank");
	</c:otherwise>
	</c:choose>
}

if("<c:out value="${commandMap.CHAT}" />" =="Y" && "<c:out value="${sessionUser.MEM_MST_MEM_ID}" />" !="") chat();
