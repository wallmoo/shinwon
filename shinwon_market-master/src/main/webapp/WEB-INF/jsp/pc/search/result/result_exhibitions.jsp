...
/*
* subject: exhibitions 페이지
* @original author: SearchTool
*/
	thisCollection = "exhibitions";
...
%>
		...
		<%if(idx%2==0){ %>
			<a href="<c:out value="${frontDomain }"/>/pc/event/planProductList.do?PLN_MST_IDX=<%=PLN_MST_IDX%>" class="f">
				<img src="<c:out value="${cdnDomain}" /><%=PC_ATT_PATH%><%=PC_SYS_NM%>" alt="">
				<strong><%=PLN_MST_TITLE%></strong>
				<span><ui:formatDate value="<%=PLN_MST_ST_DT%>" pattern="yyyy.MM.dd"/> ~ <ui:formatDate value="<%=PLN_MST_ED_DT%>" pattern="yyyy.MM.dd"/></span>
			</a>
		<%}else{ %>
			<a href="<c:out value="${frontDomain }"/>/pc/event/planProductList.do?PLN_MST_IDX=<%=PLN_MST_IDX%>" class="l">
				<img src="<c:out value="${cdnDomain}" /><%=PC_ATT_PATH%><%=PC_SYS_NM%>" alt="">
				<strong><%=PLN_MST_TITLE%></strong>
				<span><ui:formatDate value="<%=PLN_MST_ST_DT%>" pattern="yyyy.MM.dd"/> ~ <ui:formatDate value="<%=PLN_MST_ED_DT%>" pattern="yyyy.MM.dd"/></span>
			</a>
