<%@ page pageEncoding = "UTF-8" %><%!
	...
	*/
	...
	public String[] COLLECTIONS = new String[]{"exhibitions","emagazine","product","cate_product","review","category","thema"};
	public String[] COLLECTIONS_NAME = new String[]{"exhibitions","emagazine","product","cate_product","review","category","thema"};
	public class WNCollection{
	public String[][] MERGE_COLLECTION_INFO = null;
	public String[][] COLLECTION_INFO = null;
		WNCollection(){
			...
			{
			{
				"exhibitions", // set index name
				"exhibitions", // set collection name
				...
				"PLN_MST_TITLE/50,PLN_MST_PC_BANNER,PC_ALT_TXT/10",// set search field
				...
				"exhibitions" // collection display name
				}
	         ,
	         {
	 			"emagazine", // set index name
	 			"emagazine", // set collection name
	 			"0,3",  // set pageinfo (start,count)
	 			"1,0,0,0,0", // set query analyzer (useKMA,isCase,useOriginal,useSynonym, duplcated detection)
	 			"RANK/DESC,DATE/DESC",  // set sort field (field,order) multi sort '/'
	 			"basic,rpfmo,100",  // set sort field (field,order) multi sort '/'
	 			"MGZ_MST_TITLE/50,MGZ_MST_CPY/30,MGZ_MST_CTS,IMG_PATH_PC_ALT",// set search field
	 			...
	 			"", // set categoryQuery (fieldname:value)
	 			"", // set property group (fieldname,min,max, groupcount)
	 			"ALIAS,MGZ_MST_CTG_NM", // use check prefix query filed
	 			"MGZ_MST_IDX,MGZ_MST_CTG", // set use check fast access field
	 			"", // set multigroupby field
	 			"", // set auth query (Auth Target Field, Auth Collection, Auth Reference Field, Authority Query)
	 			"", // set Duplicate Detection Criterion Field, RANK/DESC,DATE/DESC
	 			"emagazine" // collection display name
	 			}
	          ,
			{
			"product", // set index name
			"product", // set collection name
			"0,3",  // set pageinfo (start,count)
			...
			"DOCID,PRD_MST_CD,PRD_MST_NM,PRD_MST_PRC,PRD_IMAGE,PRD_IMAGE_ALT,PRD_MST_ERP_DSP_PRC,ERP_MST_DSP_PRC,PRD_MST_PRICE,PRD_MST_PC_CPN_PRC,PRD_MST_LNG_DIV_YN,PRD_MST_DLV_FEE,PRD_MST_O2O_YN,REP_POINT,REP_CNT,REP_POINT_GRD,PRD_NEW_ICO,PRD_MST_OPT_EST_CNT,PRD_MST_ICON_LIST,PRD_MST_SIKR_ICON,PRD_MST_ERP_PRC,PRD_MST_OFF_ALL_CNT,PRD_MST_SEL_STATE,DSP_PRD_CTG_IDX,CATE_LEV_1,CATE_LEV_2,CATE_LEV_3,CATE_LEV_4,CATE_LEV_5,CATE_LEV_NM_1,CATE_LEV_NM_2,CATE_LEV_NM_3,CATE_LEV_NM_4,CATE_LEV_NM_5,PRD_MST_MTA_TAG1,PRD_MST_MTA_TAG2,PRD_MST_MTA_TAG3,PRD_CTG_INFO,ONLINE_HIT,OFFLINE_HIT,PRD_MST_REG_DT,DATE,PRD_MST_CPR_PRC1,PRD_MST_CPR_PRC2,PRD_NOW_DST_POINT,PRD_CPN_DST_POINT,SEARCH_RED_PRICE,SEARCH_SILVER_PRICE,SEARCH_GOLD_PRICE,SEARCH_VIP_PRICE,SEARCH_PLATINUM_PRICE,SEARCH_SIMPLE_PRICE,PRD_MST_PRICE_ASC,PRD_MST_SEL_CNT,ALIAS",// set document field
			"", // set date range
			"", // set rank range
			"", // set prefix query, example: <fieldname:contains:value1>|<fieldname:contains:value2>/1,  (fieldname:contains:value) and ' ', or '|', not '!' / operator (AND:1, OR:0)
			"", // set collection query (<fieldname:contains:value^weight | value^weight>/option...) and ' ', or '|'
			...
			"PRD_MST_CD,PRD_MST_PRC,PRD_MST_LNG_DIV_YN,PRD_MST_O2O_YN,ONLINE_HIT,OFFLINE_HIT,PRD_MST_REG_DT,PRD_MST_PRICE,PRD_MST_CPR_PRC1,PRD_MST_CPR_PRC2,SEARCH_RED_PRICE,SEARCH_SILVER_PRICE,SEARCH_GOLD_PRICE,SEARCH_VIP_PRICE,SEARCH_PLATINUM_PRICE,SEARCH_SIMPLE_PRICE,PRD_MST_PRICE_ASC", // set use check fast access field
			"", // set multigroupby field
			"", // set auth query (Auth Target Field, Auth Collection, Auth Reference Field, Authority Query)
			"", // set Duplicate Detection Criterion Field, RANK/DESC,DATE/DESC
			"product" // collection display name
			}
         ,
         {
          ...
          ,
          {
  			"review", // set index name
  			"review", // set collection name
  			"0,3",  // set pageinfo (start,count)
  			"1,0,0,0,0", // set query analyzer (useKMA,isCase,useOriginal,useSynonym, duplcated detection)
  			"RANK/DESC,DATE/DESC",  // set sort field (field,order) multi sort '/'
  			"basic,rpfmo,100",  // set sort field (field,order) multi sort '/'
  			"PRD_REV_TITLE/50,PRD_REV_CONTENTS/30,PRD_MST_NM/10",// set search field
  			"DOCID,PRD_REV_IDX,PRD_PRD_MST_CD,PRD_REV_TITLE,PRD_REV_CONTENTS/70,PRD_REV_FNC_STAR,PRD_REV_PRC_STAR,PRD_REV_SCORE_STAR,PRD_REV_AVG_STAR,PRD_REV_AVG_STAR_POINT,PRD_ORD_PRD_IDX,PRD_REV_REG_DT,PRD_REV_CHN_GBN,PRD_MST_NM,PRD_IMG,REP_CNT,ALIAS",// set document field
  			"", // set date range
  			"", // set rank range
  			"", // set prefix query, example: <fieldname:contains:value1>|<fieldname:contains:value2>/1,  (fieldname:contains:value) and ' ', or '|', not '!' / operator (AND:1, OR:0)
  			"", // set collection query (<fieldname:contains:value^weight | value^weight>/option...) and ' ', or '|'
  			...
  			"", // set auth query (Auth Target Field, Auth Collection, Auth Reference Field, Authority Query)
  			"", // set Duplicate Detection Criterion Field, RANK/DESC,DATE/DESC
  			"review" // collection display name
  			}
           ,
			{
			"category", // set index name
			"category", // set collection name
			"0,3",  // set pageinfo (start,count)
			"1,0,0,0,0", // set query analyzer (useKMA,isCase,useOriginal,useSynonym, duplcated detection)
			"RANK/DESC,DATE/DESC",  // set sort field (field,order) multi sort '/'
			"basic,rpfmo,100",  // set sort field (field,order) multi sort '/'
			"PRD_CTG_IDX",// set search field
			"DOCID,PRD_CTG_IDX,PRD_CTG_NM,PRD_CTG_LEVEL,PRD_CTG_UP_IDX,PRD_CTG_TOP_IDX,PRD_CTG_ENV,ALIAS",// set document field
			"", // set date range
			"", // set rank range
			"", // set prefix query, example: <fieldname:contains:value1>|<fieldname:contains:value2>/1,  (fieldname:contains:value) and ' ', or '|', not '!' / operator (AND:1, OR:0)
			"", // set collection query (<fieldname:contains:value^weight | value^weight>/option...) and ' ', or '|'
			...
			"", // set categoryQuery (fieldname:value)
			"", // set property group (fieldname,min,max, groupcount)
			"PRD_CTG_ENV,ALIAS", // use check prefix query filed
			"", // set use check fast access field
			"", // set multigroupby field
			"", // set auth query (Auth Target Field, Auth Collection, Auth Reference Field, Authority Query)
			"", // set Duplicate Detection Criterion Field, RANK/DESC,DATE/DESC
         ...
         ,
			{
			"thema", // set index name
			"thema", // set collection name
			"0,3",  // set pageinfo (start,count)
			"1,0,0,0,0", // set query analyzer (useKMA,isCase,useOriginal,useSynonym, duplcated detection)
			"RANK/DESC,DATE/DESC",  // set sort field (field,order) multi sort '/'
			"basic,rpfmo,100",  // set sort field (field,order) multi sort '/'
			"MGZ_MST_TITLE,MGZ_MST_CPY",// set search field
			"DOCID,MGZ_MST_IDX,MGZ_MST_THM_CTG1,MGZ_MST_THM_CTG1_NM,MGZ_MST_THM_CTG2,MGZ_MST_THM_CTG2_NM,MGZ_MST_THM_CTG3,MGZ_MST_THM_CTG3_NM,MGZ_MST_GBN,MGZ_MST_TITLE/16,MGZ_MST_REG_DT,MGZ_MST_CNT,MGZ_MST_REG_NM,MGZ_MST_YN,MGZ_MST_CTS,CMM_FLE_SAVE_PATH,MGZ_MST_CPY,MGZ_MST_CTG_NM,MGZ_MST_KID,MGZ_MST_CTG,ALIAS",// set document field
			"", // set date range
			"", // set rank range
			"", // set prefix query, example: <fieldname:contains:value1>|<fieldname:contains:value2>/1,  (fieldname:contains:value) and ' ', or '|', not '!' / operator (AND:1, OR:0)
			"", // set collection query (<fieldname:contains:value^weight | value^weight>/option...) and ' ', or '|'
			...
			"", // set categoryQuery (fieldname:value)
			"", // set property group (fieldname,min,max, groupcount)
			"MGZ_MST_THM_CTG1,MGZ_MST_THM_CTG2,MGZ_MST_THM_CTG3,ALIAS", // use check prefix query filed
			"", // set use check fast access field
			"", // set multigroupby field
			"", // set auth query (Auth Target Field, Auth Collection, Auth Reference Field, Authority Query)
			"", // set Duplicate Detection Criterion Field, RANK/DESC,DATE/DESC
			"thema" // collection display name
			}
			};
		}
	}
