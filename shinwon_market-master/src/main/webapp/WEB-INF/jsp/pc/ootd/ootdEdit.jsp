...
<div id="container">
	<div id="content" class="content">
		...
		<div class="sub_content">
				...
				<ul class="imgs_wrap">
					<c:if test="${not empty fileList}">
						<c:forEach var="imgrow" items="${fileList }" varStatus="status">
							<li><img src="${cdnDomain}${imgrow.CMM_FLE_ATT_PATH}${imgrow.CMM_FLE_SYS_NM}" alt="${imgrow.CMN_FLE_ALT_TXT}"><a href="#n" class="del prev_imgs" data-id="${imgrow.CMM_FLE_IDX}">삭제</a></li>
						</c:forEach>
					</c:if>
					<li id="add_img"><a href="#n" id="add_img_button" class="add">등록</a></li>
				</ul>
				<span id="img_limit" class="limit">(${fileList.size()}/10)</span>
			</div>
			...
			<div class="inf_edit">
				<div class="form">
					<input type="text" id="title" name="title" onkeyup="javascript:titleValidate(this)" value="${row.CNT_MST_TITLE}" placeholder="제목 입력" title="제목 입력" maxlength="50"> 
					<span id="title_limit" class="limit">(0/50)</span>
				</div>
				<div class="form">
					<textarea id="contents" name="contents"	onkeyup="javascript:contentsValidate(this)" placeholder="내용 입력" 	maxlength="200">${row.CNT_MST_DESC}</textarea>
			...
			<div class="sch-box">
				<div class="sch-box-inner">
					<div class="recom-tag-list">
						<ul id="selected_tag_containner">
							<c:forEach items="${tagList}" var="tag" varStatus="i">
								<li id="selected_tag_item_${tag.TAG_MST_ID}"><strong>${tag.TAG_MST_TITLE}</strong>
									<button type='button' class='btn-del' onclick="javascript:removeSelectedTagUI('${tag.TAG_MST_ID}')"><span>삭제</span></button>
								</li>
							</c:forEach>
						</ul>
					</div>
							...
							<ul id="tag_containner">
								<c:if test="${not empty TAG_LIST }">
									<c:forEach var="tagRow" items="${TAG_LIST}" varStatus="status">
										...
										<c:set var="tagChecked" value=""></c:set>
										
										<c:forEach var="intagRow" items="${tagList}" varStatus="instatus">
											<c:if test="${tagMstId eq intagRow.TAG_MST_ID }">
												<c:set var="tagChecked" value="checked"></c:set>	
											</c:if>
										</c:forEach>
										
										<li>
											<input class="tag_containner_checkbox" ${tagChecked} type="checkbox" id='<c:out value="${tagRow.TAG_MST_ID }" />' name="TAG_MST_ID" onchange="onChangeTagEvent(this, '${tagMstId }','${tagRow.TAG_MST_TITLE }')"name="" />
			...
			<c:set var="with_tag_var" value=""></c:set>
			<c:forEach var="wishtag" items="${wishTagList}" varStatus="status">
				<c:set var="with_tag_var" value="${with_tag_var + wishtag.WSH_TAG_TITLE}"></c:set>
				<c:if test="${status.last eq false}">
					<c:set var="with_tag_var" value="${with_tag_var}', '"></c:set>
				</c:if>
			</c:forEach>
			
			<div class="hope-tag">
				<input type="text" id="WSH_TAG_TITLE_LIST" placeholder="희망태그 입력" title="희망태그" value="${with_tag_var}"/>
				<div class="txt">
			...
			<div class="goods_edit">
				<ul id="product_container">
					<c:forEach var="productRow" items="${ootdProductList }" varStatus="i">
						<li>
							<div class="goods_img">
							...
							<div class="goods_chk">
								<input type="checkbox" value="${productRow.CNT_PRD_MST_CD}" id="${productRow.CNT_PRD_MST_CD}" checked>
								<label for="${productRow.CNT_PRD_MST_CD}"><span class="icon"></span>등록하기</label>
							</div>
						</li>
					</c:forEach>
...
<form id="ootd_form" method="post" enctype="multipart/form-data">
	...
	<input type="hidden" name="CNT_MST_IDX" value="${ commandMap.CNT_MST_IDX}"/>
...
<script>
	...
	$(document).ready(function() {
		...
		$("#goRegist").on("click", function() {
			...
			if (selFiles.length == 0 && $('.prev_imgs').length <= 0){
			...
			for(var i = 0 ; i < prdMstIdxs.length; i++) 
				ootdForm.append($('<input>', {type:'hidden', name:'PRD_MST_CD', value:prdMstIdxs[i]}));
			
			var ootdTitle = $("#OOTD_TITLE");
			var ootdContents = $("#OOTD_CONTENTS");
			var ootdImageList = $("#OOTD_IMAGE_LIST");
			var ootdTagIdList = $("#TAG_ID_LIST");
			var ootdPrd1 = $("#OOTD_PRD_1");
			var ootdPrd2 = $("#OOTD_PRD_2");
			var ootdPrd3 = $("#OOTD_PRD_3");
			...
			$(ootdForm).attr({"action":"/pc/ootd/ootdModify.do", "target":"_self", "method":"post"}).submit();
	...
	function handleImgFileSelet(e) {
		...
		filesArr.forEach(function(f) {
			...
			reader.onload = function(e) {
					//imgTag 생성
					var html = "";
					html += "<li id='ootd_image_container_id"+selFileSize+"'><img src='" + e.target.result + "' alt=''><a href='#n' onclick='delImg("
							+ selFileSize + ")' class='del'>삭제</a></li>";
	...
	function getSelectedTagIDList(){		
		...
		for(var index=0; index< selectedTagUIList.length; index++){
			...
			selectedTagIDList.push({ TAG_MST_ID : id });
		}
