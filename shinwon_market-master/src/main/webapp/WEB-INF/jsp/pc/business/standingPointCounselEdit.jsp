...
<body class="page-join">
	...
	<section id="wrap">
		...
		<input type="hidden" name="TEMP_PRD_CTG_IDX" id="TEMP_PRD_CTG_IDX" value="<c:out value="${row.PRD_CTG_IDX}" />" />
		<input type="hidden" name="TEMP_PRD_CTG_UP_IDX" id="TEMP_PRD_CTG_UP_IDX" value="<c:out value="${row.PRD_CTG_UP_IDX}" />" />
				...
				<section class="sub_lay">
					...
					<dl class="minW counsel_ask">
								...
								<tbody>
										...
										<td class="td_personName">
											<div class="inp_box">
												<label for="VDR_CST_MNG_NM">
													<input type="text" name="VDR_CST_MNG_NM" id="VDR_CST_MNG_NM" maxlength="30" value="<c:out value="${row.VDR_CST_MNG_NM}" />" />
												</label>
											</div>
										</td>
										...
										<td class="td_tel">
											<div class="inp_box">
						                                ...
						                                <c:if test="${not empty codes.TEL_CODE }">
							                                <c:forEach var="telRow" items="${codes.TEL_CODE }" varStatus="i">
							                                    <option value="<c:out value="${telRow.CMN_COM_NM }"/>" 
							                                    		<c:if test="${telRow.CMN_COM_NM eq row.VDR_CST_TEL1 }">selected="selected"</c:if>>
												...
												<label for="VDR_CST_TEL2">
													<input type="text" class="text small number" name="VDR_CST_TEL2" id="VDR_CST_TEL2" maxlength="4" title="전화번호 중간자리" value="<c:out value="${row.VDR_CST_TEL2}" />" />
												</label>
												<span class="ico_tx">-</span>
												<label for="VDR_CST_TEL3">
													<input type="text" class="text small number" name="VDR_CST_TEL3" id="VDR_CST_TEL3" maxlength="4" title="전화번호 끝자리" value="<c:out value="${row.VDR_CST_TEL3}" />" />
												</label>
											</div>
										</td>
									</tr>
										...
										<td class="td_phone">
											<div class="inp_box">
						                                ...
						                                <c:if test="${not empty codes.HP_CODE }">
						                                    <c:forEach var="hpRow" items="${codes.HP_CODE }" varStatus="i">
						                                        <option value="<c:out value="${hpRow.CMN_COM_NM }"/>" 
						                                        		<c:if test="${hpRow.CMN_COM_NM eq row.VDR_CST_HP1 }">selected="selected"</c:if>>
												...
												<label for="VDR_CST_HP2">
													<input type="text" class="text small number" id="VDR_CST_HP2" name="VDR_CST_HP2" maxlength="4" title="휴대폰 번호 중간자리" value="<c:out value="${row.VDR_CST_HP2}" />" />
												</label>
												<span class="ico_tx">-</span>
												<label for="VDR_CST_HP3">
													<input type="text" class="text small number" id="VDR_CST_HP3" name="VDR_CST_HP3" maxlength="4" title="휴대폰 번호 끝자리" value="<c:out value="${row.VDR_CST_HP3}" />" />
												</label>
											</div>
										</td>
										<th>팩스번호</th>
										<td class="td_fax">
											<div class="inp_box">
												<label for="VDR_CST_FAX1" class="">
													<select class="sel_chg" id="VDR_CST_FAX1" name="VDR_CST_FAX1" title="팩스 앞자리">
						                                ...
						                                <c:if test="${not empty codes.TEL_CODE }">
							                                <c:forEach var="telRow" items="${codes.TEL_CODE }" varStatus="i">
							                                    <option value="<c:out value="${telRow.CMN_COM_NM }"/>" 
							                                    		<c:if test="${telRow.CMN_COM_NM eq row.VDR_CST_FAX1 }">selected="selected"</c:if>>
							                                    	<c:out value="${telRow.CMN_COM_NM }"/>
							                                    </option>
							                                </c:forEach>
						                                </c:if>
												...
												<label for="VDR_CST_FAX2">
													<input type="text" class="text small number" name="VDR_CST_FAX2" id="VDR_CST_FAX2" maxlength="4" title="팩스 중간자리" value="<c:out value="${row.VDR_CST_FAX2}" />" />
												</label>
												<span class="ico_tx">-</span>
												<label for="VDR_CST_FAX3">
													<input type="text" class="text small number" name="VDR_CST_FAX3" id="VDR_CST_FAX3" maxlength="4" title="팩스 끝자리" value="<c:out value="${row.VDR_CST_FAX3}" />" />
												</label>
											</div>
										</td>
									</tr>
									<tr>
										...
										<td colspan="3" class="td_email">
											<div class="inp_box">
												<c:set var="emailary" value="${fn:split(row.VDR_CST_MAIL,'@') }" />
												<label for="VDR_CST_MAIL_ID" class="blind">email 아이디</label>
												<input type="text" name="VDR_CST_MAIL_ID" id="VDR_CST_MAIL_ID" maxlength="20" value="<c:out value="${emailary[0] }" />" />
												<span class="ico_tx">@</span>
												<label for="VDR_CST_MAIL_DOMAIN" class="blind">email 도메인</label>
												<input type="text" name="VDR_CST_MAIL_DOMAIN" id="VDR_CST_MAIL_DOMAIN" maxlength="30"  value="<c:out value="${emailary[1] }" />" />
													...
													<select id="emailDomainSelectbox" class="sel_chg">
							                            ...
							                            <c:if test="${not empty codes.EMAIL_DOMAIN_CODE }">
								                            <c:forEach var="emailRow" items="${codes.EMAIL_DOMAIN_CODE }">
								                            	<option value="<c:out value="${emailRow.CMN_COM_NM }"/>"
								                            		<c:if test="${emailRow.CMN_COM_NM eq emailary[1] }">selected="selected"</c:if>>
								                            	<c:out value="${emailRow.CMN_COM_NM }"/></option>
							                            	</c:forEach>
							                            </c:if>
													</select>													
												</label>
									...
									<tr>
										...
										<td colspan="3" class="td_askTx">
											<div class="f inp_box">
												<label for="">
													<input type="text" name="VDR_CST_SUM" id="VDR_CST_SUM" maxlength="20" value="<c:out value="${row.VDR_CST_SUM }" />" />
												</label>
									...
									<tr>
										...
										<td colspan="3" class="td_askTx2">
											<div class="f inp_box">
												<textarea id="VDR_CST_CTS" name="VDR_CST_CTS" maxlength="1000" placeholder="입점신청내용입니다."><c:out value="${row.VDR_CST_CTS }" /></textarea>
											</div>
									...
									<tr>
										...
										<td colspan="3" id="td_channel" class="td_channel">
											...
											<c:forEach var="hope" items="${codes.HOPESALE_TYPE }" varStatus="i">
	                                      		<label for="VDR_CST_ON_OFF_${i.index }" class="<c:if test="${hope.CMN_COM_IDX eq row.VDR_CST_ON_OFF}">on</c:if>">
	                                            <input type="radio" name="VDR_CST_ON_OFF" id="VDR_CST_ON_OFF_${i.index }" value="${hope.CMN_COM_IDX }" <c:if test="${hope.CMN_COM_IDX eq row.VDR_CST_ON_OFF}">checked="checked"</c:if> />
	                                            <span>${hope.CMN_COM_NM}</span>
	                                           	</label>
	                                      	</c:forEach>	
	                                      	</c:if>		
											</div>
									...
									<tr>
										...
										<td colspan="3" class="td_homepg">
											<div class="inp_box">
												<label for=""> 
													<span>http://</span>
													<input type="text" name="VDR_CST_URL" id="VDR_CST_URL" maxlength="100" value="<c:out value="${row.VDR_CST_URL }" />"  />
												</label>
											</div>
										</td>
									</tr>
									<tr>
										...
										<td colspan="3" class="td_msg">
											<div class="inp_box">
												<textarea id="VDR_CST_ETC" name="VDR_CST_ETC" maxlength="100"><c:out value="${row.VDR_CST_ETC }" /></textarea>
											</div>
										</td>
									</tr>
									<tr>
										...
										<td colspan="3" id="td_file" class="td_file">
											<div class="f inp_box">
												...
												<a href="javascript:void(0)" class="btn_gre2"><span>파일지우기</span></a>
												<p class="tx_ex">※10MB이하(zip, ppt/pptx, hwp, doc/docx, pdf)
												 ...
												 &nbsp;&nbsp;
													<c:forEach items="${fileList}" var="fileRow" varStatus="i">	
														<c:if test="${fileRow.CMM_FLE_TB_TYPE == 'S'}">
															...
															<label for="CMM_FLE_IDX_1" class="small">삭제</label>			                                        			 
														</c:if>
													</c:forEach>
												</c:if>												
												</p>
												 ...
												 &nbsp;&nbsp;
													<c:forEach items="${fileList}" var="fileRow" varStatus="i">	
														<c:if test="${fileRow.CMM_FLE_TB_TYPE == 'I'}">
															<i class="icon-file"></i>
			                                        			<c:out value="${fileRow.CMM_FLE_ORG_NM }"/>
															&nbsp;<input type="checkbox" class="checkbox cmmFleIdx" id="CMM_FLE_IDX_2" name="CMM_FLE_IDX" data-file_name="multiFile1" value="<c:out value="${fileRow.CMM_FLE_IDX }"/>" />
															<label for="CMM_FLE_IDX_2" class="small">삭제</label>			                                        			 
														</c:if>
													</c:forEach>
												</c:if>													
											</div>
						...
						<dd class="dd_btn">
							...
							<a href="javascript:goList();" class="btn_gre"><span>취소</span></a>
						</dd>
					</dl>
				</section>
				<!--// .sub_lay E -->
...
<script>
	...
	$(function(){
		...
		// 대표상품군 SelectBox
		getCategoryList("searchPrdCtgDepth1", "-1", $("#TEMP_PRD_CTG_UP_IDX").val(), "<c:out value="${frontDomain}"/>", "P");
		getCategoryList("searchPrdCtgDepth2", $("#TEMP_PRD_CTG_UP_IDX").val(), $("#TEMP_PRD_CTG_IDX").val(), "<c:out value="${frontDomain}"/>", "P");
		...
		// 유통방법 Checked
		var p_VDR_CST_DTB = '${row.VDR_CST_DTB}';
		
   		$('input[name=VDR_CST_DTB]').each(function(){
   			if($(this).val() != '' && p_VDR_CST_DTB.indexOf($(this).val()) > -1){
		...
		_td_file.find('input[type="file"]').on('change', function(){
			var _this = $(this);
			_this.prev('input[type="text"]').val(_this.val());
		});
		_td_file.find('.btn_gre2').on('click', function(){
	 		...
	 		if($frm.valid()){
                ...
                $("#VDR_CST_MAIL").val($("#VDR_CST_MAIL_ID").val() + "@" + $("#VDR_CST_MAIL_DOMAIN").val());
           		...
           		if(confirm( "저장 하시겠습니까?")) {
   	                $frm.attr({"action":"<c:out value="${serverSslDomain}" />/pc/business/standingPointCounselModify.do", "target":"_self", "method":"post"}).submit();
                }   
	        }
	    };
