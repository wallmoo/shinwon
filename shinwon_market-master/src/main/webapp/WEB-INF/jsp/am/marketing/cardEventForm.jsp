	                    		...
	                    		<td>
	                    			<c:if test="${not empty cardEventCodeList }">
		 	                    		<c:forEach var="gbRow" items="${cardEventCodeList }" varStatus="i">
				                            <input type="radio" name="CRD_PMT_GBN" id="state<c:out value="${i.index}" />" class="radio" value="<c:out value="${gbRow.CMN_COM_IDX }"/>" <c:if test="${i.index eq 0 }">checked="checked"</c:if>/>
...
<tr>
	...
	<td><input type="text" class="number right" name="PMT_CRD_SAL_PRC" id="PMT_CRD_SAL_PRC\${NUM}" /> 만원 / %</td>
	<td><input type="text" class="number right" name="PMT_CRD_SML_PRC" id="PMT_CRD_SML_PRC\${NUM}" /> 만원</td>
	<td><input type="text" class="number right" name="PMT_CRD_MXM_PRC" id="PMT_CRD_MXM_PRC\${NUM}" /> 만원</td>
...
$(function(){
    ...
    fnCardChk3 = function(){
    	var $obj = $("#cardTable").find(".applyCardList .selectCard");
	   	...
	   	for(i=0;i<len;i++)
	   	{   var PMT_CRD_SAL_GBN = $("#PMT_CRD_SAL_GBN"+i).val();
	   		var temp = $("#PMT_CRD_MXM_PRC"+i).val();
	   		if(temp == ""&&PMT_CRD_SAL_GBN != 'M')bReturn=i;
	   	}
    ...
    $(document).on("click", ".delCardEvent", function () {
       ...
       //var $topParent = $(this).parents(".section-form");
        ...
        if (confirm("삭제 하시겠습니까?")) 
        {
	        ...
	        {
	            $cardList.append('<tr class="cardNodata"><td colspan="8">행사 카드를 추가해 주세요.</td></tr>');
	        }
	        //체크박스 전체 선택
