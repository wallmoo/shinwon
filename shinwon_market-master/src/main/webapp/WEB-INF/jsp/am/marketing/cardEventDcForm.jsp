	                    		...
	                    		<td>
	                    			<c:if test="${not empty cardEventCodeList }">
		 	                    		<c:forEach var="gbRow" items="${cardEventCodeList }" varStatus="i">
				                            <input type="radio" name="CRD_PMT_GBN" id="state<c:out value="${i.index}" />" class="radio" value="<c:out value="${gbRow.CMN_COM_IDX }"/>" <c:if test="${i.index eq 1 }">checked="checked"</c:if>/>
