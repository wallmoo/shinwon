		...
		*/
		...
		public String getPageLinks(int startCount, int totalCount, int viewListCount, int bundleCount) {
			...
			if(wnanchor.getFirstPage() != -1) {
				sbRet.append("<a class=\"ir first\" href=\"#none\" onClick=\"javascript:doPostPaging('"+wnanchor.getFirstPage()+"');\"><em>처음 페이지</em></a>");
				sbRet.append("<a class=\"ir prev\" href=\"#none\" onClick=\"javascript:doPostPaging('"+wnanchor.getBundleBefore()+"');\"><em>이전 페이지</em></a>");
			} else {
				sbRet.append("<a class=\"ir first\" href=\"#none\"><em>처음 페이지</em></a>");
				sbRet.append("<a class=\"ir prev\" href=\"#none\"><em>이전 페이지</em></a>");
			} 
			...
			for(int i=0; i<pageCount && i < pages.length; i++) {
				if(pages[i][1].equals("-1")) {
					sbRet.append("<strong>"+ pages[i][0] + "<span class=\"semantic\">&nbsp;페이지 - 현재 위치</span></strong>");
				} else {
					sbRet.append("<a href=\"#none\" onClick=\"javascript:doPostPaging('"+pages[i][1]+"');\"> "+pages[i][0]+" </a>");
				}
			}

			if(wnanchor.getBundleNext() != -1) {
    			sbRet.append("<a class=\"ir next\" href=\"#none\" onClick=\"javascript:doPostPaging('"+wnanchor.getBundleNext()+"')\"><em>다음 페이지</em></a>");
    			sbRet.append("<a class=\"ir last\" href=\"#none\" onClick=\"javascript:doPostPaging('"+wnanchor.getLastPage()+"')\"><em>끝 페이지</em></a>");
			} else {
				sbRet.append("<a class=\"ir next\" href=\"#none\"><em>다음 페이지</em></a>");
				sbRet.append("<a class=\"ir last\" href=\"#none\"><em>끝 페이지</em></a>");
			}
