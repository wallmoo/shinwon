            ...
            <section id="contents">
                ...
                <div id="breadcrumb">
                    ...
                    <a href="#none"><i class="icon-home em"></i> HOME</a> &gt; <a
                        href="#none">시스템 관리</a> &gt; <span>인기 검색어 관리</span>
                </div>
                <!-- // breadcrumb -->
                                            ...
                                            <c:when test="${not empty list }">                                              
                                                <c:forEach var="list" items="${list }" varStatus="i">
                                                    <tr>
                                                        ...
                                                        <td><input type="text" name="PPL_WRD_SORT" value="${list.PPL_WRD_SORT }" /></td>
                                                        <td><input type="text" name="PPL_WRD_TEXT" class="text long" value="${list.PPL_WRD_TEXT }"/></td>
                                                    </tr>                                                   
                                                </c:forEach>
                                            </c:when>   
                                            <c:otherwise> 
                                                <c:forEach var="row" begin="1" end="10" varStatus="i">
                                                    <tr>
                                                        ...
                                                        <td><input type="text" name="PPL_WRD_SORT"/></td>
                                                        <td><input type="text" name="PPL_WRD_TEXT" class="text long"  /></td>
                                                    </tr>                                                   
                                                </c:forEach>
                                             </c:otherwise>                                      
                                        </c:choose> 
...
<script type="text/javascript">
    $(document).ready(function() 
    {
        ...
        $(document).on("click", "#goRegist", function() 
        {
            ...
            $.each($("input[name^='PPL_WRD_SORT']"),function(index,item)
            {                   
                if($("input[name^='PPL_WRD_SORT']").eq(index).val() == "") 
                {   
                    alert("[순서] 반드시 입력해 주십시오.");
//                  $("input[name^='REL_SCH_IDX']").eq(index).focus();
            ...
            {                   
                if($("input[name^='PPL_WRD_TEXT']").eq(index).val() == "") 
                {   
                    alert("[검색어] 반드시 입력해 주십시오.");
