...
<div id="container"><!-- container -->
    ...
    <div id="wrapper"><!-- wrapper -->
        ...
        <div class="container">
            ...
            <h2 class="title"><span>추가혜택 상세</span></h2>
            ...
            <div class="section-form"><!-- section-form-a -->
            	<input type="hidden" name="BNF_MST_IDX"  value="<c:out value="${row.BNF_MST_IDX }"/>" />
				<input type="hidden" name="cPage" value="<c:out value="${commandMap.cPage }"/>"/>
				<input type="hidden" name="pageSize" value="<c:out value="${commandMap.pageSize }"/>" />
				<input type="hidden" name="searchBenefitNm" value="${commandMap.searchBenefitNm }" />
                <input type="hidden" name="searchBenefitStDt" value="${commandMap.searchBenefitStDt }" />
                ...
                <table cellspacing="0" class="table-row table-a"><!-- table -->
                    ...
                    <tbody>
                        <tr>
                            ...
                            <td>
                                <input type="text" class="text xlarge" name="BNF_MST_NM" value="${row.BNF_MST_NM }" placeholder="추가혜택명 입력">
                            </td>
                        </tr>
                        <tr>
                            ...
                            <input type="text" class="text" id="BNF_MST_ST_DT" name="BNF_MST_ST_DT" readonly="readonly" data-target-end="#BNF_MST_ED_DT" value="<ui:formatDate value="${row.BNF_MST_ST_DT }" pattern="yyyy-MM-dd"/>"/>
                            <select id="BNF_MST_ST_HH" name="BNF_MST_ST_HH" class="select">
                               	<option value="">시</option>
		                        	<c:forEach begin="0" end="23" step="1" varStatus="i">
		                        		...
		                        		<option value="${hh}" <c:if test="${row.BNF_MST_ST_HH eq hh}">selected="selected"</c:if> >${hh}</option>
		                        	</c:forEach>
							</select>
                            <select id="BNF_MST_ST_MM" name="BNF_MST_ST_MM" class="select">
                                <option value="">분</option>
		                        	<c:forEach begin="0" end="59" step="1" varStatus="i">
		                        		...
		                        		<option value="${mm}" <c:if test="${row.BNF_MST_ST_MM eq mm}">selected="selected"</c:if> >${mm}</option>
		                        	</c:forEach>
							</select>
                            <a href="javascript:;" class="js-datepickr"><span class="icon datepickr"><em>날짜선택</em></span></a>
                            <input type="text" class="text" id="BNF_MST_ED_DT" name="BNF_MST_ED_DT" readonly="readonly" data-target-start="#BNF_MST_ST_DT" value="<ui:formatDate value="${row.BNF_MST_ED_DT }" pattern="yyyy-MM-dd"/>"/>
                            <select id="BNF_MST_ED_HH" name="BNF_MST_ED_HH" class="select">
                            	<option value="">시</option>
		                        	<c:forEach begin="0" end="23" step="1" varStatus="j">
		                        		...
		                        		<option value="${hh}" <c:if test="${row.BNF_MST_ED_HH eq hh}">selected="selected"</c:if> >${hh}</option>
		                        	</c:forEach>
                            </select>
                            <select id="BNF_MST_ED_MM" name="BNF_MST_ED_MM" class="select">
	                        	...
	                        	<c:forEach begin="0" end="59" step="1" varStatus="i">
	                        		...
	                        		<option value="${mm}" <c:if test="${row.BNF_MST_ED_MM eq mm}">selected="selected"</c:if>  >${mm}</option>
								</c:forEach>
                            </select>
                        </td>
                        </tr>
                        <tr>
                            ...
                            <td>
                                <input type="radio" id="BRD_INQ_USE_YN_A" name="BNF_MST_USE_YN" class="radio"  value="Y" <c:if test="${row.BNF_MST_USE_YN eq 'Y' }">checked="checked"</c:if>/>
                                <label for="BRD_INQ_USE_YN_A">사용</label>
                                <input type="radio" id="BRD_INQ_USE_YN_B" name="BNF_MST_USE_YN"class="radio"  value="N" <c:if test="${row.BNF_MST_USE_YN eq 'N' }">checked="checked"</c:if>/>
	                    	...
	                    	<c:when test="${not empty detailList }">
	                    		<c:forEach var="item" items="${detailList }" varStatus="i">
	                    			<tr>
			                            ...
			                            <td><input type="text" name="BNF_DTL_NM" value="<c:out value="${item.BNF_DTL_NM }"/>" class="text xlarge" placeholder="한글 10자, 영문숫자 20자 입력" maxlength="20"></td>
			                            <td><input type="text" name="BNF_DTL_DESC" value="<c:out value="${item.BNF_DTL_DESC }"/>" class="text xlarge" placeholder="한글 10자, 영문숫자 20자 입력" maxlength="50"></td>
			                            <td>
			                                <select class="select" name="BNF_DTL_DPL_USE_YN">
			                                    <option value="Y" <c:if test="${item.BNF_DTL_DPL_USE_YN eq 'Y'}">selected</c:if>>Y</option>
			                                    <option value="N" <c:if test="${item.BNF_DTL_DPL_USE_YN eq 'N'}">selected</c:if>>N</option>
			                                </select>
			                            </td>
	                        		</tr>
	                    		</c:forEach>
...
$(function(){
	...
	$(document).on("click",	"#goRegist",function() {
		var $frm = $("#frm");
        var url = "<c:out value="${serverDomain}"/>/am/site/benefitManagerModify.do";
        if($frm.valid())
        {
