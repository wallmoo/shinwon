												...
												<tr>
	                            					<td>
	                            						...
	                            						<a href="javascript:;" id="fileIdx_<c:out value="${fileRow.CMM_FLE_IDX }"/>" class="file-list-title" data-cmm_fle_idx="<c:out value="${fileRow.CMM_FLE_IDX }"/>" data-cmm_fle_save_path="<c:out value="${fileRow.CMM_FLE_SAVE_PATH }"/>"> 														
			                                    			...
			                                    			<input type="checkbox" class="checkbox cmmFleIdx" id="CMM_FLE_IDX_<c:out value="${fileRow.CMM_FLE_IDX }"/>" name="CMM_FLE_IDX" data-file_name="file" data-file_seq="<c:out value="${fileCnt }" />" value="<c:out value="${fileRow.CMM_FLE_IDX }"/>" /> 
...
$(document).ready(function () {
    ...
    $("#addFile").click(function(){
    	...
    	if(fileLen == 10){
	    	...
	    	$("#fileView").append('<tr><td>' + '<input type="hidden" name="EML_LNK_TYPE" id="EML_LNK_TYPE" value="U" /><input type="file" class="file" name="file" id="files['+ fileNum +']"/><br><br>URL&nbsp;&nbsp;<input type="text" name="EML_LNK_URLs" id="EML_LNK_URLs" class="text"  style="width: 60%;" maxlength="100" />' + '</td><td>'+'<input type="button" onClick="rowDelete(this)" value="-"/>'+'</td></tr>');
	...
	$(".cmmFleIdx").click(function(){
		...
		if($this.is(":checked"))
		{
			$("input[name=file]").eq(fileSeq).prop("disabled", false);
		}
		else
		{
			$("input[name=file]").eq(fileSeq).val("");
			$("input[name=file]").eq(fileSeq).prop("disabled", true);
		}
	});
    ...
    $(document).on("click", "#goRegist", function () 
    {
        ...
        if($frm.valid()){
        	...
        	var chkLen = 0;
        	var oldfileLen = '<c:out value="${fn:length(mailTemplatFile)}" />';
        	
        	$("input[name=CMM_FLE_IDX]").each(function(){
        		if($(this).is(":checked")){
        			chkLen ++;
        		}
        	});
        	
        	if(chkLen == oldfileLen){
        	...
        	if(($("input[name=file]").length + parseInt(chkLen)) > 10){
