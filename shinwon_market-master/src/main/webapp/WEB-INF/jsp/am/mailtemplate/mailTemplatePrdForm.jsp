        ...
        <section id="contents"><!-- contents -->
            ...
            <div class="contents-inner"><!-- contents-inner -->
				...
				<div class="title-box">
					...
					<div class="right">
						<c:choose>
							<c:when test="${path eq 'PrdA' }">
								<label class="label"><span><em class="strong">※ 1개 상품그룹에 상품은 6개까지 등록하셔야 합니다.</em></span></label>
							</c:when>
							<c:otherwise>
								<label class="label"><span><em class="strong">※ 1개 상품그룹에 상품은 4개~6개까지 등록하셔야 합니다.</em></span></label>
							</c:otherwise>
						</c:choose>
  						...
  						<div class="section-form"><!-- section-form-a -->
              				<input type="hidden" name="groupNum" class="groupNum" value="0"/>
                       		<div class="form-group">                
								...
								<div class="field">
									<input type="text" name="EML_GRP_NM0" class="text PLN_GRP_TITLE long" data-rnum="0" maxlength="66"/>
								</div>
                       		</div>
...
<div class="form-group">                
	<label class="label"><span>그룹명<em class="strong">*</em></span></label>
	<div class="field">
		<input type="text" name="EML_GRP_NM\${NUM}" class="text PLN_GRP_TITLE long" data-rnum="\${NUM}" maxlength="66"/>
	</div>
</div>
...
$(document).ready(function () {
    ...
    $("#addFileUp").click(function(){
    	...
    	if(fileLen == 10){
	    	...
	    	var fileHtml = "";
	    	fileHtml = fileHtml + '<tr><td>' + '<input type="file" class="file" name="fileUp" id="fileUp['+ fileNum +']"/>' + '<br><br>URL&nbsp;&nbsp;<input type="text" name="EML_LNK_UP_URLs" id="EML_LNK_UP_URLs" class="text"  style="width: 60%;" maxlength="100" /></td><td>'+'<input type="button" onClick="rowDelete(this)" value="-"/>'+'</td></tr>';
    ...
    $("#addFileDn").click(function(){
    	...
    	if(fileLen == 10){
    		...
    		var fileHtml = "";
	    	fileHtml = fileHtml + '<tr><td>' + '<input type="file" class="file" name="fileDn" id="fileDn['+ fileNum +']"/>' + '<br><br>URL&nbsp;&nbsp;<input type="text" name="EML_LNK_DN_URLs" id="EML_LNK_DN_URLs" class="text"  style="width: 60%;" maxlength="100" /></td><td>'+'<input type="button" onClick="rowDelete(this)" value="-"/>'+'</td></tr>';
        ...
        if($frm.valid()){
            ...
            $(".applyProductList").each(function(){
           		if($(this).find(".checkRow").length <= 0)
                {
           		...
           		//관련상품 템플릿 상품 갯수 확인
           		if(path == "PrdA"){
           			if($(this).find(".checkRow").length != 6){
           				prdCntChk = false;
           				prdCntChkAtr = "등록 상품은 6개로 마춰야 합니다.";
           				return false;
           			}
           		}else{
           			if($(this).find(".checkRow").length != 4 && $(this).find(".checkRow").length != 6){
           				prdCntChk = false;
           				prdCntChkAtr = "등록 상품은 4개 ~ 6개로 마춰야 합니다.";
           				return false;
           			}
           			if($(this).find(".checkRow").length == 5){
           				prdCntChk = false;
           				prdCntChkAtr = "등록 상품은 4개 ~ 6개 짝수로 구성되어야 합니다.";
           				return false;
           			}
           		}
            });
