...
function goOrderViewPopup(serverDomain, ordMstCd) {
    popup(serverDomain+"/am/order/orderPopup.do?ORD_MST_CD="+ordMstCd, "1700", "700", "yes", "orderView");
}

// 검색폼 초기화
function searchClear(formId){
       $("#"+formId).find("div").each(function () {
                ...
                $el.each(function () {
                    ...
                    switch ($tag.prop("tagName").toLowerCase())
                    {
                        case "input" :
                            ...
                            if ($tag.attr("type") != "checkbox" && $tag.attr("type") != "radio") // INPUT TEXT, PASSWORD, FILE
                            {
                                $tag.val(""); // VALUE CLEAR
                            }
                            else 
                            {
                                 if ($tag.attr("type") == "radio") // INPUT RADIO
                                 {
                                     if ($tag.index() == 0) // FIRST INDEX CEHCKED
                                     {
                                         $tag.prop("checked", true);
                                     }
                                 }
                                 else if ($tag.attr("type") == "checkbox") // INPUT CHECK BOX
                                 {
                                     $tag.prop("checked", false); // CEHCKED FALSE
                                 }
                            }
                            break;
                        ...
                        case "select" :
                            ...
                            $tag.children("option:eq(0)").prop("selected", true); // FIRST OPTION SELECTED
                            break;
                        ...
                        case "label" :
                            ...
                            var $labels = $tag.children(); // FIND CHILDREN
                            
                            if ($labels.length > 0)
                            {
                                $labels.each(function () {
                                    
                                    var $label = $(this); // LABEL CHILD HTML TAG
                                    
                                    if ($label.prop("tagName").toLowerCase() == "input") // INPUT
                                    {
                                        if ($label.attr("type") == "radio") // INPUT RADIO
                                        {
                                            if ($tag.index() == 0)
                                            {
                                                $label.prop("checked", true); // FIRST INDEX CHECKED
                                            }
                                        }
                                        else if ($label.attr("type") == "checkbox") // INPUT CEHCKBOX
                                        {
                                            $label.prop("checked", false); // CEHCKED FALSE
                                        }
                                    }
                                });
                            }
        ...
        $.ajax({
            ...
            success : function(data)
            {
                ...
                if(obj == "applyItemCategoryList"){
                    ...
                    {
                        if(showType == "checkbox"){
                            itmHtml = itmHtml + "<input type=\"checkbox\" name=\"searchErpMstNfis\" id=\"searchErpMstNfis_"+i+"\" value=\""+item.ITM_CTG_NM+"\" ";
                            
                            if(selectIdx != ""){
                                for(ii = 0; ii <arrySelectIdx.length; ii++){
                                    if($.trim(arrySelectIdx[ii]) == item.ITM_CTG_NM){
