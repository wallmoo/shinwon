						...
						<tbody>
							<c:choose>
								<c:when test="${not empty list}">
									...
									<tr>
										<td rowspan="2">
											<input type="checkbox" name="PRD_MST_CDs" value="<c:out value="${item.PRD_MST_CD }" />" class="checkbox checkRow" data-cd="${item.PRD_MST_CD}"/>
											<input type="hidden" id="VDR_APV_IDX_${item.PRD_MST_CD }" name="VDR_APV_IDX_${item.PRD_MST_CD }" value="<c:out value="${item.VDR_APV_IDX }"/>" />
										</td>
										<td rowspan="2"><c:out value="${listNum }" /><br /><a href="javascript:goView('<c:out value="${item.PRD_MST_CD }" />');">[수정]</a></td>
										...
										<td colspan="2">
											<input type="text" name="PRD_MST_PRC_${item.PRD_MST_CD }" id="PRD_MST_PRC_${item.PRD_MST_CD }" class="text small" value="${item.PRD_MST_PRC }" readonly>
										</td>
										
										<td colspan="2">
											<input type="number" name="PRD_MST_DLV_FEE_${item.PRD_MST_CD }" id="PRD_MST_DLV_FEE_${item.PRD_MST_CD }" value="${item.PRD_MST_DLV_FEE}"/>
										</td>
										...
										<td>
											<div class="tbl_line">
												<select class="select small" name="PRD_MST_UNT_${item.PRD_MST_CD }" id="PRD_MST_UNT_${item.PRD_MST_CD }">
													<option value="R" <c:if test="${fn:contains(item.PRD_MST_UNT_NM, '정율')}">selected</c:if>>정율</option>
													<option value="S" <c:if test="${fn:contains(item.PRD_MST_UNT_NM, '정액')}">selected</c:if>>정액</option>
												</select>
											</div>
										</td>
								...
								<c:otherwise>
									...
									<tr>
										<td class="td_no_result" colspan="13">${Code.COMMON_MESSAGE_SEARCH_RESULT_NONE_}</td>
									</tr>
									</c:if>
									<c:if test="${param.searchYn ne 'Y'}">
									<tr>
										<td class="td_no_result" colspan="13">${Code.COMMON_MESSAGE_DO_SEARCH_}</td>
									</tr>
									</c:if>
								</c:otherwise>
							</c:choose>
