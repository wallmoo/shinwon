            ...
            <div class="contents-inner"><!-- contents-inner -->
                ...
                <h3 class="title">브랜드전시-아낙</h3>
                 ...
                 <div class="tab-a justified" style="margin-top: 20px; width: 100%"><!-- tab -->
                    <ul class="tab">
                        <li id="ANAC_MAIN_TAB" class = "active"><a href="javascript:fnAnacChangeTab('MAIN');" >Main Page</a></li>
                ...
                <div class="section-form"><!-- section-form-a -->
                    ...
                    <form id="searchForm" name="searchForm">
	                    ...
	                    <div class="form-group">
                            ...
                            <div class="field">
                                <input type="radio" id="stateA" name="searchAnaMstDplYn" class="radio" <c:if test="${empty commandMap.searchAnaMstDplYn }">checked="checked"</c:if> value=""/>
                                <label for="stateA">전체</label>
                                <input type="radio" id="stateB" name="searchAnaMstDplYn" class="radio" <c:if test="${commandMap.searchAnaMstDplYn eq 'Y' }">checked="checked"</c:if> value="Y"/>
                                <label for="stateB">전시</label>
                                <input type="radio" id="stateC" name="searchAnaMstDplYn" class="radio" <c:if test="${commandMap.searchAnaMstDplYn eq 'N' }">checked="checked"</c:if> value="N"/>
                                ...
                                <c:when test="${not empty list }">
                                    ...
                                    <c:forEach var="row" items="${list }" varStatus="i">
                                        <tr>
			                                ...
			                                <td>
                                                <a href="#none" class="list-title" data-ana_mst_idx="${row.ANA_MST_IDX }" }">
                                                    <c:out value="${row.ANA_MST_IDX }"/>
                                                </a>
			                                </td>
			                                <td>
			                                	Main
			                                ...
			                                <td><ui:formatDate value="${row.ANA_MST_REG_DT }" pattern="yyyy.MM.dd HH:mm:ss"/></td>
			                                <td>
                                                <span class="label label-<c:out value="${row.ANA_MST_DPL_YN eq 'Y'?'b':'a' }"/>">
                                                    <c:out value="${row.ANA_MST_DPL_YN eq 'Y'?'전시':'미전시' }"/>
                                                </span>
                                            </td>
			                            </tr>
                ...
                <form id="frm" name="frm">
                    <input type="hidden" name="ANA_MST_IDX" value="" />
...
$(document).ready(function () {
    ...
    $(document).on("click", "#goForm", function () {
        $("#frm").attr({"action":"<c:out value="${serverDomain}" />/am/display/anacMainForm.do", "target":"_self", "method":"post"}).submit();
    });
    
    // 제목 a Tag 클릭 이벤트
    $(document).on("click", "#list-field .list-title", function () {
        var $frm = $("#frm");
        $frm.find("input[name='ANA_MST_IDX']").val($(this).data("ana_mst_idx"));
        
        $frm.attr({"action":"<c:out value="${serverDomain}" />/am/display/anacMainEdit.do", "target":"_self", "method":"post"}).submit();
    ...
    goPage = function (cPage) 
    {
        ...
        $frm.attr({"action":"<c:out value="${serverDomain}" />/am/display/anacMainList.do", "target":"_self", "method":"post"}).submit();
    };
