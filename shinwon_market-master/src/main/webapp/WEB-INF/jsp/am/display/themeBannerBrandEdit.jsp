...
<form name="frm" id="frm" enctype="multipart/form-data">
    ...
    <input type="hidden" id="BNR_MST_TITLE" name="BNR_MST_TITLE" value=""/>
	<table cellspacing="0" class="table-row table-a"><!-- table -->
		...
		<tbody>
			...
			<tr>
				...
				<td>
					<a href="javascript:searchBrand();" class="button primary"><span>브랜드선택</span></a>
	            	<input type="text" name="file" id="file" value="<c:out value="${row.BND_MST_BRAND_NM }"/>" disabled />
	            	<input type="hidden" id="BND_MST_IDX" name="BND_MST_IDX" value="<c:out value="${row.BND_MST_IDX }"/>" />
				    <input type="hidden" id="BNR_MST_NM" name="BNR_MST_NM" value="" />
				    <input type="hidden" id="BNR_MST_SERV" name="BNR_MST_SERV" value="" />
				</td>
			</tr>
			...
			<tr>
				...
				<td>
					<textarea  name="BNR_MST_DESC" id="BNR_MST_DESC" class="textarea xlarge" style="width: 80%;" maxlength="300">${row.BNR_MST_DESC }</textarea>
				</td>
			</tr>
...
$(function () {
 ...
 // 폼체크
    $("#frm").validate({
        rules: 
        {
            ...
            BNR_DTL_URL:{required:{depends:function(){
                if( $(':radio[name="BNR_DTL_TARGET"]:checked').val() != "" ) return true;
                else return false;
            }}, url:true} // 링크 주소
        },
        messages:
    ...
    goModify = function () {
        ...
        if($frm.valid()){
        	if($("#initState").val() == 'N' && $("input:radio[name='BNR_MST_USE_YN']:checked").val() == 'Y' && $("#maxListLimit").val() == $("#useListNum").val() ){
				alert("최대 " + $("#maxListLimit").val() +  + "1개의 배너를 노출 할 수 있습니다.");
        		return;
    ...
    searchBrand = function(){
    	popup("<c:out value="${serverDomain}" />/am/product/searchBrandPopup.do", "900", "500", "yes");
    }
    fnResultBrand = function(brdMstIdx, brdMstNm,brdMstServ){
    	$("input[name='file']").val(brdMstNm);
    	$("#BND_MST_IDX").val(brdMstIdx);
    	$("#BNR_MST_NM").val(brdMstNm);
    	$("#BNR_MST_SERV").val(brdMstServ);
    	$("#BNR_MST_TITLE").val(brdMstNm);
    }
});
