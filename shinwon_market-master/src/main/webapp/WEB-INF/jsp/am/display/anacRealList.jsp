        ...
        <section id="contents"><!-- contents -->
            ...
            <div class="contents-inner"><!-- contents-inner -->
                ...
                <h3 class="title">브랜드전시-아낙</h3>
                 ...
                 <div class="tab-a justified" style="margin-top: 20px; width: 100%"><!-- tab -->
                    <ul class="tab">
                        ...
                        <li id="ANAC_REAL_TAB" class = "active" ><a href="javascript:fnAnacChangeTab('REAL');" >REAL ANAC</a></li>
                ...
                <div class="section-form"><!-- section-form-a -->
                    ...
                    <form id="searchForm" name="searchForm">
	                    ...
	                    <div class="form-group">
                            ...
                            <div class="field">
                                <input type="radio" id="stateA" name="searchAnaRalDplYn" class="radio" <c:if test="${empty commandMap.searchAnaRalDplYn }">checked="checked"</c:if> value=""/>
                                <label for="stateA">전체</label>
                                <input type="radio" id="stateB" name="searchAnaRalDplYn" class="radio" <c:if test="${commandMap.searchAnaRalDplYn eq 'Y' }">checked="checked"</c:if> value="Y"/>
                                <label for="stateB">전시</label>
                                <input type="radio" id="stateC" name="searchAnaRalDplYn" class="radio" <c:if test="${commandMap.searchAnaRalDplYn eq 'N' }">checked="checked"</c:if> value="N"/>
                                ...
                                <c:when test="${not empty list }">
                                    ...
                                    <c:forEach var="row" items="${list }" varStatus="i">
                                        <tr>
			                                ...
			                                <td>
                                                <a href="#none" class="list-title" data-ana_ral_idx="${row.ANA_RAL_IDX }" }">
                                                    <c:out value="${row.ANA_RAL_IDX }"/>
                                                </a>
			                                </td>
			                                <td>
			                                ...
			                                <td>
                                                <a href="<c:out value="${row.ANA_RAL_SNS_LNK }"/>" target="_blank">
                                                    <c:out value="${row.ANA_RAL_SNS_LNK }"/>
                                                </a>
			                                </td>
			                                <td><ui:formatDate value="${row.ANA_RAL_REG_DT }" pattern="yyyy.MM.dd HH:mm:ss"/></td>
			                                <td>
                                                <span class="label label-<c:out value="${row.ANA_RAL_DPL_YN eq 'Y'?'b':'a' }"/>">
                                                    <c:out value="${row.ANA_RAL_DPL_YN eq 'Y'?'전시':'미전시' }"/>
                                                </span>
                                            </td>
			                            </tr>
                ...
                <form id="frm" name="frm">
                    <input type="hidden" name="ANA_RAL_IDX" value="" />
...
$(document).ready(function () {
    ...
    $(document).on("click", "#goForm", function () {
        $("#frm").attr({"action":"<c:out value="${serverDomain}" />/am/display/anacRealForm.do", "target":"_self", "method":"post"}).submit();
    });
    
    // 제목 a Tag 클릭 이벤트
    $(document).on("click", "#list-field .list-title", function () {
        var $frm = $("#frm");
        $frm.find("input[name='ANA_RAL_IDX']").val($(this).data("ana_ral_idx"));
        
        $frm.attr({"action":"<c:out value="${serverDomain}" />/am/display/anacRealEdit.do", "target":"_self", "method":"post"}).submit();
    ...
    goPage = function (cPage) 
    {
        ...
        $frm.attr({"action":"<c:out value="${serverDomain}" />/am/display/anacRealList.do", "target":"_self", "method":"post"}).submit();
    };
