				                       	...
				                       	<c:when test="${not empty detailList }">
											<c:forEach var="rowDetail" items="${detailList }" varStatus="i">
												<tr>
													...
													<td>
														<input type="text" class="text" name="ANA_DTL_LNK" id="ANA_DTL_LNK<c:out value="${i.index }"/>" value="<c:out value="${rowDetail.ANA_DTL_LNK }"/>" />
...
$(document).ready(function() {
    ...
    $(document).on("click", "#goRegist", function() {
        ...
        {	 
        	$("#frm").attr({"action" : "<c:out value="${serverDomain}" />/am/display/anacMainModify.do", "target" : "_self", "method" : "post"}).submit();	
        }    
