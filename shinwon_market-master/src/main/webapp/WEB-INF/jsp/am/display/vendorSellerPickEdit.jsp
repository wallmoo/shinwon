                ...
                <table cellspacing="0" class="table-row table-a"><!-- table -->
                    ...
                    <tbody>
                    		...
                    		<td>
                    			<div id="multiFile1-list">
									...
									<c:if test="${fileRow.CMM_FLE_TB_TYPE == 'P' && fileRow.CMM_FLE_SORT == '1'}">
										...
										<input type="checkbox" class="checkbox cmmFleIdx" id="CMM_FLE_IDX_1" name="CMM_FLE_IDX" data-file_name="pcFile" data-file_id="multiFile1" value="<c:out value="${fileRow.CMM_FLE_IDX }"/>" />
										<label for="CMM_FLE_IDX_1">삭제</label>
									</c:if>
									</c:forEach>
								</div>
								<input type="file" name=pcFile id="multiFile1" class="file pcImgFile" data-attach_cnt="1" accept=".jpg, .png , .gif" />
                    		...
                    		<td>
                    			<div id="multiFile2-list">
									...
									<c:if test="${fileRow.CMM_FLE_TB_TYPE == 'M' && fileRow.CMM_FLE_SORT == '1'}">
										...
										<input type="checkbox" class="checkbox cmmFleIdx" id="CMM_FLE_IDX_2" name="CMM_FLE_IDX" data-file_name="mobileFile" data-file_id="multiFile3" value="<c:out value="${fileRow.CMM_FLE_IDX }"/>" />
										<label for="CMM_FLE_IDX_2">삭제</label>
									</c:if>
									</c:forEach>
								</div>
								<input type="file" name="mobileFile" id="multiFile3" class="file mobileImgFile" data-attach_cnt="1" accept=".jpg, .png , .gif" />
								...
								<p class="img-text">
									...
									<input type="hidden" class="pcImgFile" name="CMN_FLE_ALT_TXT_P2_IDX" value="${altTextM1Idx}"/>
								</p>
                    		</td>
                        </tr>
                        <tr>
                                ...
                                <div id="multiFile3-list">
                                    <c:forEach items="${fileList}" var="fileRow" varStatus="i">
                                        <c:if test="${fileRow.CMM_FLE_TB_TYPE == 'P' && fileRow.CMM_FLE_SORT == '2'}">
                                            ...
                                            <input type="checkbox" class="checkbox cmmFleIdx" id="CMM_FLE_IDX_3" name="CMM_FLE_IDX" data-file_name="pcFile" data-file_id="multiFile2" value="<c:out value="${fileRow.CMM_FLE_IDX }"/>" />
                                            <label for="CMM_FLE_IDX_3">삭제</label>
                                        </c:if>
                                    </c:forEach>
                                </div>
                                <input type="file" name="pcFile" id="multiFile2" class="file pcImgFile" data-attach_cnt="1" accept=".jpg, .png , .gif" />
                                ...
                                <div id="multiFile4-list">
                                    <c:forEach items="${fileList}" var="fileRow" varStatus="i">
                                        <c:if test="${fileRow.CMM_FLE_TB_TYPE == 'M' && fileRow.CMM_FLE_SORT == '2'}">
                                            ...
                                            <input type="checkbox" class="checkbox cmmFleIdx" id="CMM_FLE_IDX_4" name="CMM_FLE_IDX" data-file_name="mobileFile" data-file_id="multiFile4" value="<c:out value="${fileRow.CMM_FLE_IDX }"/>" />
                                            <label for="CMM_FLE_IDX_4">삭제</label>
                                        </c:if>
                                    </c:forEach>
                                </div>
                                <input type="file" name="mobileFile" id="multiFile4" class="file mobileImgFile" data-attach_cnt="1" accept=".jpg, .png, .gif" />
                                <small class="desc">이미지 사이즈 : 000*000 / 업로드 가능 파일 : jpg, gif, png / 업로드 용량 : 1MB / 이미지 최대 10개 등록</small>
                                <p class="img-text">
                        ...
                        <tr class="detailTypeScript" <c:if test="${row.CNT_MST_DETAIL_TYPE ne 'S' }">style="display:none"</c:if>>
                            ...
                            <td>
                                <textarea id="CNT_MST_PC_SCRIPT" name="CNT_MST_PC_SCRIPT" rows="8" cols="80">${row.CNT_MST_PC_SCRIPT}</textarea>
                            </td>
                        </tr>
                        <tr class="detailTypeScript" <c:if test="${row.CNT_MST_DETAIL_TYPE ne 'S' }">style="display:none"</c:if>>
                            <th><label class="label"><span>MOBILE 스크립트<i class="require"><em>필수입력</em></i></span></label></th>
                            <td>
                                <textarea id="CNT_MST_MOBILE_SCRIPT" name="CNT_MST_MOBILE_SCRIPT" rows="8" cols="80">${row.CNT_MST_MOBILE_SCRIPT}</textarea>
                            </td>
                        </tr>
                        <tr>
...
$(function(){
	...
	$("#frm").validate({
		rules: 
		{
			...
			CNT_MST_DESC :{required:true},
			CNT_MST_USE_YN : {required:true}
		},
		messages:
		{
			CNT_MST_TITLE:{required:"[제목]은 필수 정보입니다."},
			CNT_MST_DESC :{required:"[설명]은 필수 정보입니다."},
			CNT_MST_USE_YN :{required:"[전시여부]는 필수 정보입니다."}
		}
	});
    ...
    $(document).on("click", "#goRegist", function() {
        ...
        var file3 = $("#multiFile3").val();//Mobile 목록 이미지
        var file4 = $("#multiFile4").val();//Mobile 상세 이미지
        ...
        if(file3 == "" && $("#CMM_FLE_IDX_2").val() == undefined){
        ...
        if(type=="I"){
			...
			if(file4 == "" && $("#CMM_FLE_IDX_4").val() == undefined){
		...
		if($frm.valid())
		{
			...
			{
				$("#frm").attr({"action" : "<c:out value="${serverDomain}" />/am/display/vendorSellerPickModify.do", "target" : "_self", "method" : "post"}).submit();
             }
		}
    });
