<%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8"%>
	...
	<input type="hidden" name="TAG_BND_ID" id="TAG_BND_ID" value="<c:out value="${commandMap.BNR_MST_IDX}"/>">
	<%-- <input type="hidden" name="BNR_MST_IDX" id="BNR_MST_IDX" value="<c:out value="${commandMap.BNR_MST_IDX}"/>"> --%>
	...
	<table cellspacing="0" class="table-row table-a">
		...
		<tbody>
			...
			<tr>
				...
				<td><a href="#" id="searchSellerBtn" class="searchSellerBtn button button-a xsmall">셀러선택</a> <span id="selNM">${row.BND_MST_SEL_NM }</span></td>
			</tr>			
					...
					<div class="tag_cloud_wrap_side cf">
						<div class="tag_cloud_wrap">
							...
							<c:forEach var="item" items="${tagList }" >
								<li class="sel_tag_list txt" name="TAG_MST_ID" value="${item.TAG_MST_ID }">${item.TAG_MST_TITLE}<button type="button" class="btn_del button">X</button></li>
							</c:forEach>
							</ul>
						</div>
				...
				<td>
					...
					<table class="table-row table-a section-form">
						...
						<tbody class="center applyProductList">
							...
							<tr>
							    <td><input type="checkbox" class="checkbox checkRow" name="PRD_MST_CDs" value="${item.BNR_PRD_MST_CD}"/></td>
							    <td><a href="\${serverDomain}/am/product/basicProductEdit.do?PRD_MST_CD=${item.BNR_PRD_MST_CD}" target="_blank" title="새 창 으로 열립니다.">${item.BNR_PRD_MST_CD}</a><input type="hidden" class="PLN_PRD_MST_CD" name="PLN_PRD_MST_CD${i.index }" value="${item.BNR_PRD_MST_CD}"/></td>
							    <td class="left"> <img name="PRD_IMAGE" src="<c:out value="${cdnDomain}" />${item.PRD_IMAGE }"></td>
							    <td>${item.PRD_MST_NM}</td>
							    <td>${item.PRD_MST_PRC}</td>
							    <td>
							    	<c:if test="${item.PRD_MST_SEL_STATE eq 'I'}">판매중</c:if>
							    	<c:if test="${item.PRD_MST_SEL_STATE eq 'T'}">품절</c:if>
							    	<c:if test="${item.PRD_MST_SEL_STATE eq 'S'}">미판매</c:if>
							    </td>
								<td>
									<c:choose>
										<c:when test="${item.PRD_MST_CNL_PC_YN eq 'Y' && item.PRD_MST_CNL_MB_YN eq 'Y' }">전체</c:when>
										<c:otherwise>
