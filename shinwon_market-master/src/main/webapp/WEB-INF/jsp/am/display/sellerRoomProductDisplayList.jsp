...
<div class="contents-inner"><!-- contents-inner -->
	...
	<h3 class="title">
		<span>
		   <c:out value="${row.UPPER_CMN_COM_NM }"/> - <c:out value="${row.CMN_COM_NM }"/> 
		</span>
	</h3>
	...
	<input type="hidden" id="fileRegLimit" name="fileRegLimit" value="<c:out value="${row.CMN_COM_ETC3 }"/>" />
		...
		<div class="table-a"><!-- col list -->
			<table class="table-col table-b">
				...
				<c:forEach var="row" items="${list }" varStatus="i">
					<tr>
						<td class="col-sort" data-dsp_mst_idx="<c:out value="${row.DSP_MST_IDX }"/> " data-bnr_dtl_srt="<c:out value="${row.DSP_MST_SRT }"/> ">
...
$(document).ready(function () {
   ...
   // 페이지 이동
	...
	goRemoveChecked = function(){
		...
		$("#frm").attr({"action":"<c:out value="${serverDomain}" />/am/display/sellerProductDisplayRemove.do", "target":"_self", "method":"post"}).submit();
	...
	fnResultProduct = function(params){
		...
		var vfileRegLimit = $('#fileRegLimit').val() * 1;
		
		/*
		if ( vfileRegLimit != null &&  vfileRegLimit == '') {
			vfileRegLimit =vfileRegLimit *1 ;
		}else{
			vfileRegLimit = 8;
		...
		*/
		...
		alert( 'vfileRegLimit' +vfileRegLimit  );
		debugger;
		var addedCnt = eval(params.length);
		var bCheck = true;
		if($(".PRD_MST_CDs").size()+addedCnt> vfileRegLimit ){
			alert( 'vfileRegLimit' +vfileRegLimit  );
			alert("최대 " +vfileRegLimit + "개까지 등록 가능합니다.");
			bCheck = false;
			return false;
		}
		//상품중복체크
		...
		if( bCheck )
		{
			frm.attr({"action":"<c:out value="${serverDomain}" />/am/display/sellerproductDisplayRegist.do", "target":"_self", "method":"post"}).submit();
	...
	$(document).on("click", "#modifySort", function () {
		if (confirm( "저장 하시겠습니까?")) {
			...
			$(".sort-target").find("tr").each(function (i) {
				...
				idxArray.push($tr.data("dsp_mst_idx"));
			});
			...
			$.ajax ({
			   async : false,
			   type : "POST",
			   url : "<c:out value="${serverDomain}" />/am/display/sellerProductDisplaySortArrayAjax.do", 
