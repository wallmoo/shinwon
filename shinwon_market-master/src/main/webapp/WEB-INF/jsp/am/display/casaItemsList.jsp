            ...
            <div class="contents-inner"><!-- contents-inner -->
                ...
                <div class="tab-a justified" style="margin-top: 20px; width: 100%"><!-- tab -->
		            <ul class="tab">
			            ...
			            <li id="CASA_ITEMS_TAB" class = "active"><a href="javascript:fnCasaChangeTab('ITEMS');" >ITEMS</a></li>
		            </ul>
	            </div><!-- // tab -->
                ...
                <div class="section-form"><!-- section-form-a -->
                    ...
                    <form id="searchForm" name="searchForm">
	                    ...
	                    <div class="form-group">
                            ...
                            <div class="field">
                                <input type="radio" id="stateA" name="searchCbtItmDplYn" class="radio" <c:if test="${empty commandMap.searchCbtItmDplYn }">checked="checked"</c:if> value=""/>
                                <label for="stateA">전체</label>
                                <input type="radio" id="stateB" name="searchCbtItmDplYn" class="radio" <c:if test="${commandMap.searchCbtItmDplYn eq 'Y' }">checked="checked"</c:if> value="Y"/>
                                <label for="stateB">전시</label>
                                <input type="radio" id="stateC" name="searchCbtItmDplYn" class="radio" <c:if test="${commandMap.searchCbtItmDplYn eq 'N' }">checked="checked"</c:if> value="N"/>
                        ...
                        <div class="form-group">
                        	<label class="label"><span>카테고리</span></label>
                            	<div class="field">
    		                    	<select id="searchCbtItmCtg" name="searchCbtItmCtg" class="select" title="카테고리">
    									<option value="">전체</option>
    									<c:forEach items="${categoryList}" var="data">
     									<option value="<c:out value="${data.CMN_COM_IDX}"/>" <ui:select v="${data.CMN_COM_IDX}" t="${commandMap.searchCbtItmCtg}" g="selected"/>>
                            ...
                            <c:choose>
                                <c:when test="${not empty list }">
                                    ...
                                    <c:forEach var="row" items="${list }" varStatus="i">
                                        <tr>
			                                <td>
			                                	<input type="checkbox" class="checkbox checkRow CBT_ITM_IDXs" id="CBT_ITM_IDX<c:out value="${i.index }"/>" name="CBT_ITM_IDXs" value="<c:out value="${row.CBT_ITM_IDX }"/>"/>
			                                	<input type="hidden" name="CBT_ITM_IDX" value="<c:out value="${row.CBT_ITM_IDX }"/>" />
			                                </td>
			                                <td>
			                                	<input type="text" name="CBT_ITM_SRT" id="CBT_ITM_SRT" class="number center short" value="<c:out value="${row.CBT_ITM_SRT }"/>"/>
			                                </td>
			                                <td>
			                                	 <c:out value="${row.CBT_ITM_CTG_NM }"/>
			                                </td>
			                                <td>
                                                <a href="#none" class="list-title" data-cbt_itm_idx="${row.CBT_ITM_IDX }" >
                                                    <c:out value="${row.CBT_ITM_IDX }"/>
                                                </a>
			                                </td>
			                                <td>
			                                	 <c:out value="${row.CBT_ITM_PRD_NM }"/>
			                                </td>
			                                <td>
			                                	 <c:out value="${row.CBT_ITM_PRD_MST_CD }"/>
			                                </td>
			                                 <td><ui:formatDate value="${row.CBT_ITM_REG_DT }" pattern="yyyy.MM.dd HH:mm:ss"/></td>
			                                <td>
                                                <span class="label label-<c:out value="${row.CBT_ITM_DPL_YN eq 'Y'?'b':'a' }"/>">
                                                    <c:out value="${row.CBT_ITM_DPL_YN eq 'Y'?'전시':'미전시' }"/>
                                                </span>
                                            </td>
			                            </tr>
                ...
                <form id="frm" name="frm">
                    <input type="hidden" name="CBT_ITM_IDX" value="" />
...
$(document).ready(function () {
    ...
    $(document).on("click", "#goForm", function () {
        $("#frm").attr({"action":"<c:out value="${serverDomain}" />/am/display/casaItemsForm.do", "target":"_self", "method":"post"}).submit();
    });
    ...
    $(document).on("click", "#modifySrt", function () {
    	...
    	for(var i= 0;i<len;i++)
    	{
    		if($("#CBT_ITM_IDX"+i).is(":checked"))chk++;
    	}
    	...
    	if(confirm("수정하시겠습니까?"))
    	{
        	$("#frmTable").attr({"action":"<c:out value="${serverDomain}" />/am/display/casaItemsSrtMod.do", "target":"_self", "method":"post"}).submit();
    	}
    });
    ...
    $(document).on("click", "#list-field .list-title", function () {
        var $frm = $("#frm");
        $frm.find("input[name='CBT_ITM_IDX']").val($(this).data("cbt_itm_idx"));
        
        $frm.attr({"action":"<c:out value="${serverDomain}" />/am/display/casaItemsEdit.do", "target":"_self", "method":"post"}).submit();
    ...
    goPage = function (cPage) 
    {
        ...
        $frm.attr({"action":"<c:out value="${serverDomain}" />/am/display/casaItemsList.do", "target":"_self", "method":"post"}).submit();
    };
