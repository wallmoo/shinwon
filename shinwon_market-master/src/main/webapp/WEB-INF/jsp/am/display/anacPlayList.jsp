        ...
        <section id="contents"><!-- contents -->
            ...
            <div class="contents-inner"><!-- contents-inner -->
                ...
                <h3 class="title">브랜드전시-아낙</h3>
                 ...
                 <div class="tab-a justified" style="margin-top: 20px; width: 100%"><!-- tab -->
                    <ul class="tab">
                        ...
                        <li id="ANAC_PLAY_TAB" class = "active"><a href="javascript:fnAnacChangeTab('PLAY');" >PLAY</a></li>
                ...
                <div class="section-form"><!-- section-form-a -->
                    ...
                    <form id="searchForm" name="searchForm">
	                    ...
	                    <div class="form-group">
                            ...
                            <div class="field">
                                <input type="radio" id="stateA" name="searchAnaPlyDplYn" class="radio" <c:if test="${empty commandMap.searchAnaPlyDplYn }">checked="checked"</c:if> value=""/>
                                <label for="stateA">전체</label>
                                <input type="radio" id="stateB" name="searchAnaPlyDplYn" class="radio" <c:if test="${commandMap.searchAnaPlyDplYn eq 'Y' }">checked="checked"</c:if> value="Y"/>
                                <label for="stateB">전시</label>
                                <input type="radio" id="stateC" name="searchAnaPlyDplYn" class="radio" <c:if test="${commandMap.searchAnaPlyDplYn eq 'N' }">checked="checked"</c:if> value="N"/>
                                ...
                                <c:when test="${not empty list }">
                                    ...
                                    <c:forEach var="row" items="${list }" varStatus="i">
                                        <tr>
			                                ...
			                                <td>
                                                <a href="#none" class="list-title" data-ana_ply_idx="${row.ANA_PLY_IDX }" >
                                                    <c:out value="${row.ANA_PLY_IDX }"/>
                                                </a>
			                                </td>
			                                <td>
			                                	 <c:if test="${row.ANA_PLY_GBN eq 'C' }">CF</c:if>
			                                	 <c:if test="${row.ANA_PLY_GBN eq 'P' }">상품</c:if>
			                                	 <c:if test="${row.ANA_PLY_GBN eq 'A' }">애니</c:if>
			                                	 <c:if test="${row.ANA_PLY_GBN eq 'B' }">방송</c:if>
			                                </td>
			                                <td>
			                                	 <c:out value="${row.ANA_PLY_VDO }"/>
			                                </td>
			                                <td><ui:formatDate value="${row.ANA_PLY_REG_DT }" pattern="yyyy.MM.dd HH:mm:ss"/></td>
			                                <td>
                                                <span class="label label-<c:out value="${row.ANA_PLY_DPL_YN eq 'Y'?'b':'a' }"/>">
                                                    <c:out value="${row.ANA_PLY_DPL_YN eq 'Y'?'전시':'미전시' }"/>
                                                </span>
                                            </td>
			                            </tr>
                ...
                <form id="frm" name="frm">
                    <input type="hidden" name="ANA_PLY_IDX" value="" />
...
$(document).ready(function () {
    ...
    $(document).on("click", "#goForm", function () {
        $("#frm").attr({"action":"<c:out value="${serverDomain}" />/am/display/anacPlayForm.do", "target":"_self", "method":"post"}).submit();
    });
    
    // 제목 a Tag 클릭 이벤트
    $(document).on("click", "#list-field .list-title", function () {
        var $frm = $("#frm");
        $frm.find("input[name='ANA_PLY_IDX']").val($(this).data("ana_ply_idx"));
        
        $frm.attr({"action":"<c:out value="${serverDomain}" />/am/display/anacPlayEdit.do", "target":"_self", "method":"post"}).submit();
    ...
    goPage = function (cPage) 
    {
        ...
        $frm.attr({"action":"<c:out value="${serverDomain}" />/am/display/anacPlayList.do", "target":"_self", "method":"post"}).submit();
    };
