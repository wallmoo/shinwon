			...
			<table class="core">
				...
				<c:choose>
				    <c:when test="${not empty list}">
				    	...
				    	<c:forEach var="regularStoreRow" items="${list }" varStatus="i">
							<tr><!-- row -->
				                <td>
				                	<c:out value="${regularStoreRow.BCNC_CD }"/>
				                </td>
				                <td>
				                	<a href="javascript:goReturn('<c:out value="${regularStoreRow.BCNC_CD }" />','<c:out value="${regularStoreRow.BCNC_NM }" />');"><c:out value="${regularStoreRow.BCNC_NM }" /></a>
				                </td>
			            	</tr><!-- // row -->	            		
	            		</c:forEach>
				    </c:when>
...
$(document).ready(function () {
    ...
    search = function() 
    {
        ...
        $searchForm.attr({"action":"<c:out value="${serverDomain}" />/am/stats/orderStatsRegularStorePopup.do", "target":"_self", "method":"post"}).submit(); 
    };
    ...
    goPage = function (cPage) 
    {
        ...
        $frm.attr({"action":"<c:out value="${serverDomain}"/>/am/stats/orderStatsRegularStorePopup.do", "target":"_self", "method":"post"}).submit();
    };
    
    // 매장 선택 
    goReturn = function (bcncCd,bcncNm)
    {
        var obj = new Object();
        obj.SHOP_CD = bcncCd;
        obj.SHOP_NM = bcncNm;

        if(	window.opener.fnResultStore(obj) )    	
    		self.close();
    };
