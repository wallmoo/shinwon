...
@Controller("pcMyDeliveController")
public class MyDeliveController {
	...
	*/
    ...
    @RequestMapping("/pc/mypage/myDeliveList")
	...
	/**
	* <pre>
	* 1. MethodName : myDelivePopup
	* 2. ClassName  : MyDeliveController.java
	* 3. Comment    : 나의 배송지 추가/수정 팝업
	* 4. 작성자       : kanghk
	* 5. 작성일       : 2014. 7. 21.
	* </pre>
	*
	...
	*/
	@AccessLevelUser(ROLEUSER.user)
    @RequestMapping("/pc/mypage/myDelivePopup")
    public ModelAndView myDelivePopup (HttpServletRequest request, @RequestParams Map<String, Object> commandMap) throws Exception
    {
        if(SessionsUser.isLogin(request))
        {
            ModelAndView mv = new ModelAndView();
            ...
            // 해당 브랜드로 이동
            mv.setViewName("pc/mypage/myDelivePopup");
	...
	*/
	@AccessLevelUser(ROLEUSER.user)
    @RequestMapping("/pc/mypage/myDeliveProc")
        ...
        if(SessionsUser.isLogin(request))
        {
            ...
            if("N".equals(pageStatus)){
            	...
            	if(result > 0){
            		...
            		mv.addObject("returnUrl", StringUtil.getString(request.getAttribute("frontDomain"), "") + "/pc/mypage/myDeliveList.do");
            	}else{
            ...
            if("N".equals(pageStatus) || "M".equals(pageStatus)){//주문페이지에서는 reload 안한다.
	...
	*/
	@AccessLevelUser(ROLEUSER.user)
    @RequestMapping("/pc/mypage/myDeliveBaseAddrProc")
	...
	*/
	@AccessLevelUser(ROLEUSER.user)
    @RequestMapping("/pc/mypage/myDeliveRemove")
