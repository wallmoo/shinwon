...
@Controller("pcEstimateManagementController")
public class EstimateManagementController {
	 ...
	 * @throws Exception
	 */
	@RequestMapping("/pc/product/myEstimateMain")
	public ModelAndView myEstimateMain(HttpServletRequest request, @RequestParams() Map<String, Object> commandMap) throws Exception{
		...
		if(SessionsUser.isLogin(request) ){
			...
			totalCount				= this.pcEstimateManagementService.selectEsmCartCnt(commandMap);
			List list				= this.pcEstimateManagementService.selectMyEstimateList(commandMap);
			List optList			= this.pcEstimateManagementService.selectEsmCartOptDetailList(commandMap);
		...
		mv.setViewName("/pc/product/myEstimateMain");
			
//		}else{
//			commandMap.put("MEM_LOGIN_YN", "N");
//			
//			commandMap.put("url", request.getAttribute("frontDomain") + "/pc/product/myEstimateMain.do");
...
//		}
	 ...
	 * 2. ClassName : EstimateManagementController.java
	 * 3. Comment : 나만의 맞춤 견적 > keeping 상품 리스트 ajax
	 * 4. 작성자 : Lee
	 * 5. 작성일 : 2016. 1. 28.
	 * </pre>
	 *
	 ...
	 * @throws Exception
	 */
	@RequestMapping("/pc/product/myEstimateKeepingAjax")
	public JSON myEstimateKeepingAjax(HttpServletRequest request, @RequestParams() Map<String, Object> commandMap) throws Exception{
		...
		if("Y".equals(strMemLoginYN)){
			...
			int totalCount		= this.pcEstimateManagementService.selectMyEstimateKeepingListCount(commandMap);
			...
			if(totalCount > 0){
	        	...
	        	commandMap.put("IMG_THUMBNAIL_SIZE_BIG", 	Code.THUMBNAIL_SIZE_PC_PRODUCT_ITEM_BIG);
	        	
				List list		= this.pcEstimateManagementService.selectMyEstimateKeepingList(commandMap);
	 ...
	 * 2. ClassName : EstimateManagementController.java
	 * 3. Comment : 나만의 맞춤 견적 > 장바구니 상품 리스트 ajax
	 * 4. 작성자 : Lee
	 * 5. 작성일 : 2016. 1. 28.
	 * </pre>
	 *
	 ...
	 * @throws Exception
	 */
	@RequestMapping("/pc/product/myEstimateCartAjax")
	public JSON myEstimateCartAjax(HttpServletRequest request, @RequestParams() Map<String, Object> commandMap) throws Exception{
		...
		if("Y".equals(strMemLoginYN)){
			...
			int totalCount		= this.pcEstimateManagementService.selectMyEstimateCartListCount(commandMap);
			...
			if(totalCount > 0){
				...
				List list		= this.pcEstimateManagementService.selectMyEstimateCartList(commandMap);
				json.addObject("list", 						list);
	 ...
	 * 2. ClassName : EstimateManagementController.java
	 * 3. Comment : 나만의 맞춤 > 카테고리 selecbox 리스트 
	 * 4. 작성자 : Lee
	 * 5. 작성일 : 2016. 1. 28.
	 * </pre>
	 *
	 ...
	 * @throws Exception
	 */
	@RequestMapping("/pc/product/myEstimateCateAjax")
	 ...
	 * @throws Exception
	 */
	@RequestMapping("/pc/product/myEstimateCateSearchAjax")
	public JSON myEstimateCateSearchAjax(HttpServletRequest request, @RequestParams() Map<String, Object> commandMap) throws Exception{
    	...
    	if(!"".equals(searchPrdCtgIdx)){
    		commandMap.put("searchPrdCtgIdx", searchPrdCtgIdx);
    	}
    	...
    	commandMap.put("searchPrdCtgEnv", Code.USER_ENV_TYPE_PC);
    	commandMap.put("USER_ENV_TYPE", Code.USER_ENV_TYPE_PC);
    	commandMap.put("CART_PC_MOBILE_GUBUN", Code.USER_ENV_TYPE_PC);
    	commandMap.put("PRD_CTG_DSP_YN", "Y");
		...
		if("Y".equals(strMemLoginYN)){
			...
			int totalCount		= this.pcEstimateManagementService.selectMyEstimateCateListCount(commandMap);
			...
			if(totalCount > 0){
				...
				List list		= this.pcEstimateManagementService.selectMyEstimateCateList(commandMap);
				json.addObject("list", 						list);
	 ...
	 * @throws Exception
	 */
	@RequestMapping(value = "/pc/product/insertEsmCartAjax", method = RequestMethod.POST)
	public JSON insertCartAjax(HttpServletRequest request,
		...
		json.addObject("cartCnt", String.valueOf(this.pcEstimateManagementService.selectEsmCartCnt(paramMap)));    	
	 ...
	 * 
	 * <pre>
	 * 1. MethodName : myEstimateListAjax
	 * 2. ClassName : EstimateManagementController.java
	 * 3. Comment : 나만의 맞춤견적 > 나만의견적서 리스트  
	 * 4. 작성자 : Lee
	 * 5. 작성일 : 2016. 1. 29.
	 * </pre>
	 *
	 ...
	 * @throws Exception
	 */
	@RequestMapping("/pc/product/myEstimateListAjax")
	public JSON myEstimateListAjax(HttpServletRequest request, @RequestParams Map<String, Object> commandMap) throws Exception {
		JSON json 		= new JSON();
		...
		int totalCount			= this.pcEstimateManagementService.selectEsmCartCnt(commandMap);
		List list				= this.pcEstimateManagementService.selectMyEstimateList(commandMap);
		List optList			= this.pcEstimateManagementService.selectEsmCartOptDetailList(commandMap);
		
		json.addObject("totalCount", totalCount);
		json.addObject("list", list);
		json.addObject("optList", optList);
//		if((ROLEUSER.user.toString()).equals(user.getROLE_USER().toString())){
//			commandMap.put("MEM_MST_MEM_ID", user.getMEM_MST_MEM_ID());
//		}
	 ...
	 * @throws Exception
	 */
	@RequestMapping("/pc/product/myEstimateDelAjax")
	public JSON myEstimateDelAjax(HttpServletRequest request, @RequestParams Map<String, Object> commandMap) throws Exception {
      ...
      User user = SessionsUser.getSessionUser(request);
		...
		int result		= this.pcEstimateManagementService.deleteMyEstimateDelAjax(commandMap);
	 ...
	 * @throws Exception
	 */
	@RequestMapping("/pc/product/myEstimateProcess")
	public ModelAndView myEstimateProcess(HttpServletRequest request, @RequestParams Map<String, Object> commandMap
        ...
        User user = SessionsUser.getSessionUser(request);
      	...
      	if(SessionsUser.isLogin(request) ){
			...
			result		= this.pcEstimateManagementService.insertMyEstimateProcess(commandMap);
...
//      mv.addObject("returnUrl", StringUtil.getString(request.getAttribute("frontDomain"), "") + "/pc/product/estimateMain.do");
	 ...
	 * @throws Exception
	 */
	@RequestMapping("/pc/product/estimateProcess")
	 ...
	 * @throws Exception
	 */
	@RequestMapping("/pc/product/estimateMain")
	public ModelAndView estimateMain(HttpServletRequest request, @RequestParams() Map<String, Object> commandMap) throws Exception{
		...
		if(SessionsUser.isLogin(request) ){
			...
			commandMap.put("PRD_MST_CNL_PC_YN", Code.USER_ENV_TYPE_PC);
			commandMap.put("USER_ENV_TYPE", Code.USER_ENV_TYPE_PC);
			commandMap.put("CART_PC_MOBILE_GUBUN", Code.USER_ENV_TYPE_PC); 
		...
		mv.addObject("commandMap", commandMap);
		mv.setViewName("/pc/product/estimateMain");
			
//		}else{
//			commandMap.put("MEM_LOGIN_YN", "N");
//			
//			commandMap.put("url", request.getAttribute("frontDomain") + "/pc/product/estimateMain.do");
//            mv.addObject("returnUrl", request.getAttribute("serverSslDomain") + "/pc/manager/login.do");
//            // 해당 브랜드로 이동
//            mv.setViewName("pc/common/result");
//		}
