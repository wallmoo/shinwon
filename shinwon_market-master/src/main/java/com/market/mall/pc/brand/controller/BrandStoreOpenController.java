 ...
 * @Date : 2015. 6. 11.
 * @프로그램 설명 : 사용자 > 브랜드사이트 > 매장개설 관련하여 처리하는 Controller Class
 * @author DEV_KIMSOYOUNG
 */

@Controller("pcBrandStoreOpenController")
public class BrandStoreOpenController 
{
	...
	* @throws Exception
	*/
	@RequestMapping(value="/pc/brand/brandStoreOpenMarmot")
	public ModelAndView brandStoreOpenMarmot (HttpServletRequest request, @RequestParams Map<String,Object> commandMap) throws Exception
	{
		...
		// 해당 브랜드로 이동
        mv.setViewName("pc/brand/brandStoreOpenMarmot");
	...
	* @throws Exception
	*/
	@RequestMapping(value="/pc/brand/brandStoreOpenForm")
	public ModelAndView brandStoreOpenForm (HttpServletRequest request, @RequestParams Map<String,Object> commandMap) throws Exception
	{
		if(SessionsUser.isLogin(request))
        {
			...
			// 해당 브랜드로 이동
	        mv.setViewName("pc/brand/brandStoreOpenForm");
	...
	* @throws Exception
	*/
	@RequestMapping(value="/pc/brand/brandStoreOpenRegist", method = RequestMethod.POST)
	public ModelAndView brandStoreOpenRegist (HttpServletRequest request, @RequestParams Map<String,Object> commandMap) throws Exception
	{
		if(SessionsUser.isLogin(request))
        {
			...
			if(result > 0)
			{
				...
				mv.addObject("returnUrl", StringUtil.getString(request.getAttribute("frontDomain"), "") + "/pc/brand/brandStoreOpenMarmot.do");
			}
			else
			{
	...
	* @throws Exception
	*/
	@RequestMapping(value="/pc/brand/brandStoreOpenMountia")
	public ModelAndView brandStoreOpenMountia (HttpServletRequest request, @RequestParams Map<String,Object> commandMap) throws Exception
	{
		...
		// 해당 브랜드로 이동
        mv.setViewName("pc/brand/brandStoreOpenMountia");
