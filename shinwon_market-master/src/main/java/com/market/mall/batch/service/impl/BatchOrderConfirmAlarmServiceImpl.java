 ...
 * @FileName : BatchOrderConfirmAlarmServiceImpl.java
 * @Date : 2016. 3. 15.
 * @프로그램 설명 : 배송완료+1 구매확정 요청 메일 발송 Class
 * @author DEV_KANGHYUNKYU
 */
@Service("batchOrderConfirmAlarmService")
public class BatchOrderConfirmAlarmServiceImpl implements BatchOrderConfirmAlarmService{
	...
	* 2. ClassName  : BatchOrderConfirmAlarmServiceImpl.java
	...
	*/
	public Map<String, Object> updateOrderConfirmAlarm(Map<String, Object> commandMap) throws Exception{
		...
		List<Map<String, Object>> list =  defaultDAO.selectList("BatchOrderConfirmAlarm.selectOrderConfirmAlarmList",commandMap);    	
    	...
    	for( Map<String,Object>orderMap : list )
    	{
    		...
    		try{
				...
				body += VelocityEngineUtils.mergeTemplateIntoString(
						velocityEngine, "/MAIL-ORDER-ALARM-CONFIRM.vm", "utf-8", mailContentsMap);
				body += VelocityEngineUtils.mergeTemplateIntoString(
				...
				mailMap.put("MAIL_CODE", "18");	// 이메일 코드
	    		...
	    		mailMap.put("EMAIL_TITLE", StringUtil.getString(propertiesService.getString("app.name")+" ","") + FormatUtil.markingName(StringUtil.getString(orderPayInfo.get("ORD_MST_ORD_NM")))  + "님, 주문하신 상품은 잘 받으셨나요?");	// 메일 제목
