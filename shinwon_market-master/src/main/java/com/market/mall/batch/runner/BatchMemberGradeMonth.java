 ...
 * @FileName : BatchMemberGradeMonth.java
 * @Date : 2016. 1. 20.
 * @프로그램 설명 : Batch > 개인회원(온라인 통합회원) 최근3개월 구매금액에 따른 등급 변경 처리하는 Class
 * @author : DEV_LEEKWANGWOON
 */
@Component
public class BatchMemberGradeMonth{
	@Resource(name="propertiesService") protected EgovPropertyService propertiesService;
	@Resource(name="batchManagerService") private BatchManagerService batchManagerService;
    @Resource(name="batchMemberGradeMonthService") private BatchMemberGradeMonthService batchMemberGradeMonthService;
    private Logger logger = LoggerFactory.getLogger(this.getClass());
    
    /**
     * <pre>
     * 1. MethodName : updateMemberGradeMonth
     * 2. ClassName  : BatchMemberGradeMonth.java
	...
	public void updateMemberGradeMonth() throws Exception{
		...
		commandMap.put("BCH_MST_ID", Code.BATCH_CODE_MEMBER_MONTH_GRADE);  // 배치 아이디 	- 24
		commandMap.put("BCH_MST_NM", Code.BATCH_NAME_MEMBER_MONTH_GRADE);  // 배치 명 - [관리] 회원등급(월단위) 갱신
		...
		if( totalCount == 0 ){	        
	    	...
	    	try{
	    		// 배치작업.
	    		resultMap = this.batchMemberGradeMonthService.updateMemberGradeMonth(commandMap);
	    		json.put("resultYn","Y");
