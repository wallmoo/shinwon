 ...
 * @FileName : BatchMemberBlackCancel.java
 * @Date : 2014. 11. 19.
 * @프로그램 설명 : Batch > 블랙리스트 종료 회원 상태값 원복을 처리하는 Class
 * @author DEV_NAMGIUNG
 */
@Component
public class BatchMemberBlackCancel {
	
	@Resource(name="propertiesService") protected EgovPropertyService propertiesService;
	@Resource(name="batchManagerService") private BatchManagerService batchManagerService;
    @Resource(name="batchMemberBlackCancelService") private BatchMemberBlackCancelService batchMemberBlackCancelService;
     ...
     * 
     * <pre>
     * 1. MethodName	: memberBlackCancel
     * 2. ClassName		: BatchMemberBlackCancel.java
     * 3. Commnet		: 블랙리스트 종료 회원 상태값 원복
     * 4. 작성자		: DEV_NAMGIUNG
     * 5. 작성일		: 2015. 12. 21. 오전 11:17:09
     * </pre>
     *
     * @return void
     * @throws Exception
    ...
    //@Scheduled(cron="0 40 3 * * *") // 매일 새벽 3시 40분! 
    //@Scheduled(cron="0 0 * * * *")
	public void memberBlackCancel() throws Exception{
...
//		}
		...
		commandMap.put("BCH_MST_ID", Code.BATCH_CODE_BLACK_STATE); 	// 배치 아이디 	- "03"
		commandMap.put("BCH_MST_NM", Code.BATCH_NAME_BLACK_STATE); 	// 배치 명 		- [관리] 블랙리스트 기한종료 상태값변경"
        ...
        if( totalCount == 0 ){	        
	    	...
	    	try{
	    		// 배치작업.
	    		resultMap = this.batchMemberBlackCancelService.updateMemberState(commandMap);
	    		json.put("resultYn","Y");
