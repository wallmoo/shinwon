...
import com.market.mall.batch.service.BatchOrderCountService;
 ...
 * @FileName : BatchOrderCount.java
 * @Date : 2014. 11. 19.
 * @프로그램 설명 : Batch > 상품테이블 판매갯수 업데이트 처리하는 Class
 * @author LeeJongHo
 */
@Component
public class BatchOrderCount {
	
	@Resource(name="propertiesService") protected EgovPropertyService propertiesService;
	@Resource(name="batchManagerService") private BatchManagerService batchManagerService;
    @Resource(name="batchOrderCountService") private BatchOrderCountService batchOrderCountService;

    /**
     * 
     * <pre>
     * 1. MethodName	: OrderCount
     * 2. ClassName		: BatchOrderCount.java
     * 3. Commnet		: 상품테이블 판매갯수 업데이트
     * 4. 작성자			: LeeJongHo
    ...
    //@Scheduled(cron="0 30 8 * * *") // 매일 새벽 1시 0분
	public void OrderCount() throws Exception{
		...
		*/
		...
		commandMap.put("BCH_MST_ID", Code.BATCH_CODE_UPDATE_ORDER_COUNT); 	// 배치 아이디 	- 19
		commandMap.put("BCH_MST_NM", Code.BATCH_NAME_UPDATE_ORDER_COUNT); 	// 배치 명 		- [관리] 상품 판매갯수 업데이트
        ...
        if( totalCount == 0 ){	        
	    	...
	    	try{
	    		// 배치작업.
	    		resultMap = this.batchOrderCountService.updateOrderCount(commandMap);
	    		json.put("resultYn","Y");
