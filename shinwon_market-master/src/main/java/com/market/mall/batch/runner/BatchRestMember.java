 ...
 * @FileName : BatchRestMember.java
 * @Date : 2014. 11. 19.
 * @프로그램 설명 : Batch > 휴면회원 정보 이관을 처리하는 Class
 * @author DEV_NAMGIUNG
 */
@Component
public class BatchRestMember {
	
	@Resource(name="propertiesService") protected EgovPropertyService propertiesService;
	@Resource(name="batchManagerService") private BatchManagerService batchManagerService;
    @Resource(name="batchRestMemberService") private BatchRestMemberService batchRestMemberService;
     ...
     * 
     * <pre>
     * 1. MethodName	: restMember
     * 2. ClassName		: BatchRestMember.java
     * 3. Commnet		: 휴면회원 정보 이관
     * 4. 작성자		: DEV_NAMGIUNG
	...
	public void restMember() throws Exception{
...
//		}
		...
		commandMap.put("BCH_MST_ID", Code.BATCH_CODE_REST_CHANGE); 	// 배치 아이디 	- "04"
		commandMap.put("BCH_MST_NM", Code.BATCH_NAME_REST_CHANGE); 	// 배치 명 		- [관리] 휴면회원 전환
        ...
        if( totalCount == 0 ){	        
	    	...
	    	try{
	    		// 배치작업.
	    		resultMap = this.batchRestMemberService.transMember(commandMap);
	    		json.put("resultYn","Y");
