...
public class BatchEpNaverAll {
    ...
    //@Scheduled(cron="0 */1 * * * *")
    //@Scheduled(cron="0 30 7 * * * ")	//실제설정
	public void BatchEpNaverAll() throws Exception{
        logger.info("===============================");
		// 서버IP 확인
...
//		}
		...
		commandMap.put("BCH_MST_ID", Code.BATCH_CODE_EP_ALL_NAVER); 	// 배치 아이디 	- "57"
		commandMap.put("BCH_MST_NM", Code.BATCH_NAME_EP_ALL_NAVER); 	// 배치 명 		- [관리] 네이버 EP 데이터 전체분 생성
...
//	    		this.batchWeatherApiService.insertWeather();
	    		naverEPService.makeNaverAllProductEP();
	    		json.put("resultYn","Y");
