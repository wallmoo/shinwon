...
public class BatchEpEnuriAll {
    ...
    //@Scheduled(cron="0 */1 * * * *")
    //@Scheduled(cron="21 0 * * * *")	//실제설정
	public void BatchEpEnuriAll() throws Exception{
        logger.info("===============================");
		// 서버IP 확인
...
//		}
		...
		commandMap.put("BCH_MST_ID", Code.BATCH_CODE_EP_ALL_ENURI); 	// 배치 아이디 	- "59"
		commandMap.put("BCH_MST_NM", Code.BATCH_NAME_EP_ALL_ENURI); 	// 배치 명 		- [관리] 에누리 EP 데이터 전체분 생성
...
//	    		this.batchWeatherApiService.insertWeather();
	    		enuriEPService.makeEnuriAllProductEP();
	    		json.put("resultYn","Y");
