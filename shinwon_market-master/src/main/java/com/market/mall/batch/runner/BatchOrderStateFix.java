 ...
 * 
    ...
    * @FileName : BatchOrderStateEnd.java
    * @Date : 2016. 1. 13.
    * @프로그램 설명 : Batch > 주문 배송완료 후 구매확정(7일) 처리하는 Class
    * @author : DEV_LEEKWANGWOON
 */
@Component
public class BatchOrderStateFix{
	@Resource(name="propertiesService") protected EgovPropertyService propertiesService;
	@Resource(name="batchManagerService") private BatchManagerService batchManagerService;
    @Resource(name="batchOrderStateFixService") private BatchOrderStateFixService batchOrderStateFixService;
     ...
     * 
        ...
        * 2. ClassName  : BatchOrderStateFix.java
        * 3. Comment    : 주문 배송완료 후 주문확정(7일)
	...
	public void updateOrderStateEnd() throws Exception{
		...
		*/
		...
		commandMap.put("BCH_MST_ID", Code.BATCH_CODE_UPDATE_ORDER_STATE); // 배치 아이디 	- 22
		commandMap.put("BCH_MST_NM", Code.BATCH_NAME_UPDATE_ORDER_STATE); // 배치 명 - [관리] 주문 배송완료 후 구매확정 업데이트
        ...
        if( totalCount == 0 ){	        
	    	...
	    	try{
	    		// 배치작업.
	    		resultMap = this.batchOrderStateFixService.updateOrderStateEnd(commandMap);
	    		json.put("resultYn","Y");
