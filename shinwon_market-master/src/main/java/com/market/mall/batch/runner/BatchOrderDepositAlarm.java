 ...
 * @FileName : BatchOrderDepositAlarm.java
 * @Date : 2016. 3. 14.
 * @프로그램 설명 : Batch > 주문 입금요청 처리하는 Class
 * @author DEV_KANGHYUNKYU
 */
@Component
public class BatchOrderDepositAlarm {

	@Resource(name="propertiesService") protected EgovPropertyService propertiesService;
	@Resource(name="batchManagerService") private BatchManagerService batchManagerService;
	@Resource(name="batchOrderDepositAlarmService") private BatchOrderDepositAlarmService batchOrderDepositAlarmService;

    /**
     * 
        * <pre>
        * 1. MethodName : updateOrderDepositAlarm
        * 2. ClassName  : BatchOrderDepositAlarm.java
        * 3. Comment    : 주문 입금요청
	...
	public void updateOrderDepositAlarm() throws Exception{
		...
		*/
		...
		commandMap.put("BCH_MST_ID", Code.BATCH_CODE_ORDER_DEPOSIT_ALARM); // 배치 아이디 	- 44
		commandMap.put("BCH_MST_NM", Code.BATCH_NAME_ORDER_DEPOSIT_ALARM); // 배치 명 - [관리] 입금대기 주문 입금요청
        ...
        if( totalCount == 0 ){	        
	    	...
	    	try{
	    		// 배치작업.
	    		resultMap = this.batchOrderDepositAlarmService.updateOrderDepositAlarm(commandMap);
	    		json.put("resultYn","Y");
