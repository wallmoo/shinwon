		...
		try{
			...
			if(emailSendYn.equals("Y")){
				...
				if("0".equals(mailMap.get("resultCode"))){
					...
					int tr_num = defaultDAO.insert("BatchEMAILSEND.insertEmailSendForm", mailMap); // 이벤트발송 마스터 Insert
					logger.info("tr_num >> " + tr_num);

					mailMap.put("SEQ"	,String.valueOf(tr_num));
					defaultDAO.insert("BatchEMAILSEND.insertEmailSendList", mailMap);	// 이벤트발송 서브 Insert
					
					defaultDAO.update("BatchEMAILSEND.updateEmailSendForm", mailMap);	// 이벤트발송 마스터 Update
