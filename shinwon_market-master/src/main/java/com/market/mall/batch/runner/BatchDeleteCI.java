 ...
 * @FileName : BatchDeleteCI.java
 * @Date : 2015. 12. 21.
 * @프로그램 설명 : Batch > 탈퇴회원 CI 값 삭제를 처리하는 Class
 * @author DEV_NAMGIUNG
 */
@Component
public class BatchDeleteCI {
	
	@Resource(name="propertiesService") protected EgovPropertyService propertiesService;
	@Resource(name="batchManagerService") private BatchManagerService batchManagerService;
    @Resource(name="batchCIDeleteService") private BatchDeleteCIService batchCIDeleteService;
     ...
     * 2. ClassName		: BatchDeleteCI.java
     * 3. Commnet		: 탈퇴회원 CI 값 삭제
     * 4. 작성자		: DEV_NAMGIUNG
    ...
    //@Scheduled(cron="0 20 3 * * *") // 매일 새벽 3시 20분!  
...
//		}
		...
		commandMap.put("BCH_MST_ID", Code.BATCH_CODE_DELETE_CI); 	// 배치 아이디 	- "02"
		commandMap.put("BCH_MST_NM", Code.BATCH_NAME_DELETE_CI); 	// 배치 명 		- [관리] 탈퇴회원 CI값 삭제
        ...
        if( totalCount == 0 ){	        
	    	...
	    	try{
	    		// 배치작업.
	    		resultMap = this.batchCIDeleteService.deleteCI(commandMap);
	    		json.put("resultYn","Y");
