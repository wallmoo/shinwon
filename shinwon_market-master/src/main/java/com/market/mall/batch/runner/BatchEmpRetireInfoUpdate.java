...
@Component
public class BatchEmpRetireInfoUpdate {
	@Resource(name="propertiesService") protected EgovPropertyService propertiesService;
	@Resource(name="batchManagerService") private BatchManagerService batchManagerService;
    @Resource(name="batchEmpRetireInfoUpdateService") private BatchEmpRetireInfoUpdateService batchEmpRetireInfoUpdateService;
    private Logger logger = LoggerFactory.getLogger(this.getClass());
    
    /**
     * <pre>
     * 1. MethodName : updateEmpRetireInfo
     * 2. ClassName  : BatchEmpRetireInfoUpdate.java
     * 3. Comment    : 개인회원 퇴직자처리 변경
	...
	public void updateEmpRetireInfo() throws Exception{
		...
		commandMap.put("BCH_MST_ID", "100");  // 배치 아이디 	- 24
		commandMap.put("BCH_MST_NM", "[관리] 퇴직자(월단위) 갱신");  // 배치 명 - [관리] 회원등급(월단위) 갱신
		...
		if( totalCount == 0 ){	        
	    	...
	    	try{
	    		// 배치작업.
	    		resultMap = this.batchEmpRetireInfoUpdateService.updateEmpRetireInfoMonth(commandMap);
	    		json.put("resultYn","Y");
