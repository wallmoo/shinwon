...
public class BatchEpDanawaAll {
    ...
    //@Scheduled(cron="0 */1 * * * *")
    //@Scheduled(cron="0 35 7 * * *")	//실제설정
	public void BatchEpDanawaAll() throws Exception{
        logger.info("===============================");
		// 서버IP 확인
...
//		}
		...
		commandMap.put("BCH_MST_ID", Code.BATCH_CODE_EP_ALL_DANAWA); 	// 배치 아이디 	- "55"
		commandMap.put("BCH_MST_NM", Code.BATCH_NAME_EP_ALL_DANAWA); 	// 배치 명 		- [관리] 다나와 EP 데이터 전체분 생성
...
//	    		this.batchWeatherApiService.insertWeather();
	    		danawaEPService.makeDanawaAllProductEP();
	    		json.put("resultYn","Y");
