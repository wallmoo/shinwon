 ...
 */
@Component
public class BatchBestIcon {
	
	@Resource(name="propertiesService") protected EgovPropertyService propertiesService;
	@Resource(name="batchManagerService") private BatchManagerService batchManagerService;
    @Resource(name="batchBestIconService") private BatchBestIconService batchBestIconService;

    /**
     * 
     * <pre>
     * 1. MethodName	: updateBestIcon
     * 2. ClassName		: BatchBestIcon.java
     * 3. Commnet		: 베스트 아이콘 생성
     * 4. 작성자			: LeeJongHo
	...
	public void updateBestIcon() throws Exception{
		...
		*/
		...
		commandMap.put("BCH_MST_ID", Code.BATCH_CODE_UPDATE_BEST_ICON); 	// 배치 아이디 	- 20
		commandMap.put("BCH_MST_NM", Code.BATCH_NAME_UPDATE_BEST_ICON); 	// 배치 명 		- [관리] 상품 베스트 아이콘 업데이트
        ...
        if( totalCount == 0 ){	        
	    	...
	    	try{
	    		// 배치작업.
	    		resultMap = this.batchBestIconService.updateBestIcon(commandMap);
	    		json.put("resultYn","Y");
