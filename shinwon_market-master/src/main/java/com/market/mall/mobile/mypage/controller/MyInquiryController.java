...
@Controller("mobileMyInquiryController")
public class MyInquiryController {
	...
	*/
    ...
    @RequestMapping("/mobile/mypage/myInquiryList")
	...
	*/
	@AccessLevelUser(ROLEUSER.user)
	@RequestMapping("/mobile/mypage/myInquiryForm")
	...
	*/
	@FrontBannerSkip()
	@AccessLevelUser(ROLEUSER.user)
	@RequestMapping("/mobile/mypage/inquiryRegist")
	public ModelAndView inquiryRegist (HttpServletRequest request, @RequestParams Map<String, Object> commandMap) throws Exception
	{
		if(SessionsUser.isLogin(request))
        {
			...
			commandMap.put("MEM_ADV_CHN_GBN", Code.USER_ENV_TYPE_MOBILE);//문의채널
	        ...
	        if(result > 0)
	        {
	            ...
	            mv.addObject("returnUrl", StringUtil.getString(request.getAttribute("mobileDomain"), "") + "/mobile/mypage/myInquiryList.do");
	        }
	        else
	        {
	...
	*/
	@AccessLevelUser(ROLEUSER.user)
	@RequestMapping("/mobile/mypage/myInquiryView")
	...
	*/
	@AccessLevelUser(ROLEUSER.user)
	@RequestMapping("/mobile/mypage/myInquiryEdit")
	...
	*/
	@AccessLevelUser(ROLEUSER.user)
	@RequestMapping("/mobile/mypage/myInquiryModify")
	public ModelAndView myInquiryModify (HttpServletRequest request, @RequestParams Map<String, Object> commandMap) throws Exception
	{
		if(SessionsUser.isLogin(request))
        {
			...
			if(result > 0)
			{
				...
				mv.addObject("returnUrl", StringUtil.getString(request.getAttribute("mobileDomain"), "") + "/mobile/mypage/myInquiryView.do");
				
			}
			else
			{
				mv.addObject("alertMsg", "수정실패하였습니다.");
				mv.addObject("returnUrl", StringUtil.getString(request.getAttribute("mobileDomain"), "") + "/mobile/customer/myInquiryView.do");
			}
			mv.addObject("commandMap", commandMap);
			// 해당 브랜드로 이동
	        mv.setViewName("mobile/common/result");
	...
	*/
	@AccessLevelUser(ROLEUSER.user)
	@RequestMapping("/mobile/mypage/myInquiryRemove")
	...
	*/
	@FrontBannerSkip()
	@RequestMapping(value="/mobile/mypage/getSubCodeAjax.do")
